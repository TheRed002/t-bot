...
class TestEmergencyControls:
    ...
    async def test_activate_emergency_stop_failure(self, emergency_controls):
        ...
        emergency_controls._execute_emergency_procedures = AsyncMock(side_effect=Exception("Test error"))
    ...
    @pytest.mark.asyncio
    async def test_validate_order_during_emergency_recovery_mode(self, emergency_controls, mock_exchange):
        ...
        order = OrderRequest(
            ...
            side=OrderSide.BUY,
            order_type=OrderType.MARKET,
            quantity=Decimal("0.01"),  # Small order (0.01 BTC = $500 out of $10k = 5%)
            price=Decimal("50000")
        )
    ...
    async def test_deactivate_emergency_stop(self, emergency_controls):
        ...
        # Set emergency state
        emergency_controls.state = EmergencyState.EMERGENCY
        emergency_controls.emergency_start_time = datetime.now()
    ...
    @pytest.mark.asyncio
    async def test_validate_recovery_completion_success(self, emergency_controls, mock_risk_manager):
    ...
    @pytest.mark.asyncio
    async def test_validate_recovery_completion_high_risk(self, emergency_controls, mock_risk_manager):
    ...
    @pytest.mark.asyncio
    async def test_validate_recovery_completion_circuit_breakers_triggered(self, emergency_controls):
        """Test recovery completion validation with triggered circuit breakers."""
        # Mock triggered circuit breakers
        emergency_controls.circuit_breaker_manager.get_triggered_breakers.return_value = ["daily_loss_limit"]
...
class TestEmergencyControlsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_emergency_cycle(self, mock_config, mock_risk_manager, mock_circuit_breaker_manager):
        """Test complete emergency cycle from activation to recovery."""
        with patch('src.risk_management.emergency_controls.ErrorHandler', return_value=Mock()):
            emergency_controls = EmergencyControls(mock_config, mock_risk_manager, mock_circuit_breaker_manager)
    ...
    @pytest.mark.asyncio
    async def test_emergency_with_exchanges(self, mock_config, mock_risk_manager, mock_circuit_breaker_manager):
        ...
        with patch('src.risk_management.emergency_controls.ErrorHandler', return_value=Mock()):
            ...
            exchange1.get_positions = AsyncMock(return_value=[Mock(symbol="BTCUSDT", quantity=Decimal("1.0"), side=OrderSide.BUY)])
    ...
    @pytest.mark.asyncio
    async def test_manual_override_cycle(self, mock_config, mock_risk_manager, mock_circuit_breaker_manager):
