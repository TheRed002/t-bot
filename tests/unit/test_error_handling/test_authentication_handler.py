...
class TestAuthenticationErrorHandler:
    ...
    def handler(self):
        ...
        with patch('src.error_handling.handlers.authentication.get_security_sanitizer') as mock_sanitizer, \
             patch('src.error_handling.handlers.authentication.get_security_rate_limiter') as mock_rate_limiter:
    ...
    def test_record_auth_failure(self, handler):
        ...
        handler._record_auth_failure(
            client_ip, user_id, "invalid_credentials", {}
        )
    ...
    def test_threat_analysis(self, handler):
        ...
        handler._failed_attempts[client_ip] = [
            datetime.now(timezone.utc) for _ in range(2)
        ]
        ...
        handler._failed_attempts[client_ip] = [
            datetime.now(timezone.utc) for _ in range(60)
        ]
    ...
    def test_create_secure_auth_response(self, handler):
        ...
        assert response["threat_level"] == SecurityThreat.MEDIUM.value
