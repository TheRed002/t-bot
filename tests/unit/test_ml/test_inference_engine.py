...
class TestInferenceService:
    ...
    async def test_predict_success(self, service, sample_prediction_request, mock_model_cache):
        ...
        features_df = pd.DataFrame({"price": [100.0], "volume": [1000], "rsi": [50.0]})
    ...
    async def test_predict_model_not_found(
        self, service, sample_prediction_request, mock_model_cache
    ):
        """Test prediction when model not found."""
        service.model_cache_service = mock_model_cache
        service.model_registry_service = AsyncMock()
        service.model_registry_service.load_model = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_predict_preprocessing_error(
        self, service, sample_prediction_request, mock_model_cache
    ):
    ...
    def test_preprocess_features_dataframe(self, service):
        ...
        features = pd.DataFrame(
            {"price": [100.0, 101.0], "volume": [1000, 1100], "rsi": [50.0, 55.0]}
        )
