...
class TestCoinbaseExchange:
    ...
    async def test_get_account_balance(
            self, coinbase_exchange, mock_rest_client):
    ...
    async def test_cancel_order_success(
        ...
        coinbase_exchange.client = mock_rest_client
        mock_rest_client.cancel_orders.return_value = {
            'results': [{'order_id': 'test_order_123'}]}

        result = await coinbase_exchange.cancel_order('test_order_123')

        assert result is True
        mock_rest_client.cancel_orders.assert_called_once_with(
            ['test_order_123'])
    ...
    async def test_cancel_order_failure(
        ...
        coinbase_exchange.client = mock_rest_client
        mock_rest_client.cancel_orders.side_effect = Exception(
            "Cancellation failed")
    ...
    def test_convert_timeframe_to_granularity(self, coinbase_exchange):
        ...
        assert coinbase_exchange._convert_timeframe_to_granularity(
            "1m") == "ONE_MINUTE"
        assert coinbase_exchange._convert_timeframe_to_granularity(
            "5m") == "FIVE_MINUTE"
        assert coinbase_exchange._convert_timeframe_to_granularity(
    ...
    def test_convert_coinbase_status_to_order_status(self, coinbase_exchange):
        ...
        assert coinbase_exchange._convert_coinbase_status_to_order_status(
...
class TestCoinbaseOrderManager:
    ...
    def test_convert_order_to_coinbase(self, order_manager):
        ...
        coinbase_order = order_manager._convert_order_to_coinbase(
            order_request)
    ...
    def test_convert_coinbase_order_to_response(self, order_manager):
        ...
        order_response = order_manager._convert_coinbase_order_to_response(
            coinbase_response)
    ...
    def test_convert_coinbase_status_to_order_status(self, order_manager):
        """Test Coinbase status to OrderStatus conversion."""
        assert order_manager._convert_coinbase_status_to_order_status(
            'OPEN') == OrderStatus.PENDING
        assert order_manager._convert_coinbase_status_to_order_status(
            'FILLED') == OrderStatus.FILLED
        assert order_manager._convert_coinbase_status_to_order_status(
