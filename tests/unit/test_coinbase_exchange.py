...
class TestCoinbaseExchange:
    ...
    def test_convert_coinbase_status_to_order_status(self, coinbase_exchange):
        ...
        assert coinbase_exchange._convert_coinbase_status_to_order_status('OPEN') == OrderStatus.PENDING
        assert coinbase_exchange._convert_coinbase_status_to_order_status('FILLED') == OrderStatus.FILLED
        assert coinbase_exchange._convert_coinbase_status_to_order_status('CANCELLED') == OrderStatus.CANCELLED
        assert coinbase_exchange._convert_coinbase_status_to_order_status('EXPIRED') == OrderStatus.EXPIRED
        assert coinbase_exchange._convert_coinbase_status_to_order_status('REJECTED') == OrderStatus.REJECTED
        assert coinbase_exchange._convert_coinbase_status_to_order_status('PARTIALLY_FILLED') == OrderStatus.PARTIALLY_FILLED
        assert coinbase_exchange._convert_coinbase_status_to_order_status('UNKNOWN_STATUS') == OrderStatus.UNKNOWN
...
class TestCoinbaseWebSocketHandler:
    ...
    async def test_connect_failure(self, ws_handler):
        ...
        with patch('src.exchanges.coinbase_websocket.WSClient', side_effect=Exception("Connection failed")):
            result = await ws_handler.connect()
...
class TestCoinbaseOrderManager:
    ...
    async def test_initialize_failure(self, order_manager):
        ...
        with patch('src.exchanges.coinbase_orders.RESTClient', side_effect=Exception("Init failed")):
            result = await order_manager.initialize()
    ...
    def test_get_order_statistics(self, order_manager):
        ...
        order_manager.order_history = [
            OrderResponse(id='1', symbol='BTC-USD', side=OrderSide.BUY, order_type=OrderType.MARKET,
                        quantity=Decimal('1.0'), price=None, filled_quantity=Decimal('1.0'),
                        status='FILLED', timestamp=datetime.now(timezone.utc), client_order_id='client_1'),
            OrderResponse(id='2', symbol='BTC-USD', side=OrderSide.SELL, order_type=OrderType.LIMIT,
                        quantity=Decimal('0.5'), price=Decimal('50000'), filled_quantity=Decimal('0.5'),
                        status='CANCELLED', timestamp=datetime.now(timezone.utc), client_order_id='client_2')
        ]
    ...
    def test_convert_coinbase_status_to_order_status(self, order_manager):
        ...
        assert order_manager._convert_coinbase_status_to_order_status('FILLED') == OrderStatus.FILLED
        assert order_manager._convert_coinbase_status_to_order_status('CANCELLED') == OrderStatus.CANCELLED
        assert order_manager._convert_coinbase_status_to_order_status('EXPIRED') == OrderStatus.EXPIRED
        assert order_manager._convert_coinbase_status_to_order_status('REJECTED') == OrderStatus.REJECTED
        assert order_manager._convert_coinbase_status_to_order_status('PARTIALLY_FILLED') == OrderStatus.PARTIALLY_FILLED
        assert order_manager._convert_coinbase_status_to_order_status('UNKNOWN_STATUS') == OrderStatus.UNKNOWN 