...
def mock_config():
    """Create a fresh mock configuration for each test."""
    config = MagicMock(spec=Config)
    config.error_handling = MagicMock()

    # Set up execution config that OrderIdempotencyManager expects
    config.execution = MagicMock()
    config.execution.idempotency_cleanup_interval_minutes = 60
    config.execution.get = MagicMock(return_value=24 * 3600)  # Default TTL in seconds
...
def sample_order_request():
    """Create a sample order request - bypasses ValidationFramework contamination."""
    # Create order request directly by bypassing all contaminated enum/class imports
    # This works around the comprehensive test suite contamination issue
    order_request = OrderRequest.__new__(OrderRequest)

    # Create enum values manually to avoid import contamination
    class MockOrderSide:
    ...
    order_request.__dict__.update({
        ...
        'stop_price': None,
        'quote_quantity': None,
        'time_in_force': MockTimeInForce("GTC"),
...
def idempotency_manager(mock_config):
    """Create a fresh idempotency manager instance for each test."""
    # Create a fresh manager instance for each test to ensure isolation
...
class TestOrderIdempotencyManager:
    ...
    @pytest.mark.asyncio
    async def test_get_or_create_new_key(self):
        """Test creating a new idempotency key - bypasses ValidationFramework contamination."""
        ...
        config.execution = MagicMock()
        config.execution.idempotency_cleanup_interval_minutes = 60
        config.execution.get = MagicMock(return_value=24 * 3600)

        from src.execution.idempotency_manager import OrderIdempotencyManager
        idempotency_manager = OrderIdempotencyManager(config)
        ...
        assert idempotency_manager.stats["total_keys_created"] == 0
        assert idempotency_manager.stats["cache_misses"] == 0
    ...
    async def test_invalid_order_validation(self, idempotency_manager):
        ...
        # Order without symbol - bypass model validation using construct
        invalid_order = OrderRequest.model_construct(
            symbol="", side=OrderSide.BUY, order_type=OrderType.LIMIT, quantity=Decimal("0.001")
        )
        ...
        # Order with negative quantity - bypass model validation using construct
        invalid_order2 = OrderRequest.model_construct(
            symbol="BTCUSDT",
            side=OrderSide.BUY,
    ...
    async def test_statistics(self):
        """Test statistics collection - bypasses ValidationFramework contamination."""
        from unittest.mock import MagicMock
        from decimal import Decimal
        ...
        # Verify clean initial state
        assert idempotency_manager.stats["total_keys_created"] == 0
        assert idempotency_manager.stats["duplicate_orders_prevented"] == 0

        # Create some keys and operations
        await idempotency_manager.get_or_create_idempotency_key(order_request)
        await idempotency_manager.get_or_create_idempotency_key(order_request)  # Duplicate
    ...
    async def test_error_handling_in_operations(self, idempotency_manager):
        ...
        # mark_order_failed creates a failed order entry even if original doesn't exist (for auditing)
        success = await idempotency_manager.mark_order_failed("invalid_id", "error")
        assert success is True

        # The failed order entry is created with retry_count=0, so it can be retried
        can_retry, count = await idempotency_manager.can_retry_order("invalid_id")
        assert can_retry is True
        assert count == 1  # Retry count incremented by can_retry_order call
    ...
    async def test_metadata_handling(self, idempotency_manager, sample_order_request):
        ...
        client_id, _ = await idempotency_manager.get_or_create_idempotency_key(
            sample_order_request, metadata=metadata
        )
