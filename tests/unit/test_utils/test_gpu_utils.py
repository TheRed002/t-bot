...
class TestServiceIntegration:
    ...
    def test_get_gpu_manager_service_success(self):
        ...
        # Test with injected GPU manager
        result = _get_gpu_manager_service(gpu_manager=mock_gpu_manager)

        assert result == mock_gpu_manager

    def test_get_gpu_manager_service_registration_fallback(self):
        """Test GPU manager service fallback behavior."""
        # Test that function creates a default GPUManager when none is provided
        from src.utils.gpu_utils import _get_gpu_manager_service

        # Mock GPUManager to return a known instance
        with patch("src.utils.gpu_utils.GPUManager") as mock_gpu_class:
            mock_instance = Mock()
            mock_gpu_class.return_value = mock_instance
            
            result = _get_gpu_manager_service()

            assert result == mock_instance
            mock_gpu_class.assert_called_once()

    def test_get_gpu_manager_service_failure(self):
        """Test GPU manager service fallback behavior."""
        # Mock GPUManager to raise an exception during creation
        with patch("src.utils.gpu_utils.GPUManager") as mock_gpu_class:
            mock_gpu_class.side_effect = RuntimeError("GPU initialization failed")
            
            from src.utils.gpu_utils import _get_gpu_manager_service

            with pytest.raises(ServiceError, match="Failed to create GPUManager"):
