...
class TestPingHost:
    ...
    async def test_ping_exception_handling(self):
        ...
        with patch("src.utils.network_utils.measure_latency", side_effect=Exception("Unexpected error")):
        ...
        with patch("src.utils.network_utils.check_connection") as mock_check:
            ...
            with patch("asyncio.sleep", new_callable=AsyncMock):
                with patch("asyncio.get_running_loop") as mock_loop:
                    ...
                    result = await wait_for_service("example.com", 80, max_wait=5.0, check_interval=1.0)
                    assert result is False
