...
class TestDatabaseConfig:
    ...
    def test_database_config_from_env(self):
        ...
        with patch.dict(
            os.environ,
            {"DB_HOST": "db.example.com", "DB_PORT": "5433", "REDIS_HOST": "redis.example.com"},
        ):
            db_config = DatabaseConfig()
...
class TestMainConfig:
    ...
    def test_config_from_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            config_data = {
                "database": {"postgresql_host": "file.db.com", "postgresql_port": 5433},
                "exchange": {"default_exchange": "coinbase"},
                "risk": {"risk_per_trade": 0.03},
            }
...
class TestConfigurationConsistency:
    ...
    def test_environment_specific_configs(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development", "SANDBOX_ENVIRONMENT": "sandbox"}):
            config_dev = Config()
            assert config_dev.environment == "development"

        # Test production environment
        with patch.dict(os.environ, {"ENVIRONMENT": "production", "SANDBOX_ENVIRONMENT": "production"}):
            config_prod = Config()
            assert config_prod.environment == "production"

        # Test staging environment
        with patch.dict(os.environ, {"ENVIRONMENT": "staging", "SANDBOX_ENVIRONMENT": "testnet"}):
