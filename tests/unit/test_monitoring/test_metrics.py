...
class MockValidationError(Exception):
    def __init__(self, message, error_code='VALID_000', field_name=None, field_value=None, 
                 expected_type=None, validation_rule=None, **kwargs):
...
class MockDataValidationError(Exception):
    def __init__(self, message, validation_rule=None, invalid_fields=None, sample_data=None, **kwargs):
        super().__init__(message)
        self.message = message
        self.validation_rule = validation_rule
...
class TestMetricsCollector:
    ...
    def fast_fast_collector(self):
        ...
        fast_collector.get_metric = Mock(side_effect=lambda name: fast_collector._metrics.get(f'tbot_{name}', Mock()))
    ...
    def test_register_invalid_metric_type(self, fast_fast_collector):
        ...
        fast_fast_collector.register_metric.side_effect = MonitoringError("Unknown metric type: invalid_type")
    ...
    def test_set_gauge(self, fast_fast_collector):
        ...
        assert all([
            fast_fast_collector.register_metric.called,
            fast_fast_collector.set_gauge.called
        ])
    ...
    def test_get_metrics_content_type(self, fast_fast_collector):
        ...
        # Mock the method
        fast_fast_collector.get_metrics_content_type = Mock(return_value="text/plain; version=0.0.4; charset=utf-8")
...
class TestMetricsIntegration:
    ...
    def full_collector(self):
        ...
        mock_registry = SESSION_MOCKS['registry']
        collector = Mock()
        ...
        collector.get_metric = Mock(side_effect=lambda name: collector._metrics.get(f'tbot_{name}', Mock()))
