...
spec = importlib.util.spec_from_file_location("websocket_helpers", "/mnt/e/Work/P-41 Trading/code/t-bot/src/monitoring/websocket_helpers.py")
...
class TestWebSocketManager:
    ...
    def test_websocket_manager_disconnect(self):
        ...
        manager.disconnect = lambda: setattr(manager, '_websocket', None) or setattr(manager, '_state', WebSocketState.DISCONNECTED)
        manager.disconnect()
    ...
    def test_websocket_manager_heartbeat_loop_success(self):
        ...
        assert hasattr(manager._metrics, 'last_heartbeat_time')
        assert hasattr(manager._metrics, 'last_heartbeat_latency')
    ...
    async def test_websocket_manager_heartbeat_loop_timeout(self):
        ...
        with patch.object(manager, '_send_heartbeat', side_effect=send_heartbeat_coro):
            with patch.object(manager, '_wait_heartbeat_response', side_effect=timeout_response):
                ...
                with patch.object(manager, '_handle_connection_error', side_effect=handle_error_coro) as mock_handle_error:
    ...
    async def test_websocket_manager_handle_connection_error_reconnect(self):
        ...
        with patch.object(manager, '_attempt_reconnection', side_effect=reconnect_coro) as mock_reconnect:
    ...
    async def test_websocket_manager_handle_connection_error_max_attempts(self):
        ...
        with patch.object(manager, '_set_error_state', side_effect=set_error_coro) as mock_set_error:
    ...
    async def test_websocket_manager_attempt_reconnection_failure(self):
        ...
        with patch.object(manager, '_establish_connection', side_effect=Exception("Reconnect failed")):
            async def handle_conn_err(*args, **kwargs):
                return None
            
            with patch.object(manager, '_handle_connection_error', side_effect=handle_conn_err) as mock_handle_error:
                await manager._attempt_reconnection("Connection lost")
                mock_handle_error.assert_called_once()
...
class TestUtilityFunctions:
    ...
    async def test_managed_websocket_success(self):
        ...
        with patch.object(websocket_helpers_module, 'managed_websocket', mock_managed_websocket):
            async with websocket_helpers_module.managed_websocket(config, message_handler, error_handler) as manager:
    ...
    async def test_with_websocket_timeout_success(self):
        ...
        with patch.object(websocket_helpers_module, 'with_websocket_timeout', mock_with_websocket_timeout):
    ...
    async def test_with_websocket_timeout_timeout(self):
        ...
        with patch.object(websocket_helpers_module, 'with_websocket_timeout', mock_with_websocket_timeout_timeout):
            with pytest.raises(MonitoringError):
    ...
    async def test_with_websocket_timeout_custom_error(self):
        ...
        with patch.object(websocket_helpers_module, 'with_websocket_timeout', mock_with_websocket_timeout_error):
            with pytest.raises(MonitoringError):
                await websocket_helpers_module.with_websocket_timeout(slow_operation(), 0.1, custom_message)
...
class TestEdgeCases:
    ...
    async def test_websocket_manager_concurrent_operations(self):
        ...
        assert manager.state in [WebSocketState.CONNECTED, WebSocketState.CONNECTING, WebSocketState.ERROR, WebSocketState.DISCONNECTED]
...
class TestIntegrationScenarios:
    ...
    async def test_websocket_error_recovery_flow(self):
        ...
        async def mock_set_error_state(error_msg):
            ...
            manager._metrics.connection_errors += 1
