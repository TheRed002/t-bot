...
class TestCorrelationMonitor:
    ...
    async def test_pairwise_correlation_calculation(self, correlation_monitor):
        ...
        for i, (btc_price, eth_price) in enumerate(zip(btc_prices, eth_prices, strict=False)):
    ...
    @pytest.mark.asyncio
    async def test_portfolio_correlation_single_position(
        self, correlation_monitor, sample_positions
    ):
    ...
    @pytest.mark.asyncio
    async def test_portfolio_correlation_multiple_positions(
        self, correlation_monitor, sample_positions
    ):
        ...
        for i, (btc_price, eth_price) in enumerate(zip(prices_btc, prices_eth, strict=False)):
            timestamp = base_time - timedelta(minutes=10 - i)
    ...
    async def test_correlation_caching(self, correlation_monitor):
        ...
        correlation1 = await correlation_monitor.calculate_pairwise_correlation(
            "BTC/USD", "ETH/USD"
        )

        # Second call should use cache
        correlation2 = await correlation_monitor.calculate_pairwise_correlation(
            "BTC/USD", "ETH/USD"
        )
