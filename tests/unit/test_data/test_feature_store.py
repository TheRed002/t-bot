...
class TestFeatureCalculationPipeline:
    ...
    async def test_calculate_symbol_batch_success(self, sample_market_data):
        ...
        requests = [
            FeatureRequest(symbol="BTCUSDT", feature_names=["sma_20", "rsi_14"], lookback_period=100),
        ]
    ...
    async def test_calculate_symbol_batch_feature_error(self, sample_market_data):
        ...
        mock_store.logger = Mock()
        mock_store._get_market_data = AsyncMock(return_value=sample_market_data)
        mock_store._calculate_single_feature = AsyncMock(side_effect=Exception("Feature calc failed"))
...
class TestFeatureStore:
    ...
    async def test_initialize_success(self, mock_config, mock_data_service):
        ...
        with patch.object(store, '_register_builtin_features', new_callable=AsyncMock) as mock_builtin, \
             patch.object(store, '_initialize_technical_indicators', new_callable=AsyncMock) as mock_tech, \
             patch.object(store, '_initialize_statistical_features', new_callable=AsyncMock) as mock_stat, \
             patch.object(store, '_initialize_alternative_features', new_callable=AsyncMock) as mock_alt, \
...
class TestErrorHandlingAndEdgeCases:
    ...
    async def test_pipeline_with_mixed_success_failure(self, sample_market_data):
        ...
        requests = [
            FeatureRequest(
                symbol="BTCUSDT", 
                feature_names=["working_feature", "failing_feature"]
            ),
        ]
