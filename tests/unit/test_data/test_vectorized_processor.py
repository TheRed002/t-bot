...
with patch.dict('sys.modules', {
    ...
    with patch('src.utils.technical_indicators.calculate_sma_vectorized', mock_calculate_sma_vectorized), \
         patch('src.utils.technical_indicators.calculate_ema_vectorized', mock_calculate_ema_vectorized), \
         patch('src.utils.technical_indicators.calculate_rsi_vectorized', mock_calculate_rsi_vectorized), \
         patch('src.utils.technical_indicators.calculate_bollinger_bands_vectorized', mock_calculate_bollinger_bands_vectorized), \
         patch('src.utils.technical_indicators.calculate_macd_vectorized', mock_calculate_macd_vectorized):
...
def sample_volumes():
    ...
    return np.array([1000.0, 1100.0, 1200.0, 1300.0, 1400.0, 1500.0, 1400.0, 1300.0, 1200.0, 1100.0])
...
class TestVectorizedCalculations:
    ...
    def test_calculate_volume_profile_vectorized(self, sample_prices, sample_volumes):
        ...
        try:
            ...
            assert np.sum(volume_profile) == np.sum(sample_volumes), "Total volume should be preserved"
        finally:
    ...
    def test_calculate_volume_profile_vectorized_same_prices(self, sample_volumes):
        ...
        try:
            ...
            assert volume_profile[0] == np.sum(sample_volumes), "All volume should be in the first bin"
...
class TestVectorizedProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_market_data_batch_insufficient_data(self, mock_config, sample_market_data):
        """Test processing with insufficient data for indicators."""
        processor = VectorizedProcessor(mock_config)
        
        # Mock get_recent_vectorized to return insufficient data
        processor.price_buffer.get_recent_vectorized = Mock(return_value=np.array([[1, 2, 3, 4, 5, 6]] * 10))
    ...
    def test_calculate_real_time_indicators_success(self, mock_config):
        ...
        with patch('src.data.vectorized_processor.calculate_ema_vectorized', mock_calculate_ema_vectorized), \
             patch('src.data.vectorized_processor.calculate_rsi_vectorized', mock_calculate_rsi_vectorized), \
             patch('src.data.vectorized_processor.calculate_macd_vectorized', mock_calculate_macd_vectorized):
            processor = VectorizedProcessor(mock_config)
            ...
            expected_indicators = ["ema_12", "ema_26", "rsi", "macd_line", "macd_signal", "macd_histogram"]
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_vectorized_calculations_with_empty_arrays(self):
        ...
        with patch('src.data.vectorized_processor.calculate_sma_vectorized', mock_empty_sma), \
