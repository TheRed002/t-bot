...
class TestAlternativeFeatureCalculator:
    ...
    def sample_social_sentiment(self):
        ...
        return {
            "twitter": [
                {
                    "sentiment": SocialSentiment.EXTREMELY_POSITIVE,
    ...
    async def test_calculator_initialization(self, config):
        ...
        assert calculator.sentiment_weights["bullish"] == 0.5
        assert calculator.news_source is None
        assert calculator.social_source is None
        assert calculator.alt_data_source is None

    @pytest.mark.asyncio
    async def test_set_data_sources(
        self, calculator, mock_news_source, mock_social_source, mock_alt_data_source
    ):
    ...
    @pytest.mark.asyncio
    async def test_news_sentiment_calculation(
        self, calculator, mock_news_source, sample_news_articles
    ):
    ...
    @pytest.mark.asyncio
    async def test_social_sentiment_calculation(
        self, calculator, mock_social_source, sample_social_sentiment
    ):
    ...
    @pytest.mark.asyncio
    async def test_economic_indicators_calculation(
        self, calculator, mock_alt_data_source, sample_economic_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_batch_features_calculation(
        self,
        calculator,
        mock_news_source,
        mock_social_source,
        ...
        mock_news_source.get_news_for_symbol.return_value = sample_news_articles
        mock_social_source.get_social_sentiment.return_value = sample_social_sentiment
        calculator.set_data_sources(news_source=mock_news_source, social_source=mock_social_source)
    ...
    @pytest.mark.asyncio
    async def test_calculation_summary(self, calculator, mock_news_source, sample_news_articles):
    ...
    async def test_sentiment_mapping_social_to_news(self, calculator, mock_social_source):
        ...
        social_data = {
            "twitter": [
                {"sentiment": SocialSentiment.EXTREMELY_POSITIVE, "engagement_score": 100},
                {"sentiment": SocialSentiment.POSITIVE, "engagement_score": 50},
                {"sentiment": SocialSentiment.NEUTRAL, "engagement_score": 25},
                {"sentiment": SocialSentiment.NEGATIVE, "engagement_score": 75},
                {"sentiment": SocialSentiment.EXTREMELY_NEGATIVE, "engagement_score": 100},
            ]
        }
...
class TestAlternativeFeatureIntegration:
    ...
    async def test_news_sentiment_integration(self):
        ...
        news_articles = [
            {"sentiment": NewsSentiment.BULLISH, "score": 0.6},
            {"sentiment": NewsSentiment.BULLISH, "score": 0.7},
            {"sentiment": NewsSentiment.NEUTRAL, "score": 0.0},
            {"sentiment": NewsSentiment.BEARISH, "score": -0.4},
        ]
