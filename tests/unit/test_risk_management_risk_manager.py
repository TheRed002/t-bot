...
class TestRiskManager:
    ...
    @pytest.mark.asyncio
    async def test_calculate_risk_metrics_critical_level(self, risk_manager, sample_position, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_should_exit_position_stop_loss(self, risk_manager, sample_position, sample_market_data):
        """Test position exit evaluation with stop loss."""
        # Modify market data to create a large loss
        sample_market_data.price = Decimal("40000")  # Large loss (from 50000 to 40000)
        
        result = await risk_manager.should_exit_position(sample_position, sample_market_data)
        assert result is True
    
    @pytest.mark.asyncio
    async def test_should_exit_position_drawdown_limit(self, risk_manager, sample_position, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_should_exit_position_high_risk_level(self, risk_manager, sample_position, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_should_exit_position_no_exit(self, risk_manager, sample_position, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_risk_metrics_calculation_failure(self, risk_manager, sample_position, sample_market_data):
    ...
    def test_position_limits_initialization(self, risk_manager, config):
        ...
        assert risk_manager.position_limits is not None
        assert risk_manager.position_limits.max_position_size == Decimal(str(config.risk.max_position_size_pct))
        ...
        assert risk_manager.position_limits.max_portfolio_exposure == Decimal(str(config.risk.max_portfolio_exposure))
        assert risk_manager.position_limits.max_sector_exposure == Decimal(str(config.risk.max_sector_exposure))
        assert risk_manager.position_limits.max_correlation_exposure == Decimal(str(config.risk.max_correlation_exposure))
        assert risk_manager.position_limits.max_leverage == Decimal(str(config.risk.max_leverage))
    
    @pytest.mark.asyncio
    async def test_component_integration(self, risk_manager, sample_signal, sample_position, sample_market_data):
