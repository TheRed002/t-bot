...
from src.web_interface.socketio_manager import (
    SocketIOManager,
    TradingNamespace,
    socketio_manager
)
...
class TestTradingNamespace:
    ...
    async def test_on_connect_with_invalid_token(self, trading_namespace):
        ...
        with patch.object(trading_namespace, "_validate_token", return_value=False):
            with patch.object(trading_namespace, "emit") as mock_emit:
                result = await trading_namespace.on_connect(sid, environ, auth)
...
class TestIntegration:
    ...
    async def test_full_connection_flow(self):
        ...
        # 4. Execute order
        await namespace.on_execute_order(sid, {
            "type": "limit",
            "symbol": "ETH/USDT",
            "side": "sell",
            "amount": 1.0
