...
class MockExchange(BaseExchange):
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if order_id in self.mock_data["orders"]:
            self.mock_data["orders"][order_id].status = OrderStatus.CANCELLED
            return True
        return False
    ...
    async def get_order_status(self, order_id: str) -> OrderStatus:
        ...
        if order_id in self.mock_data["orders"]:
            return self.mock_data["orders"][order_id].status
    ...
    async def get_order_book(self, symbol: str, depth: int = 10) -> OrderBook:
        ...
        return OrderBook(
            symbol=symbol,
            bids=[
                OrderBookLevel(price=Decimal("49999.0"), quantity=Decimal("1.0")),
                OrderBookLevel(price=Decimal("49998.0"), quantity=Decimal("2.0"))
            ],
            asks=[
                OrderBookLevel(price=Decimal("50001.0"), quantity=Decimal("1.0")),
                OrderBookLevel(price=Decimal("50002.0"), quantity=Decimal("2.0"))
            ],
            timestamp=datetime.now(timezone.utc),
            exchange=self.exchange_name,
        )
    ...
    async def _get_market_data_from_exchange(
        ...
        """Mock market data retrieval from exchange."""
        if not self.connected:
            raise ExchangeConnectionError("Not connected")
    ...
    async def _get_trade_history_from_exchange(self, symbol: str, limit: int = 100) -> list[Trade]:
        """Mock trade history retrieval from exchange."""
        if not self.connected:
            raise ExchangeConnectionError("Not connected")
    ...
    async def _place_order_on_exchange(self, order: OrderRequest) -> OrderResponse:
        """Mock order placement on exchange."""
        order_id = f"order_{len(self.mock_data['orders']) + 1}"
        now = datetime.now(timezone.utc)
        response = OrderResponse(
            order_id=order_id,
    ...
    async def _create_websocket_stream(self, symbol: str, stream_name: str):
        ...
        return f"mock_stream_{symbol}_{stream_name}"
...
class TestBaseExchange:
    ...
    async def test_cancel_order(self, exchange):
        ...
        # Cancel the order
        cancelled = await exchange.cancel_order(response.order_id)
        assert cancelled

        # Check order status
        status = await exchange.get_order_status(response.order_id)
    ...
    async def test_get_exchange_info(self, exchange):
        ...
        assert info.exchange == "test_exchange"
    ...
    async def test_pre_trade_validation(self, exchange):
        ...
        # Test invalid order creation (negative quantity) - caught by Pydantic validation
...
class TestBaseExchangeErrorHandling:
    ...
    async def test_validation_error_handling(self, exchange):
        ...
        # Test invalid order validation - empty symbol should fail
        invalid_order = OrderRequest(
            symbol="", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=Decimal("0.1")
        )
