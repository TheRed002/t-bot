...
class TestCapitalAllocator:
    ...
    def test_initialization(self, capital_allocator, config):
        ...
        assert capital_allocator.total_capital == Decimal(str(config.capital_management.total_capital))
        assert capital_allocator.emergency_reserve == Decimal(str(config.capital_management.total_capital * config.capital_management.emergency_reserve_pct))
        assert capital_allocator.available_capital == Decimal(str(config.capital_management.total_capital * (1 - config.capital_management.emergency_reserve_pct)))
    ...
    async def test_allocate_capital_basic(self, capital_allocator):
        ...
        assert allocation.allocation_percentage == float(allocation_amount / capital_allocator.total_capital)
    ...
    async def test_rebalance_allocations_no_rebalance_needed(self, capital_allocator):
        ...
        assert isinstance(result, dict)  # Should return allocations even if no rebalance needed
    ...
    async def test_get_capital_metrics(self, capital_allocator):
        ...
        # Available capital should be the available_capital attribute (which is total - emergency_reserve)
    ...
    async def test_equal_weight_allocation(self, capital_allocator):
        ...
        capital_allocator.strategy_allocations = {
            "strategy1_binance": CapitalAllocation(
                strategy_id="strategy1", exchange="binance",
                allocated_amount=Decimal("10000"), utilized_amount=Decimal("8000"),
                available_amount=Decimal("2000"), allocation_percentage=0.1,
                last_rebalance=datetime.now()
            ),
            "strategy1_okx": CapitalAllocation(
                strategy_id="strategy1", exchange="okx",
                allocated_amount=Decimal("5000"), utilized_amount=Decimal("4000"),
                available_amount=Decimal("1000"), allocation_percentage=0.05,
                last_rebalance=datetime.now()
            ),
            "strategy2_binance": CapitalAllocation(
                strategy_id="strategy2", exchange="binance",
                allocated_amount=Decimal("15000"), utilized_amount=Decimal("12000"),
                available_amount=Decimal("3000"), allocation_percentage=0.15,
                last_rebalance=datetime.now()
            )
        }
    ...
    async def test_calculate_performance_metrics(self, capital_allocator):
        ...
        capital_allocator.strategy_allocations = {
            f"{strategy_name}_binance": CapitalAllocation(
                strategy_id=strategy_name, exchange="binance",
                allocated_amount=Decimal("10000"), utilized_amount=Decimal("8000"),
                available_amount=Decimal("2000"), allocation_percentage=0.1,
                last_rebalance=datetime.now()
            )
    ...
    async def test_update_total_capital(self, capital_allocator):
        ...
        assert capital_allocator.emergency_reserve == new_total * Decimal(str(capital_allocator.capital_config.emergency_reserve_pct))
        assert capital_allocator.available_capital == new_total * (1 - Decimal(str(capital_allocator.capital_config.emergency_reserve_pct)))
    ...
    async def test_get_emergency_reserve(self, capital_allocator):
        ...
        assert reserve == capital_allocator.total_capital * Decimal(str(capital_allocator.capital_config.emergency_reserve_pct))
    ...
    async def test_get_available_capital(self, capital_allocator):
        ...
        assert available == capital_allocator.total_capital * (1 - Decimal(str(capital_allocator.capital_config.emergency_reserve_pct)))
