...
class TestVolatilityBreakoutStrategy:
    ...
    async def test_calculate_atr(self, strategy):
        ...
        strategy.price_history["BTC/USD"] = [
            100,
            101,
            102,
            103,
        ...
        strategy.high_low_history["BTC/USD"] = [
            (101, 99),
            (102, 100),
            (103, 101),
            (104, 102),
    ...
    def test_is_signal_valid_for_regime(self, strategy):
        ...
        # Test different regimes
        assert (
            strategy._is_signal_valid_for_regime(signal, MarketRegime.LOW_VOLATILITY) is False
        )  # Requires 0.7
        assert (
            strategy._is_signal_valid_for_regime(signal, MarketRegime.MEDIUM_VOLATILITY) is True
        )  # Requires 0.5
        assert (
            strategy._is_signal_valid_for_regime(signal, MarketRegime.HIGH_VOLATILITY) is True
        )  # Requires 0.3
    ...
    def test_strategy_configuration_validation(self):
        ...
        valid_config = {
            ...
            "position_size_pct": 0.02,
            "min_confidence": 0.6,
            "parameters": {"atr_period": 14, "breakout_multiplier": 2.0},
        }
