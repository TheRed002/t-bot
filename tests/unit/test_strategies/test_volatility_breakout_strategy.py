...
class TestVolatilityBreakoutStrategy:
    ...
    def mock_regime_detector(self):
        ...
        detector.detect_comprehensive_regime = AsyncMock(return_value=MarketRegime.MEDIUM_VOLATILITY)
        return detector
    ...
    @pytest.mark.asyncio
    async def test_get_current_regime_with_detector(self, strategy, mock_regime_detector, sample_market_data):
    ...
    async def test_calculate_atr(self, strategy):
        ...
        strategy.price_history["BTC/USD"] = [100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114]
    ...
    @pytest.mark.asyncio
    async def test_calculate_consolidation_score_insufficient_data(self, strategy):
    ...
    @pytest.mark.asyncio
    async def test_generate_breakout_signals_upper_breakout(self, strategy, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_generate_breakout_signals_lower_breakout(self, strategy, sample_market_data):
    ...
    @pytest.mark.asyncio
    async def test_generate_breakout_signals_no_breakout(self, strategy, sample_market_data):
    ...
    async def test_apply_regime_filtering_with_manager(self, strategy, mock_adaptive_risk_manager):
        ...
        signals = [
            Signal(
                ...
                symbol="BTC/USD",
                strategy_name="volatility_breakout",
                metadata={'atr_value': 0.02, 'consolidation_score': 0.7, 'regime': 'medium_volatility'}
            )
        ]
    ...
    def test_is_signal_valid_for_regime(self, strategy):
        ...
        # Test different regimes
        assert strategy._is_signal_valid_for_regime(signal, MarketRegime.LOW_VOLATILITY) is False  # Requires 0.7
        assert strategy._is_signal_valid_for_regime(signal, MarketRegime.MEDIUM_VOLATILITY) is True   # Requires 0.5
        assert strategy._is_signal_valid_for_regime(signal, MarketRegime.HIGH_VOLATILITY) is True   # Requires 0.3
    ...
    async def test_apply_time_decay(self, strategy):
        ...
        strategy.last_breakout_time["BTC/USD"] = datetime.now() - timedelta(hours=1)
        
        decayed_signals = await strategy._apply_time_decay(signals, "BTC/USD")
        
        assert len(decayed_signals) == 1
        assert decayed_signals[0].confidence < signals[0].confidence  # Should be reduced
    ...
    @pytest.mark.asyncio
    async def test_validate_signal_atr_inconsistency(self, strategy, sample_signal):
    ...
    @pytest.mark.asyncio
    async def test_validate_signal_consolidation_inconsistency(self, strategy, sample_signal):
        """Test signal validation with consolidation inconsistency."""
        # Add different consolidation data
        strategy.consolidation_scores["BTC/USD"] = 0.3  # Different from signal's 0.7
        
        is_valid = await strategy.validate_signal(sample_signal)
        assert is_valid is False
    
    @pytest.mark.asyncio
    async def test_get_position_size_with_adaptive_manager(self, strategy, mock_adaptive_risk_manager, sample_signal):
    ...
    def test_strategy_integration_with_existing_components(self, strategy, mock_regime_detector, mock_adaptive_risk_manager):
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_signal_generation_workflow(self, strategy, mock_regime_detector, mock_adaptive_risk_manager, sample_market_data):
