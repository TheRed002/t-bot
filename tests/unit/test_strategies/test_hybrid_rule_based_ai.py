...
from src.core.types import MarketData, Position, Signal, SignalDirection, StrategyType, OrderSide
...
class TestTechnicalRuleEngine:
    ...
    def test_moving_averages_calculation(self, rule_engine):
        ...
        # Test with insufficient data
        short_prices = [100, 101, 102]
        ma_short_insufficient, ma_long_insufficient = rule_engine.calculate_moving_averages(short_prices)
    ...
    def test_rule_evaluation_rsi_signals(self, rule_engine):
        ...
        result_overbought = rule_engine.evaluate_rules(market_data, overbought_prices, volume_history)
        
        assert result_overbought['rsi'] > 70
        
        # Should generate sell signal
        sell_signals = [s for s in result_overbought['signals'] if s['direction'] == SignalDirection.SELL]
    ...
    def test_rule_evaluation_ma_crossover(self, rule_engine):
        ...
        bullish_prices = [100, 101, 102, 103, 104, 105, 106, 107, 108, 109,
                         110, 111, 112, 113, 114, 115, 116, 117, 118, 119]
        volume_history = [1000] * 20
    ...
    def test_rule_performance_tracking(self, rule_engine):
        ...
        assert rule_engine.rule_performance['rsi_oversold']['total'] == initial_performance['total'] + 1
        assert rule_engine.rule_performance['rsi_oversold']['wins'] == initial_performance['wins'] + 1
        assert rule_engine.rule_performance['rsi_oversold']['losses'] == initial_performance['losses']
        
        # Update performance with loss
        rule_engine.update_rule_performance('rsi_oversold', -75.0)  # Negative performance
        
        assert rule_engine.rule_performance['rsi_oversold']['total'] == initial_performance['total'] + 2
        assert rule_engine.rule_performance['rsi_oversold']['losses'] == initial_performance['losses'] + 1
...
class TestAIPredictor:
    ...
    async def test_model_training(self, ai_predictor):
        ...
        prediction = await ai_predictor.predict(training_data[0]['price_history'], 
                                               training_data[0]['volume_history'])
        
        assert 'direction' in prediction
        assert 'confidence' in prediction
        assert 'probabilities' in prediction
        assert prediction['direction'] in [SignalDirection.BUY, SignalDirection.SELL, SignalDirection.HOLD]
...
class TestRuleBasedAIStrategy:
    ...
    def test_weighted_average_conflict_resolution(self, strategy):
        ...
        result_conflict = strategy._weighted_average_resolution(
            SignalDirection.BUY, 0.7,
            SignalDirection.SELL, 0.8
        )
        
        # Should have some direction based on weights
        assert result_conflict['direction'] in [SignalDirection.BUY, SignalDirection.SELL, SignalDirection.HOLD]
...
async def test_hybrid_strategy_integration():
    ...
    market_data = MarketData(
        ...
        volume=Decimal("2000"),
        bid=Decimal(str(prices[-1] * 0.999)),
        ask=Decimal(str(prices[-1] * 1.001))
    )
...
async def test_hybrid_strategy_performance():
    ...
    market_data = MarketData(
        symbol=symbol,
        timestamp=datetime.now(),
        price=Decimal(str(large_prices[-1])),
        volume=Decimal("2000"),
        bid=Decimal(str(large_prices[-1] * 0.999)),
        ask=Decimal(str(large_prices[-1] * 1.001))
    )
