...
class TestAnalyticsServiceFactory:
    ...
    def mock_injector(self):
        ...
        injector = Mock()
        # Configure resolve to raise an exception (simulating service not found)
        # This matches the factory behavior which catches exceptions and sets to None
    ...
    def test_factory_init_without_injector(self):
        """Test factory initialization without injector works with None."""
        # Factory allows None injector
    ...
    def test_create_analytics_service(self, factory_with_mock_injector, mock_injector):
        ...
        # Mock the AnalyticsService class directly since factory creates it
        from unittest.mock import patch
        with patch('src.analytics.service.AnalyticsService') as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service

            result = factory_with_mock_injector.create_analytics_service()

            # Factory creates AnalyticsService directly, not through injector resolve
    ...
    def test_create_portfolio_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.portfolio_analytics_service.PortfolioAnalyticsService') as mock_service_class:
    ...
    def test_create_risk_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.risk_service.RiskService') as mock_service_class:
    ...
    def test_create_reporting_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.reporting_service.ReportingService') as mock_service_class:
    ...
    def test_create_operational_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.operational_service.OperationalService') as mock_service_class:
    ...
    def test_create_alert_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.alert_service.AlertService') as mock_service_class:
    ...
    def test_create_export_service(self, factory_with_mock_injector, mock_injector):
        ...
        from unittest.mock import patch
        with patch('src.analytics.services.export_service.ExportService') as mock_service_class:
    ...
    def test_factory_reuses_injector_across_calls(self, factory_with_mock_injector, mock_injector):
        """Test factory keeps reference to injector for multiple service creations."""
        # These services are created directly by the factory, not through injector
        from unittest.mock import patch
        
        with patch('src.analytics.services.portfolio_analytics_service.PortfolioAnalyticsService') as mock_portfolio_class, \
             patch('src.analytics.services.risk_service.RiskService') as mock_risk_class:
            
            mock_portfolio_service = Mock(spec=PortfolioServiceProtocol)
            mock_risk_service = Mock(spec=RiskServiceProtocol)
            mock_portfolio_class.return_value = mock_portfolio_service
            mock_risk_class.return_value = mock_risk_service
            ...
            # Verify the factory still has the same injector
            assert factory_with_mock_injector._injector == mock_injector
...
class TestCreateDefaultAnalyticsService:
    ...
    def test_create_default_service_without_params(self, mock_factory_class):
    ...
    @patch("src.analytics.factory.AnalyticsServiceFactory")
    def test_create_default_service_with_config(self, mock_factory_class):
    ...
    def test_create_default_service_with_injector(self, mock_factory_class):
        ...
        mock_factory_class.assert_called_once_with(custom_injector)
        mock_factory_instance.create_analytics_service.assert_called_once_with(None)
        assert result == mock_service
...
class TestFactoryIntegration:
    ...
    def test_factory_method_return_types(self):
        """Test that factory methods are callable."""
        # This test ensures factory methods exist and are callable
        factory = AnalyticsServiceFactory(injector=Mock())

        # Check that methods exist and are callable
        assert callable(factory.create_portfolio_service)
        assert callable(factory.create_risk_service) 
        assert callable(factory.create_reporting_service)
        assert callable(factory.create_operational_service)
        assert callable(factory.create_alert_service)
        assert callable(factory.create_export_service)

    def test_default_injector_configuration(self):
        """Test that factory handles injector configuration."""
        # Factory allows None injector
        factory = AnalyticsServiceFactory()
        assert factory._injector is None
...
class TestFactoryErrorHandling:
    ...
    def test_service_resolution_failure_handling(self):
        ...
        # Test that service creation errors are propagated
        with patch('src.analytics.services.portfolio_analytics_service.PortfolioAnalyticsService') as mock_service_class:
    ...
    def test_injector_method_missing(self):
        ...
        # Factory catches all exceptions during dependency resolution
        # and sets dependencies to None, so this should succeed
        from unittest.mock import patch
        with patch('src.analytics.service.AnalyticsService') as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            
            result = factory.create_analytics_service()
    ...
    def test_configuration_validation_error_propagation(self):
        ...
        # Should raise an AttributeError when trying to call model_dump on invalid config
        with pytest.raises(AttributeError, match="'object' object has no attribute 'model_dump'"):
            factory.create_analytics_service(invalid_config)
