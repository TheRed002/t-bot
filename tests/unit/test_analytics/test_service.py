...
class TestAnalyticsService:
    ...
    def test_analytics_service_init_with_default_metrics(self, mock_services):
        """Test initialization with default metrics collector."""
        # Don't pass metrics_collector so it uses the default from get_metrics_collector()
        ...
        # Verify it has a metrics collector (either from DI or get_metrics_collector)
        assert service.metrics_collector is not None
        # Verify the metrics collector has the expected methods
        assert hasattr(service.metrics_collector, "increment_counter")
    ...
    def test_analytics_service_docstring_exists(self):
        ...
        assert len(AnalyticsService.__doc__.strip()) > 100
        assert "institutional" in AnalyticsService.__doc__.lower()
    ...
    def test_service_accepts_none_dependencies(self):
        ...
        # Service now validates that all dependencies are provided
    ...
    def test_service_dependencies_required(self):
        """Test that all service dependencies are required."""
        # This should raise ComponentError due to missing dependencies
        with pytest.raises(Exception):  # Should raise ComponentError
            service = AnalyticsService(
                config=None,
...
class TestAnalyticsServiceIntegration:
    ...
    def test_service_event_integration(self, mock_services):
        ...
        # Service should have event bus integration
        assert hasattr(service, "event_bus")
        assert service.event_bus is not None
        # Event bus should have basic methods for event handling
        assert hasattr(service.event_bus, "register_handler") or hasattr(
    ...
    def test_service_metrics_integration(self, mock_services):
        ...
        assert service.metrics_collector is not None
        # Metrics helper should be set up with the collector
        assert hasattr(service, "metrics_helper")
        assert service.metrics_helper is not None
