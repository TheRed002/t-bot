...
class TestConnectionManager:
    ...
    async def test_health_check_all(self, connection_manager):
        ...
        await connection_manager.get_rest_connection("default")  # Create REST connection
        ...
        assert health_status["rest_default"]  # REST connections are marked as healthy
        assert health_status["ws_conn1"]  # WebSocket connections should be healthy
    ...
    async def test_reconnect_all(self, connection_manager):
        ...
        assert results["rest_default"]  # REST connections are marked as healthy
...
class TestConnectionManagerErrorHandling:
    ...
    async def test_health_check_exception_handling(self, connection_manager):
        ...
        assert not health_status["ws_test_conn"]  # Should be marked as unhealthy
...
class TestWebSocketConnectionErrorHandling:
    ...
    async def test_disconnect_exception_handling(self, ws_connection):
        ...
        # The test is checking that the exception handling works, not the actual disconnection
        assert ws_connection.connected
