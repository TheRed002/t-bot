...
class TestExchangeAllocationRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_by_exchange_found(self, exchange_allocation_repository, sample_exchange_allocation):
        """Test get allocation by exchange when found."""
        with patch.object(exchange_allocation_repository, 'get_by', return_value=sample_exchange_allocation):
            result = await exchange_allocation_repository.get_by_exchange(sample_exchange_allocation.exchange)
    ...
    async def test_get_underutilized_exchanges_default_threshold(self, exchange_allocation_repository):
        ...
        allocations = [
            Mock(allocated_amount=100000.0, utilized_amount=30000.0),  # 30% utilization - underutilized
            Mock(allocated_amount=50000.0, utilized_amount=40000.0),   # 80% utilization - well utilized
            Mock(allocated_amount=75000.0, utilized_amount=25000.0),   # 33% utilization - underutilized
    ...
    @pytest.mark.asyncio
    async def test_get_underutilized_exchanges_custom_threshold(self, exchange_allocation_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_underutilized_exchanges_no_underutilized(self, exchange_allocation_repository):
...
class TestCapitalAuditLogRepository:
    ...
    async def test_get_by_strategy(self, audit_repository, mock_session):
        ...
        mock_result = Mock()
        mock_scalars = Mock()
        mock_scalars.all.return_value = [Mock(strategy_id=strategy_id), Mock(strategy_id=strategy_id)]
        mock_result.scalars.return_value = mock_scalars
        mock_session.execute.return_value = mock_result
...
class TestCapitalRepositoryConcurrency:
    ...
    async def test_concurrent_allocation_updates(self, allocation_repository):
        ...
        with patch.object(allocation_repository, 'get', return_value=allocation), \
...
class TestCapitalRepositoryPerformance:
    ...
    async def test_batch_allocation_queries(self, allocation_repository):
        ...
        with patch.object(allocation_repository, 'get_by_strategy', side_effect=mock_get_by_strategy):
    ...
    async def test_large_dataset_aggregation(self, allocation_repository):
        ...
        large_allocation_set = [
            Mock(allocated_amount=Decimal(str(i * 1000)))
            for i in range(1000)
        ]
        
        with patch.object(allocation_repository, 'get_by_strategy', return_value=large_allocation_set):
            result = await allocation_repository.get_total_allocated_by_strategy("strategy_id")
