...
class TestOrderRepository:
    ...
    async def test_cancel_order_success(self, order_repository, sample_order):
        ...
        with patch(
            "src.database.repository.utils.RepositoryUtils.update_entity_status", return_value=True
        ):
...
class TestPositionRepository:
    ...
    async def test_get_position_by_symbol_not_found(self, position_repository):
        ...
        with patch.object(position_repository, "get_by", return_value=None):
            result = await position_repository.get_position_by_symbol("bot_id", "BTCUSD", "LONG")
    ...
    async def test_close_position_success(self, position_repository, sample_position):
        ...
        with patch(
            ...
            result = await position_repository.update_position_status(
                str(sample_position.id), "CLOSED", exit_price=exit_price, realized_pnl=pnl
            )
    ...
    async def test_close_position_not_open(self, position_repository, sample_position):
        ...
        with patch(
            ...
            result = await position_repository.update_position_status(
                str(sample_position.id), "CLOSED", exit_price=Decimal("46000.00")
            )
    ...
    async def test_close_position_not_found(self, position_repository):
        ...
        with patch(
            ...
            result = await position_repository.update_position_status(
                "nonexistent_id", "CLOSED", exit_price=Decimal("46000.00")
            )
    ...
    async def test_update_position_price_success(self, position_repository, sample_position):
        ...
        with patch(
            "src.database.repository.utils.RepositoryUtils.update_entity_fields", return_value=True
        ):
    ...
    async def test_update_position_price_not_found(self, position_repository):
        ...
        with patch(
            "src.database.repository.utils.RepositoryUtils.update_entity_fields", return_value=False
        ):
            result = await position_repository.update_position_fields(
                "nonexistent_id", current_price=Decimal("47000.00")
            )

            assert result is False

    # Note: get_total_exposure method has been moved to TradingService
...
class TestTradeRepository:
    ...
    # Note: get_trade_statistics method has been moved to TradingService
    # Repository should only handle data access operations
    ...
    async def test_create_from_position_success(self, trade_repository):
        ...
        exit_order = Mock(id=str(uuid.uuid4()), average_fill_price=Decimal("46000.00"))
    ...
    async def test_create_from_position_with_percentage_calculation(self, trade_repository):
        ...
        exit_order = Mock(id=str(uuid.uuid4()), average_fill_price=Decimal("52000.00"))
...
class TestOrderFillRepository:
    ...
    async def test_get_total_filled_with_fills(self, fill_repository):
        ...
        with patch.object(fill_repository, "get_fills_by_order", return_value=fills):
            ...
            expected = {"quantity": Decimal("1.0"), "average_price": Decimal("45020.0"), "total_fees": Decimal("45.02")}
    ...
    async def test_get_total_filled_no_fills(self, fill_repository):
        ...
        with patch.object(fill_repository, "get_fills_by_order", return_value=[]):
            ...
            expected = {"quantity": 0, "average_price": 0, "total_fees": 0}
