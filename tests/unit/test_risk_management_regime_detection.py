...
class TestMarketRegimeDetector:
    ...
    @pytest.mark.asyncio
    async def test_detect_volatility_regime_low_volatility(self, regime_detector, low_volatility_data):
    ...
    @pytest.mark.asyncio
    async def test_detect_volatility_regime_medium_volatility(self, regime_detector, sample_price_data):
    ...
    @pytest.mark.asyncio
    async def test_detect_volatility_regime_high_volatility(self, regime_detector, high_volatility_data):
    ...
    async def test_detect_correlation_regime_high_correlation(self, regime_detector):
        ...
        price_data = {
            "BTCUSDT": [100 + 0.8 * common_factor[i] + 0.2 * np.random.normal(0, 1) for i in range(n_points)],
            "ETHUSDT": [100 + 0.8 * common_factor[i] + 0.2 * np.random.normal(0, 1) for i in range(n_points)],
            "ADAUSDT": [100 + 0.8 * common_factor[i] + 0.2 * np.random.normal(0, 1) for i in range(n_points)]
        }
    ...
    async def test_detect_comprehensive_regime(self, regime_detector, sample_market_data):
        ...
        assert regime in [MarketRegime.MEDIUM_VOLATILITY, MarketRegime.LOW_VOLATILITY, 
                         MarketRegime.TRENDING_UP, MarketRegime.TRENDING_DOWN, MarketRegime.RANGING]
    ...
    def test_combine_regimes(self, regime_detector):
        ...
        combined = regime_detector._combine_regimes(volatility_regimes, trend_regimes, correlation_regime)
        assert combined == MarketRegime.CRISIS  # High volatility + high correlation = crisis
    ...
    def test_combine_regimes_medium_volatility(self, regime_detector):
        ...
        combined = regime_detector._combine_regimes(volatility_regimes, trend_regimes, correlation_regime)
        assert combined == MarketRegime.TRENDING_UP  # Use trend regime for medium volatility
