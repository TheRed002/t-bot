...
from src.error_handling.error_handler import (
    ErrorHandler, ErrorSeverity, ErrorContext, CircuitBreaker, error_handler_decorator
)
...
class TestErrorHandler:
    ...
    def test_error_classification(self, error_handler):
        ...
        assert error_handler.classify_error(StateConsistencyError("State corruption")) == ErrorSeverity.CRITICAL
        assert error_handler.classify_error(SecurityError("Security breach")) == ErrorSeverity.CRITICAL
        
        # Test high severity errors
        assert error_handler.classify_error(RiskManagementError("Risk limit exceeded")) == ErrorSeverity.HIGH
        assert error_handler.classify_error(ExchangeError("API timeout")) == ErrorSeverity.HIGH
        assert error_handler.classify_error(ExecutionError("Order execution failed")) == ErrorSeverity.HIGH
        
        # Test medium severity errors
        assert error_handler.classify_error(DataError("Data validation failed")) == ErrorSeverity.MEDIUM
        assert error_handler.classify_error(ModelError("Model inference failed")) == ErrorSeverity.MEDIUM
