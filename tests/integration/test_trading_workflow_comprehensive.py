...
from src.core.exceptions import ExecutionError, RiskManagementError
from src.core.types import (
    MarketData, Ticker, Signal, SignalDirection, Order, OrderSide, OrderType, OrderStatus,
    Position, ExecutionInstruction, ExecutionResult, ExecutionStatus
)
...
from src.state import StateService as StateManager
...
class TestCompleteSignalToExecutionWorkflow:
    ...
    async def test_sell_signal_execution_workflow(self, mock_config, mock_exchange, mock_strategy):
        ...
        mock_strategy.generate_signal = AsyncMock(return_value=Signal(
            symbol="BTC/USDT",
...
class TestPositionManagementWorkflow:
    ...
    async def test_position_size_increase(self, mock_config):
        ...
        new_average_price = (
            (existing_position.quantity * existing_position.average_price + 
             new_fill.filled_quantity * new_fill.average_price) / total_quantity
        )
        ...
        assert position_manager.positions["BTC/USDT"].average_price == Decimal("49666.666666666666666666666667")
    ...
    async def test_position_partial_close(self, mock_config):
        ...
        realized_pnl = (sell_order.average_price - existing_position.average_price) * sell_order.filled_quantity
    ...
    async def test_position_complete_close(self, mock_config):
        ...
        realized_pnl = (close_order.average_price - existing_position.average_price) * close_order.filled_quantity
...
class TestMultiExchangeWorkflow:
    ...
    async def test_best_price_execution_across_exchanges(self, mock_config):
        ...
        binance_exchange.get_ticker = AsyncMock(return_value=Ticker(
            symbol="BTC/USDT",
        ...
        coinbase_exchange.get_ticker = AsyncMock(return_value=Ticker(
            symbol="BTC/USDT",
    ...
    async def test_cross_exchange_position_management(self, mock_config):
        ...
        position_manager.get_combined_position = Mock(return_value=Position(
            symbol="BTC/USDT",
            side=OrderSide.LONG,
...
class TestBotInstanceIntegration:
    ...
    async def test_multi_strategy_coordination(self, mock_config):
        ...
        momentum_strategy.generate_signal = AsyncMock(return_value=Signal(
            symbol="BTC/USDT",
            direction=SignalDirection.BUY,
            confidence=0.8,
            price=Decimal("50000.0")
        ...
        mean_reversion_strategy.generate_signal = AsyncMock(return_value=Signal(
            symbol="BTC/USDT", 
            direction=SignalDirection.SELL,
            confidence=0.6,
            price=Decimal("50000.0")
    ...
    async def test_performance_monitoring_integration(self, mock_config):
        ...
        # Calculate win rate
        win_rate = (performance_monitor.metrics["winning_trades"] / 
                   performance_monitor.metrics["total_trades"]) * 100
