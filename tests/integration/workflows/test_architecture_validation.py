...
class TestTestableArchitecture:
    ...
    async def test_testable_exchange_without_adapter(self):
        ...
        class DirectTestExchange(TestableBaseExchange):
            ...
            async def _place_order_impl(self, symbol, side, order_type, quantity, price=None, **kwargs):
                return {
                    "id": f"direct_order_123",
                    "symbol": symbol,
                    "side": side,
                    "type": order_type,
        ...
        order = await exchange.place_order("ETHUSDT", "SELL", "LIMIT", Decimal("1.0"), Decimal("3500"))
