...
from src.core.dependency_injection import DependencyInjector, injector
...
from src.core.types import (
    ...
    StrategyConfig, StrategyType, ExchangeInfo
)
...
class CrossModuleIntegrationTest(BaseIntegrationTest):
    ...
    async def test_multi_exchange_trading_workflow(self, performance_monitor):
        ...
        with patch.object(self.exchange_factory, 'create_exchange') as mock_create:
            ...
            if price_diff > min_profit_threshold:
                ...
                buy_exchange = binance if btc_data_binance.price < btc_data_coinbase.price else coinbase
                ...
                performance_monitor.record_event("arbitrage_completed", {
    ...
    async def test_realtime_data_strategy_execution_pipeline(self, performance_monitor):
        ...
        with patch.object(self.ml_service, 'process_pipeline') as mock_pipeline:
            ...
            performance_monitor.record_event("ml_processing", {"predictions": len(ml_result.predictions)})
        ...
        for _ in range(5):
            ...
            performance_monitor.record_event("signal_generated", {"direction": signal.direction.value})
        ...
        with patch.object(self.exchange_factory, 'get_exchange') as mock_get_exchange:
            ...
            for signal in signals:
                if signal.direction != SignalDirection.HOLD:
                    ...
                    performance_monitor.record_api_call()
                    performance_monitor.record_event("order_executed", {
    ...
    async def test_authentication_service_integration(self, performance_monitor):
        ...
        from src.web_interface.security.auth import init_auth, create_access_token, verify_token
    ...
    async def test_state_management_cross_service_integration(self, performance_monitor):
        ...
        with patch.object(self.exchange_factory, 'get_exchange') as mock_get_exchange:
            ...
            if order_status == OrderStatus.FILLED:
                ...
                performance_monitor.record_event("state_updated", {
                    "bot_id": bot_config["bot_id"],
        ...
        performance_monitor.record_event("concurrent_updates", {
            "attempted": len(tasks),
        ...
        assert final_concurrent_state["performance"]["total_trades"] >= 1  # At least the initial trade
    ...
    async def test_error_recovery_across_module_boundaries(self, performance_monitor):
        ...
        with patch.object(self.execution_engine, 'execute_instruction') as mock_execute:
            ...
            while execution_retry_count < 2 and result is None:
                try:
                    ...
                    performance_monitor.record_event("execution_completed", {
                        "retry": execution_retry_count,
        ...
        for i in range(5):
            try:
                ...
                performance_monitor.record_event("service_call", {"attempt": i, "result": "success"})
        ...
        performance_monitor.record_event("circuit_breaker_test", {
