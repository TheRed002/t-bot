...
class DependencyInjectionIntegrationTest(BaseIntegrationTest):
    ...
    async def test_circular_dependency_prevention(self):
        ...
        # This should cause DependencyError (wrapping RecursionError) since DI container
        # doesn't have circular dependency prevention (which is acceptable - detect at runtime)
        # Use a smaller recursion limit to fail faster
        import sys
        old_limit = sys.getrecursionlimit()
        try:
            sys.setrecursionlimit(100)  # Small limit to fail fast
            with pytest.raises((RecursionError, DependencyError)):
