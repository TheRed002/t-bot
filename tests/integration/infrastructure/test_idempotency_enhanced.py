...
class EnhancedIdempotencyTest:
    ...
    async def test_financial_precision_idempotency(self, clean_database):
        ...
        try:
            ...
            test_cases = [
                ...
                (Decimal("0.12345678"), Decimal("45678.12345678"), Decimal("5639.27403841652797")),
                (Decimal("1.00000001"), Decimal("50000.00000001"), Decimal("50000.00050001")),
                (Decimal("0.000000000000000001"), Decimal("1000000.00"), Decimal("0.01000000")),  # Satoshi level
            ]
            ...
            for i, (quantity, price, expected_total) in enumerate(test_cases):
                ...
                logger.info(f"✅ Financial precision test passed: {quantity} * {price} = {actual_total}")
    ...
    async def test_database_persistence_verification(self, clean_database):
        ...
        try:
            ...
            # Check if key exists in Redis (keys have format: idempotency:order:{order_hash})
            redis_keys = []
            cursor = 0
            while True:
                cursor, keys = await redis_client.scan(cursor, match="idempotency:order:*", count=100)
                redis_keys.extend(keys)
                if cursor == 0:
                    break

            assert len(redis_keys) > 0, f"Idempotency key {idempotency_key} not found in Redis"
    ...
    async def test_high_frequency_trading_stress(self, clean_database):
        ...
        try:
            ...
            for i in range(num_unique_orders):
                unique_orders.append({
                    ...
                    "data": {
                        ...
                        "type": "limit",
                        "quantity": str(Decimal(f"1.{i:08d}").quantize(Decimal("0.000000000000000001"))),  # Unique quantities
                        "price": str(Decimal(f"45000.{i:02d}").quantize(Decimal("0.01")))  # Unique prices
                    }
                })
            ...
            for i in range(10):
                base_orders_for_duplicates.append({
                    ...
                    "data": {
                        ...
                        "type": "limit",
                        "quantity": str(Decimal(f"5.{i:08d}").quantize(Decimal("0.000000000000000001"))),  # UNIQUE quantity per base order
                        "price": str(Decimal(f"2500.{i:02d}").quantize(Decimal("0.01")))  # UNIQUE price per base order
                    }
                })
            ...
            logger.info(f"Testing with {len(unique_orders)} unique orders, {len(base_orders_for_duplicates)} base orders with duplicates, {len(duplicate_orders)} duplicates")
            ...
            logger.info(f"Results: {new_orders} new orders, {duplicate_detections} duplicate detections")
            logger.info(f"Expected: {total_expected_new} new orders, {num_duplicates} duplicate detections")

            assert new_orders == total_expected_new, f"Expected {total_expected_new} new orders, got {new_orders}"
            assert duplicate_detections == num_duplicates, f"Expected {num_duplicates} duplicates, got {duplicate_detections}"

            logger.info(f"✅ HFT stress test passed: {new_orders} new, {duplicate_detections} duplicates detected")
