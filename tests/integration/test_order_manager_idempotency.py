...
class TestOrderManagerIdempotency:
    ...
    async def test_order_submission_with_idempotency(
        self, order_manager, mock_exchange, sample_order_request
    ):
        ...
        with patch.object(order_manager.idempotency_manager, "max_retries", 0):
            with pytest.raises(ExecutionError, match="Duplicate order detected"):
                await order_manager.submit_order(
                    sample_order_request, mock_exchange, "test_execution_456"
                )
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_retry_with_same_client_order_id(
        self, order_manager, mock_exchange, sample_order_request
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_order_completion_tracking(
        self, order_manager, mock_exchange, sample_order_request
    ):
        ...
        await order_manager._handle_status_change(
            managed_order, OrderStatus.PENDING, OrderStatus.FILLED
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_concurrent_order_submissions(
        self, order_manager, mock_exchange, sample_order_request
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_order_manager_with_redis(
        self, order_manager_with_redis, mock_exchange, sample_order_request
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_order_manager_summary_includes_idempotency(
        self, order_manager, mock_exchange, sample_order_request
    ):
        ...
        await order_manager.submit_order(sample_order_request, mock_exchange, "test_execution_123")
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_order_cancellation_marks_idempotency_failed(
        self, order_manager, mock_exchange, sample_order_request
    ):
        ...
        await order_manager._handle_status_change(
            managed_order, OrderStatus.PENDING, OrderStatus.CANCELLED
        )
    ...
    async def test_different_orders_get_different_keys(self, order_manager, mock_exchange):
        ...
        assert (
            managed_order1.order_request.client_order_id
            != managed_order2.order_request.client_order_id
        )
