...
from src.exchanges.connection_manager import ConnectionManager as BinanceSandboxConnectionManager
from src.exchanges.connection_manager import ConnectionManager as CoinbaseSandboxConnectionManager
from src.exchanges.connection_manager import ConnectionManager as OKXSandboxConnectionManager
...
def pytest_runtest_setup(item):
    ...
    if not os.getenv("BINANCE_API_KEY") or not os.getenv("COINBASE_API_KEY") or not os.getenv("OKX_API_KEY"):
...
class TestCrossExchangeIntegration:
    ...
    async def test_concurrent_multi_exchange_operations(self, all_managers, sandbox_config):
        ...
        successful = sum(1 for result in results if not isinstance(result, Exception))
        assert successful >= len(tasks) * 0.8, "Too many failures in concurrent multi-exchange operations"
    ...
    async def test_environment_validation_consistency(self, all_managers):
        ...
        validation_tasks = [
            manager.validate_environment() for manager in all_managers.values()
        ]
        
        validations = await asyncio.gather(*validation_tasks, return_exceptions=True)
        
        # All validations should succeed
        successful_validations = [v for v in validations if not isinstance(v, Exception) and v.get("valid")]
    ...
    async def test_error_handling_consistency(self, all_managers):
        ...
        for exchange, error_type in error_test_results.items():
            assert error_type != "no_error", f"{exchange} should have raised an error for invalid endpoint"
...
class TestSandboxReliabilityAndStress:
    ...
    async def test_rapid_requests_handling(self, sandbox_config):
        ...
        try:
            ...
            successful = sum(1 for result in results if not isinstance(result, Exception))
            rate_limited = sum(1 for result in results if isinstance(result, Exception) 
                             and "429" in str(result))
...
def calculate_test_coverage_estimate():
    ...
    weighted_coverage = sum(
        coverage_areas[area] * weights[area] 
