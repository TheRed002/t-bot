...
def pytest_runtest_setup(item):
    ...
    if not os.getenv("BINANCE_API_KEY") or not os.getenv("COINBASE_API_KEY") or not os.getenv("OKX_API_KEY"):
        pytest.skip("WebSocket API credentials not configured - skipping WebSocket integration tests")
...
@pytest.mark.skip(reason="Uses mocks and BaseWebSocketClient which doesn't exist - needs rewrite with real WebSocket connections")
class TestWebSocketReliability:
    ...
    async def test_websocket_message_handling(self):
        ...
        test_message = json.dumps({"stream": "btcusdt@ticker", "data": {"s": "BTCUSDT", "c": "50000"}})
