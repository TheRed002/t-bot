            ...
            try:
                if method == "GET":
                    ...
                    response = client.post(
                        endpoint, json={"username": "test", "password": "test"}, timeout=5
                    )
                else:
                ...
                if status == 200:
                    results["working"].append(
                        {"endpoint": endpoint, "name": name, "status": status}
                    )
                ...
                results["errors"].append(
                    {"endpoint": endpoint, "name": name, "error": str(e)[:100]}
                )
...
def create_mock_results():
    ...
    results = {
        ...
        "auth_required": [
            {
                "endpoint": "/api/analytics/portfolio/metrics",
