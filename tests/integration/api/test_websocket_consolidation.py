...
class TestWebSocketConsolidation:
    ...
    async def test_resource_tracking(self, mock_config):
        ...
        try:
            ...
            # Register connection
            result = manager.register_connection("test_conn_1", "binance", mock_connection)
            assert result is True
    ...
    async def test_resource_leak_detection(self, resource_manager):
        ...
        mock_connection = Mock()
        resource_manager.register_connection("leak_test", "binance", mock_connection)
    ...
    async def test_force_cleanup(self, resource_manager):
        ...
        for i in range(3):
            mock_connection = Mock()
            resource_manager.register_connection(f"test_conn_{i}", "binance", mock_connection)
    ...
    async def test_resource_monitoring_overhead(self, websocket_manager):
        ...
        for i in range(5):  # Reduced from 100 to respect connection limits
            try:
                conn_id = await websocket_manager.create_connection(
                    "wss://stream.binance.com:9443/ws/btcusdt@ticker", "binance", f"perf_test_{i}"
                )
                if conn_id:
                    connection_ids.append(conn_id)
        ...
        assert (
            ...
            <= websocket_manager.config.websocket.max_total_connections
        )
