...
from src.data.pipeline.processing import DataProcessor
...
class TestWebSocketConnections:
    ...
    async def test_multi_exchange_websocket_stability(self, performance_monitor):
        ...
        stable_connections = sum(1 for status in connection_status.values() if status == "connected")
    ...
    async def test_websocket_failover_mechanisms(self):
        ...
        class FailoverWebSocketManager:
            ...
            async def failover_to_backup(self):
                ...
                self.current_connection = "backup"
                logger.info(f"{self.exchange_name} failed over to backup (failover #{self.failover_count})")
...
class TestMarketDataPipeline:
    ...
    async def test_data_processing_pipeline_throughput(self, performance_monitor):
        ...
        class MockDataProcessor:
            ...
            def get_performance_stats(self) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "throughput_per_second": self.processed_count / sum(self.processing_times) if self.processing_times else 0
                }
        ...
        logger.info(f"  Throughput: {stats['processed_count'] / total_processing_time:.1f} items/sec")
    ...
    async def test_data_pipeline_backpressure_handling(self):
        ...
        assert stats["processed_count"] + stats["dropped_count"] + stats["queue_size"] == len(test_items)
    ...
    async def test_real_time_portfolio_risk_updates(self):
        ...
        class RealTimeRiskMonitor:
            ...
            async def _calculate_risk_metrics(self) -> Dict[str, Any]:
                ...
                # Drawdown
                drawdown = (max_value - current_value) / max_value if max_value > 0 else Decimal("0")
            ...
            async def _check_risk_alerts(self, risk_metrics: Dict[str, Any]):
                ...
                if risk_metrics["volatility"] > Decimal("0.05"):  # 5% volatility
                    alert = {
                        "type": "volatility",
                        "level": "high" if risk_metrics["volatility"] > Decimal("0.1") else "medium", 
            ...
            def get_current_risk_level(self) -> str:
                ...
                if (latest_metrics["drawdown"] > Decimal("0.15") or 
                    latest_metrics["volatility"] > Decimal("0.1")):
                    return "high"
                
                # Medium risk conditions  
                elif (latest_metrics["drawdown"] > Decimal("0.05") or
                      latest_metrics["volatility"] > Decimal("0.03")):
                    return "medium"
        ...
        drawdown_alerts = [alert for alert in risk_monitor.risk_alerts if alert["type"] == "drawdown"]
        ...
        logger.info(f"Real-time risk monitoring results:")
