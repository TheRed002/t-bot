...
class TestStaticStrategiesIntegration:
    ...
    async def test_trend_following_strategy_integration(
        ...
        for i in range(60):  # More data for slow MA calculation
            ...
            # More variation to keep RSI moderate
            variation = (i % 10 - 5) * 25
    ...
    async def test_breakout_strategy_integration(self, breakout_config, mock_risk_manager):
        ...
        for i in range(25):
            if i < 20:
                # Consolidation period - create more realistic consolidation
                # around 50000
                base_price = 50000
        ...
        # Disable support/resistance update for testing (keep manually set
        # levels)
        original_update_method = strategy._update_support_resistance_levels
        strategy._update_support_resistance_levels = lambda: None

        # Disable consolidation check for this test (we want to test breakout
        # detection)
