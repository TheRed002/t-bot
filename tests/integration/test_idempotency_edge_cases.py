...
from src.core.exceptions import ExecutionError, ExchangeConnectionError
...
class TestIdempotencyEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_redis_connection_failure_during_operation(self, mock_config, sample_order_request):
        ...
        client_id1, is_duplicate1 = await manager.get_or_create_idempotency_key(sample_order_request)
        assert client_id1 is not None
        assert is_duplicate1 is False
        
        # Second call should detect duplicate from in-memory cache
        client_id2, is_duplicate2 = await manager.get_or_create_idempotency_key(sample_order_request)
    ...
    async def test_high_concurrency_race_conditions(self, mock_config, sample_order_request):
        ...
        assert len(unique_client_ids) == 1, f"Expected 1 unique client_id, got {len(unique_client_ids)}"
