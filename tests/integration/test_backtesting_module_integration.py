...
class TestBacktestingModuleIntegration:
    ...
    async def backtest_service(self, mock_config, mock_dependencies, dependency_injector):
        ...
        service = BacktestService(
            config=mock_config, injector=dependency_injector, **mock_dependencies
        )
    ...
    async def test_backtest_service_uses_data_service_correctly(self, backtest_service):
        ...
        with patch.object(
            backtest_service, "_run_core_simulation", new_callable=AsyncMock
        ) as mock_simulation:
            mock_simulation.return_value = {"equity_curve": [], "trades": [], "daily_returns": []}

            with patch.object(
                backtest_service, "_run_advanced_analysis", new_callable=AsyncMock
            ) as mock_analysis:
    ...
    async def test_strategy_service_backtest_integration(self):
        ...
        strategy_service = StrategyService(
            name="test_strategy_service", config={}, service_manager=mock_service_manager
        )
    ...
    async def test_optimization_integration_uses_service_layer(self):
        ...
        assert (
            "BacktestEngine(" not in source or source.count("BacktestEngine(") <= 1
        )  # Allow for imports
