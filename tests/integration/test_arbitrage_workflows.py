...
from src.core.exceptions import ExecutionError, ArbitrageError
from src.core.types import (
    MarketData, Order, OrderSide, OrderType, OrderStatus, ArbitrageOpportunity, Ticker
)
    ...
    return {
        "binance": {
            "BTC/USDT": create_mock_ticker(
                "BTC/USDT", Decimal("50000.0"), Decimal("49995.0"), 
                Decimal("50005.0"), Decimal("1000.0"), "binance"
            ),
            "ETH/USDT": create_mock_ticker(
                "ETH/USDT", Decimal("3000.0"), Decimal("2998.0"), 
                Decimal("3002.0"), Decimal("5000.0"), "binance"
            )
        },
        "coinbase": {
            "BTC/USDT": create_mock_ticker(
                "BTC/USDT", Decimal("50200.0"), Decimal("50190.0"), 
                Decimal("50210.0"), Decimal("800.0"), "coinbase"
            ),
            "ETH/USDT": create_mock_ticker(
                "ETH/USDT", Decimal("2980.0"), Decimal("2978.0"), 
                Decimal("2982.0"), Decimal("4000.0"), "coinbase"
            )
        },
        "okx": {
            "BTC/USDT": create_mock_ticker(
                "BTC/USDT", Decimal("49950.0"), Decimal("49945.0"), 
                Decimal("49955.0"), Decimal("1200.0"), "okx"
            ),
            "ETH/USDT": create_mock_ticker(
                "ETH/USDT", Decimal("3020.0"), Decimal("3018.0"), 
                Decimal("3022.0"), Decimal("3500.0"), "okx"
            )
        }
    }
...
class TestCrossExchangeArbitrageDetection:
    ...
    @pytest.mark.asyncio
    async def test_simple_arbitrage_detection(self, mock_config, mock_exchanges, sample_arbitrage_prices):
    ...
    @pytest.mark.asyncio
    async def test_arbitrage_with_fees_calculation(self, mock_config, mock_exchanges, sample_arbitrage_prices):
        ...
        coinbase_sell_fee = sell_price * quantity * mock_exchanges[1].fees["taker"]  # Coinbase taker fee
    ...
    @pytest.mark.asyncio
    async def test_multi_symbol_arbitrage_scanning(self, mock_config, mock_exchanges, sample_arbitrage_prices):
        ...
        for symbol in symbols:
            ...
            for exchange in mock_exchanges:
                exchange.get_market_data = AsyncMock(
                    side_effect=lambda s, exchange_name=exchange.name: sample_arbitrage_prices[exchange_name][s]
                )
...
class TestArbitrageExecutionWorkflow:
    ...
    async def test_execution_timing_and_latency(self, mock_config, mock_exchanges):
        ...
        try:
            results = await asyncio.wait_for(
                asyncio.gather(
                    mock_exchanges[0].place_order({}),
                    mock_exchanges[1].place_order({})
                ),
    ...
    async def test_partial_execution_handling(self, mock_config, mock_exchanges):
        ...
        # Mock order statuses
        buy_exchange.get_order_status = AsyncMock(return_value={
            "id": "buy_123",
        ...
        sell_exchange.get_order_status = AsyncMock(return_value={
            "id": "sell_456", 
        ...
        if (buy_status["status"] == OrderStatus.FILLED and 
            sell_status["status"] == OrderStatus.PARTIALLY_FILLED):
            
            # Calculate residual position
            residual_quantity = (buy_status["filled_quantity"] - 
                               sell_status["filled_quantity"])
...
class TestTriangularArbitrageWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_triangular_arbitrage_execution_sequence(self, mock_config, mock_exchanges, triangular_prices):
...
class TestArbitrageRiskManagement:
    ...
    async def test_exposure_limits(self, mock_config):
        ...
        new_exposure = risk_manager.current_arbitrage_exposure + (small_opportunity.quantity * Decimal("50000.0"))
        ...
        new_exposure_large = risk_manager.current_arbitrage_exposure + (large_opportunity.quantity * Decimal("50000.0"))
    ...
    async def test_exchange_reliability_scoring(self, mock_config, mock_exchanges):
        ...
        reliable_exchanges = [
            name for name, score in reliability_scores.items()
...
class TestArbitrageProfitRealization:
    ...
    async def test_profit_calculation_accuracy(self, mock_config):
        ...
        gross_profit = (trade_details["sell_price"] - trade_details["buy_price"]) * trade_details["quantity"]
        total_costs = (trade_details["buy_fee"] + trade_details["sell_fee"] + 
                      trade_details["slippage_cost"])
        net_profit = gross_profit - total_costs
        profit_margin = (net_profit / (trade_details["buy_price"] * trade_details["quantity"])) * 100
        ...
        assert abs(net_profit - Decimal("-80.905")) < Decimal("0.001")  # Actually a loss due to high fees
    ...
    async def test_arbitrage_performance_tracking(self, mock_config):
        ...
        success_rate = (performance_tracker.successful_trades / 
                       performance_tracker.executed_trades) * 100
        net_profit_after_fees = performance_tracker.total_profit - performance_tracker.total_fees_paid
        avg_profit_per_trade = performance_tracker.total_profit / performance_tracker.executed_trades
...
class TestArbitrageIntegrationScenarios:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_arbitrage_workflow(self, mock_config, mock_exchanges, sample_arbitrage_prices):
        ...
        valid_opportunities = [
            ...
            if risk_manager.validate_arbitrage_opportunity(opp)
        ]
        ...
        execution_result = {
            ...
            "realized_profit": best_opportunity.profit_amount * Decimal("0.95"),  # 5% less due to fees/slippage
    ...
    async def test_high_frequency_arbitrage_scenario(self, mock_config, mock_exchanges):
        ...
        for i in range(total_opportunities):
            ...
            opportunity.sell_exchange = "coinbase"
            opportunity.buy_price = Decimal("50000.0") + Decimal(str(i % 10))  # Slight price variation
    ...
    async def test_market_stress_arbitrage_behavior(self, mock_config, mock_exchanges):
        ...
        stressed_prices = {
            "binance": create_stress_mock(
                "BTC/USDT", Decimal("50000.0"), Decimal("49900.0"), 
                Decimal("50100.0"), Decimal("100.0"), "binance"
            ),
            "coinbase": create_stress_mock(
                "BTC/USDT", Decimal("50500.0"), Decimal("50350.0"), 
                Decimal("50650.0"), Decimal("80.0"), "coinbase"
            )
        }
        ...
        arbitrage_strategy.min_profit_threshold = Decimal("0.01")  # Increase threshold during stress
