...
from tests.integration.base_integration import (
    BaseIntegrationTest, MockExchangeFactory, MockStrategyFactory,
    performance_test, wait_for_condition
)
from src.core.types import (
    MarketData, Signal, SignalDirection, Order, OrderSide, OrderType, OrderStatus,
    Position, ExecutionInstruction, ExecutionResult, ExecutionStatus, BotStatus
)
...
class TestCompleteSignalToExecution(BaseIntegrationTest):
    ...
    async def test_buy_signal_complete_workflow(self, performance_monitor):
        ...
        state_manager.get_portfolio_state = AsyncMock(return_value={
        ...
        order_filled = await wait_for_condition(
            lambda: binance._orders[order_id].status == OrderStatus.FILLED,
    ...
    async def test_sell_signal_with_existing_position(self, performance_monitor):
        ...
        risk_manager = Mock(spec=RiskManager)
        risk_manager.get_position = AsyncMock(return_value=Position(
            symbol="BTC/USDT",
            side=OrderSide.LONG,
        ...
        risk_manager.validate_trade = AsyncMock(return_value=True)
        risk_manager.calculate_position_size = AsyncMock(return_value=Decimal("0.1"))  # Partial close
    ...
    async def test_multi_strategy_signal_aggregation(self):
        ...
        momentum.generate_signal = AsyncMock(return_value=Signal(
            symbol="BTC/USDT",
        ...
        mean_reversion.generate_signal = AsyncMock(return_value=Signal(
            symbol="BTC/USDT",
        ...
        buy_confidence = sum(s[1].confidence for s in strategy_signals if s[1].direction == SignalDirection.BUY)
        sell_confidence = sum(s[1].confidence for s in strategy_signals if s[1].direction == SignalDirection.SELL)
...
class TestOrderExecutionScenarios(BaseIntegrationTest):
    ...
    async def test_limit_order_execution_workflow(self):
        ...
        if fill_percentage >= 25:  # Accept if 25%+ filled
            ...
            logger.info(f"Accepted partial fill: {order.filled_quantity} BTC @ ${order.average_price}")
    ...
    async def test_slippage_and_price_impact(self):
        ...
        async def slippage_simulation(order_data):
            ...
            if order_data["side"] == "BUY":
                ...
                binance._balances[base_asset] = binance._balances.get(base_asset, Decimal("0")) + quantity
            
            logger.info(f"Order executed with slippage: {slippage_bps}bps, price: ${execution_price}")
            return order_id
...
class TestPositionManagementIntegration(BaseIntegrationTest):
    ...
    async def test_position_lifecycle_management(self):
        ...
        logger.info(f"Opened position: {positions['BTC/USDT'].quantity} BTC @ ${positions['BTC/USDT'].average_price}")
        ...
        new_average_price = (
            (existing_position.quantity * existing_position.average_price +
             additional_order.filled_quantity * additional_order.average_price) / total_quantity
        )
        ...
        logger.info(f"Increased position to: {positions['BTC/USDT'].quantity} BTC @ ${positions['BTC/USDT'].average_price}")
        ...
        realized_pnl = (sell_order.average_price - positions["BTC/USDT"].average_price) * sell_order.filled_quantity
        ...
        logger.info(f"Partial close: remaining {positions['BTC/USDT'].quantity} BTC, realized P&L: ${realized_pnl}")
        ...
        final_pnl = (final_order.average_price - positions["BTC/USDT"].average_price) * final_order.filled_quantity
    ...
    async def test_multi_symbol_position_management(self):
        ...
        logger.info(f"Multi-symbol portfolio value: BTC ${btc_value}, ETH ${eth_value}, Total ${total_position_value}")
        ...
        logger.info(f"Correlation risk detected: {correlation_risk}, diversification benefit: {effective_diversification}")
    ...
    async def test_position_pnl_tracking(self):
        ...
        for current_price in price_scenarios:
            ...
            logger.info(f"Price: ${current_price}, Unrealized P&L: ${unrealized_pnl} ({pnl_percent:.1f}%)")
