...
class TestBacktestingIntegrationSimple:
    ...
    def test_strategy_service_backtest_integration(self):
        ...
        strategy_service = StrategyService(
            name="test", config={}, service_manager=mock_service_manager
        )
        ...
        async def test_run():
            await strategy_service._run_backtest_impl(
                "test",
                {
                    "symbols": ["BTCUSDT"],
    ...
    def test_optimization_module_uses_service_layer(self):
        """Test optimization module uses service layer."""
        import inspect
        ...
        # Should not directly instantiate BacktestEngine in function calls
        function_calls = [line.strip() for line in source.split("\n") if "BacktestEngine(" in line]
        # Allow imports but not direct instantiation in function bodies
        assert len(function_calls) == 0 or all(
            "import" in call or "from" in call for call in function_calls
        )
