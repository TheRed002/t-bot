...
class TestCircuitBreakerActivation:
    ...
    async def test_loss_threshold_circuit_breaker(self, mock_config, sample_portfolio_in_distress):
        ...
        daily_unrealized_loss = abs(sample_portfolio_in_distress.unrealized_pnl) * Decimal(
            "0.5"
        )  # Assume 50% is today's loss
...
class TestEmergencyProcedures:
    ...
    async def test_order_cancellation_procedure(self, mock_config, sample_portfolio_in_distress):
        ...
        order_manager.get_all_open_orders = Mock(
            return_value=list(sample_portfolio_in_distress.open_orders.values())
        )
        order_manager.cancel_order = AsyncMock(return_value=True)
        order_manager.cancel_all_orders = AsyncMock(return_value=True)

        # Activate circuit breaker
        await emergency_controls.activate_circuit_breaker(
    ...
    async def test_position_liquidation_procedure(self, mock_config, sample_portfolio_in_distress):
        ...
        should_liquidate = (
            drawdown >= mock_config.risk.emergency.liquidation_threshold
            and mock_config.risk.emergency.liquidate_positions
        )
...
class TestCircuitBreakerRecovery:
    ...
    async def test_recovery_monitoring_and_fallback(self, mock_config):
        ...
        for trade in post_recovery_trades:
            ...
            if (
                abs(post_recovery_pnl) > Decimal("1000.0")  # Significant losses resume
                or consecutive_losses_after_recovery >= 3
...
class TestCircuitBreakerSystemIntegration:
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_end_to_end_circuit_breaker_workflow(
        self, mock_config, sample_portfolio_in_distress
    ):
