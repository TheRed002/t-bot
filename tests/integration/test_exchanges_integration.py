...
class TestExchangeFactoryIntegration:
    ...
    async def test_exchange_lifecycle_management(self, config, setup_logging_for_tests):
        ...
        exchange = await factory.get_exchange("mock")  # Use get_exchange to ensure tracking
    ...
    async def test_multiple_exchanges_management(self, config, setup_logging_for_tests):
        ...
        exchanges = []
        for i in range(3):
            exchange = await factory.get_exchange(f"mock_{i}")  # Use get_exchange to ensure tracking
...
class TestExchangeIntegration:
    ...
    async def test_exchange_with_connection_management(self, config, setup_logging_for_tests):
        ...
        # ExchangeInfo doesn't have a status field, but we can check it's properly initialized
        assert info.supported_symbols == ["BTCUSDT", "ETHUSDT"]
        
        # Test disconnection
        await factory.remove_exchange("mock")
        # Note: remove_exchange doesn't disconnect, it just removes from tracking
    ...
    async def test_exchange_error_handling_integration(self, config, setup_logging_for_tests):
        ...
        with patch.object(exchange, 'get_ticker', side_effect=ValidationError("Invalid symbol")):
            with pytest.raises(ValidationError):
                await exchange.get_ticker("")  # Empty symbol should raise ValidationError
...
class TestComprehensiveExchangeIntegration:
    ...
    async def test_error_recovery_scenarios(self, config, setup_logging_for_tests):
        ...
        bucket = rate_limiter.buckets["orders_per_second"]  # Use orders_per_second for faster recovery
        bucket.tokens = 0  # Empty bucket
    ...
    async def test_performance_under_load(self, config, setup_logging_for_tests):
        ...
        for result in results:
            if isinstance(result, Exception):
                # Some operations might fail due to validation errors or rate limiting
