...
class TestWebInterfaceIntegration:
    ...
    def test_app_startup(self, web_interface_client):
    ...
    def test_api_docs_available(self, web_interface_client):
        """Test that API documentation is available."""
        # Test OpenAPI spec
        response = web_interface_client.get("/openapi.json")
        assert response.status_code == 200
    ...
    def test_middleware_integration(self, web_interface_client):
    ...
    def test_error_handling_integration(self, web_interface_client):
        """Test error handling integration."""
        # Test 404 error
        response = web_interface_client.get("/nonexistent-endpoint")
    ...
    def test_bot_api_integration(self, web_interface_client, auth_headers):
    ...
    def test_portfolio_api_integration(self, web_interface_client, auth_headers):
    ...
    def test_trading_api_integration(self, web_interface_client, auth_headers):
    ...
    def test_security_headers_integration(self, web_interface_client):
    ...
    def test_complete_workflow_integration(self, web_interface_client):
        """Test a complete workflow from health check to API exploration."""
        # Step 1: Check system health
        health_response = web_interface_client.get("/health")
        assert health_response.status_code == 200
        assert "status" in health_response.json()

        # Step 2: Check API versions
        versions_response = web_interface_client.get("/api/versions")
        assert versions_response.status_code == 200
        assert "versions" in versions_response.json()

        # Step 3: Check documentation
        docs_response = web_interface_client.get("/docs")
        assert docs_response.status_code == 200

        # Step 4: Check root endpoint
        root_response = web_interface_client.get("/")
