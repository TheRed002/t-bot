...
class TestWebInterfaceModuleIntegration:
    ...
    def injector(self, config):
        ...
        mock_execution_engine.get_positions = AsyncMock(
            return_value=[
                {
                    "symbol": "BTCUSDT",
                    "size": 0.1,
        ...
        mock_bot_orchestrator.get_bot_status = AsyncMock(
            return_value={"bot_id": "bot_123", "status": "running", "performance": {"pnl": 150.0}}
        )
        mock_bot_orchestrator.get_all_bots_status = AsyncMock(
            return_value={
                "bots": {
                    "bot_123": {
        ...
        mock_data_service.get_recent_data = AsyncMock(
            return_value=[
                Mock(
        ...
        mock_portfolio_manager.get_balances = AsyncMock(return_value={"USDT": 5000.0, "BTC": 0.1})
        mock_portfolio_manager.get_summary = AsyncMock(
            return_value={
                "total_value": 10000.0,
        ...
        mock_risk_manager.validate_order = AsyncMock(
            return_value={"valid": True, "risk_score": 0.25, "warnings": []}
        )
        mock_risk_manager.get_risk_metrics = AsyncMock(
            return_value={"portfolio_var": 1250.0, "max_drawdown": 0.15, "sharpe_ratio": 1.35}
        )
        injector.register_service("RiskManager", mock_risk_manager)

        # Mock strategy service and factory
        mock_strategy_service = Mock()
        mock_strategy_service.get_all_strategies = AsyncMock(
            return_value={"momentum": {"strategy_id": "momentum", "status": "active"}}
        )
        injector.register_service("StrategyService", mock_strategy_service)

        mock_strategy_factory = Mock()
        mock_strategy_factory.list_available_strategies = Mock(
            return_value={
                "momentum": {
                    "class_name": "MomentumStrategy",
                    "required_parameters": ["fast_ma", "slow_ma"],
                }
            }
        )
    ...
    async def test_risk_service_integration(self, injector):
        ...
        result = await risk_service.validate_order(
            symbol="BTCUSDT", side=OrderSide.BUY, amount=Decimal("0.1"), price=Decimal("45000")
        )
