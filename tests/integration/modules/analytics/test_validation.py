...
class TestAnalyticsModuleBoundaries:
                        ...
                        for node in ast.walk(tree):
                            if isinstance(node, (ast.Import, ast.ImportFrom)):
                                if isinstance(node, ast.ImportFrom) and node.module:
                                    ...
                                    if (node.module.startswith('src.exchanges') and 
                                        not node.module in ['src.exchanges.interfaces', 'src.exchanges.types']):
                                        # Should not import exchange internals
                                        assert False, f"Analytics importing exchange internals: {node.module} in {file_path}"
                    except SyntaxError:
    ...
    def test_analytics_error_handling_patterns(self):
        ...
        from src.core.types import Position, Trade, Order
