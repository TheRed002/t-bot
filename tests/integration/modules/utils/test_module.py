...
from src.utils.validation.service import ValidationService, ValidationContext, ValidationResult
...
class TestUtilsModuleIntegration:
    ...
    async def test_validation_service_error_handling(self):
        ...
        try:
            ...
            high_severity_errors = [error for error in result.errors if error.severity.name == "HIGH"]
    ...
    async def test_strategy_integrated_base_dependency_injection(self):
        ...
        assert strategy.validation_service.is_running or not strategy.validation_service.is_running  # May or may not be running
