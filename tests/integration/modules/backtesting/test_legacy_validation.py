...
class TestBacktestingIntegrationValidation:
    ...
    async def test_service_dependency_resolution(self, injector):
        ...
        # Repository may be None if DatabaseServiceInterface is not available in test setup
    ...
    async def test_factory_creates_proper_components(self, injector):
        ...
        # Test factory can create repository (may fail if DatabaseServiceInterface not available)
        try:
            repository = factory.create_repository()
            assert repository is not None
        except Exception:
            # Repository creation may fail in tests without DatabaseServiceInterface
    ...
    async def test_service_error_propagation(self, injector, mock_services):
        ...
        # The service handles errors gracefully via retry mechanism
        # When all retries fail, it may return None or an error result
