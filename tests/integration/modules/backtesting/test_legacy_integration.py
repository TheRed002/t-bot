...
class TestBacktestingIntegration:
    ...
    def _create_data_service_mock(self, mock_db_manager):
        """Create a properly configured data service mock."""
        mock_data_service = AsyncMock(spec=DataServiceInterface)

        async def mock_get_market_data(request: DataRequest):
            """Mock implementation that returns the data in expected format."""
            # Use the existing mock_db_manager's fetch_all method with correct field names
            raw_data = await mock_db_manager.fetch_all(
                "query", request.symbol, request.start_time, request.end_time
            )
            # Return the data in the format expected by BacktestEngine
            return raw_data

        mock_data_service.get_market_data = mock_get_market_data
        mock_data_service.initialize = AsyncMock()
        mock_data_service.cleanup = AsyncMock()
    ...
    def mock_db_manager(self):
        ...
        def generate_market_data(symbol: str, start_date: datetime, end_date: datetime):
            ...
            # Create OHLCV data as objects with expected attributes
            from decimal import Decimal
    ...
    async def test_end_to_end_backtest_single_symbol(
        ...
        assert isinstance(result.annual_return_pct, Decimal)
        assert isinstance(result.max_drawdown_pct, Decimal)
        assert 0 <= result.win_rate_pct <= 100
        # profit_factor can be None if there are no trades or no losing trades
    ...
    async def test_backtest_metrics_integration(self, backtest_config, strategy, mock_db_manager):
        ...
        engine = BacktestEngine(
            config=backtest_config, strategy=strategy, data_service=mock_data_service
        )
        ...
        # Check for actual metric keys returned by the calculator
    ...
    async def test_backtest_performance_large_dataset(self, strategy, mock_db_manager):
        ...
        # daily_returns may be empty if no trades were made
        assert len(result.daily_returns) >= 0
            ...
            for i, date in enumerate(dates):
                ...
                if i == 0:
                    ...
                    if np.random.random() < 0.02:  # 2% chance of large jump
                        price = float(data[-1].close_price) * (1 + np.random.uniform(-0.1, 0.1))
                    else:
                        price = float(data[-1].close_price) * (1 + np.random.normal(0, 0.01))
                ...
                # Create mock record object with expected attributes
                from decimal import Decimal
...
class TestBacktestingDatabaseIntegration:
    ...
    async def test_database_connection_handling(self, clean_database):
        ...
        # Use real database service for Phase 2 integration with clean database fixture
