...
class TestMonitoringDependencyInjection:
    ...
    async def test_monitoring_services_registration(
        self, injector, mock_alert_manager, mock_metrics_collector, mock_performance_profiler
    ):
        """Test that monitoring services register correctly with dependency injection."""
        # Mock the components
        with (
            patch("src.monitoring.di_registration.AlertManager", return_value=mock_alert_manager),
            patch(
                "src.monitoring.di_registration.MetricsCollector",
                return_value=mock_metrics_collector,
            ),
            patch(
                "src.monitoring.di_registration.PerformanceProfiler",
    ...
    @pytest.mark.asyncio
    @pytest.mark.timeout(300)
    async def test_default_performance_service_dependency_injection(
        self, mock_performance_profiler
    ):
