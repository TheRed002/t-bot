...
class TestRealPositionSizing:
    ...
    @pytest.mark.asyncio
    async def test_confidence_weighted_position_sizing(
...
class TestRealRiskMetricsCalculation:
    ...
    async def test_maximum_drawdown_calculation(
        ...
        assert risk_metrics.max_drawdown > Decimal("0"), "Drawdown should be positive (percentage loss)"
...
class TestRealRiskValidation:
    ...
    @pytest.mark.asyncio
    async def test_signal_validation_with_risk_checks(
        self, real_risk_service: RiskService
    ):
    ...
    @pytest.mark.asyncio
    async def test_signal_validation_rejects_low_confidence(
        self, real_risk_service: RiskService
    ):
    ...
    @pytest.mark.asyncio
    async def test_order_validation_rejects_excessive_size(
        self, real_risk_service: RiskService
    ):
...
class TestRealPortfolioLimits:
    ...
    async def test_max_exposure_limit_enforcement(
        ...
        current_exposure = sum(
            pos.quantity * pos.current_price for pos in sample_positions
        )
...
class TestRealEmergencyControls:
    ...
    async def test_emergency_stop_prevents_new_orders(
        ...
        await real_risk_service.activate_emergency_stop(
...
class TestRealConcurrentRiskChecks:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_position_size_calculations(
        self, real_risk_service: RiskService
    ):
    ...
    async def test_concurrent_risk_metric_calculations(
        ...
        market_data_sets = [
            generate_realistic_market_data_sequence(periods=50) for _ in range(5)
        ]

        # WHEN: Calculate risk metrics concurrently
        tasks = [
            real_risk_service.calculate_risk_metrics(
                positions=sample_positions,
                market_data=md_set
            )
...
class TestRealDecimalPrecision:
    ...
    async def test_risk_metrics_decimal_precision(
        ...
        for field_name in ['var_95', 'expected_shortfall', 'sharpe_ratio',
                           'sortino_ratio', 'max_drawdown', 'total_exposure']:
...
class TestRealDatabasePersistence:
    ...
    @pytest.mark.asyncio
    async def test_risk_alert_persistence(
        self, real_risk_service: RiskService
    ):
