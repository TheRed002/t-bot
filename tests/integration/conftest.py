    ...
    return {
        ...
        "strategies": {
            "momentum": {
                ...
                "timeframe": "1m",
                "parameters": {"lookback_period": 14, "threshold": 0.02},
            },
            "mean_reversion": {
                "enabled": True,
                "symbols": ["BTC/USDT", "ETH/USDT", "ADA/USDT"],
                "timeframe": "5m",
                "parameters": {"bollinger_period": 20, "std_dev": 2.0},
            },
        },
        "websocket": {
...
async def mock_strategies():
    ...
    strategies = {
        "momentum": MockStrategyFactory.create_momentum_strategy(
            symbol="BTC/USDT", signal_confidence=0.8, signal_frequency=0.4
        ),
        "mean_reversion": MockStrategyFactory.create_mean_reversion_strategy(
            symbol="BTC/USDT", signal_confidence=0.7, contrarian=True
        ),
    }
...
def sample_signals():
    ...
    signals = [
        Signal(
            ...
            metadata={"rsi": 35.2, "macd": 1.5, "volume_ratio": 1.8, "trend": "bullish"},
        ),
        Signal(
            symbol="ETH/USDT",
            ...
            metadata={"bollinger_position": 0.95, "rsi": 78.5, "mean_reversion_strength": 0.7},
        ),
        Signal(
            symbol="ADA/USDT",
            ...
            strategy_name="momentum",
            metadata={"rsi": 52.1, "macd": -0.1, "volume_ratio": 0.9, "trend": "sideways"},
        ),
    ]
    ...
    class MockDatabase:
        ...
        async def fetch_all(self, query: str, *args) -> list[dict[str, Any]]:
            ...
            if "positions" in query.lower():
                ...
                return [
                    {"id": "order_001", "symbol": "BTC/USDT", "side": "BUY", "status": "FILLED"}
                ]
...
def mock_jwt_handler():
    ...
    class MockJWTHandler:
        ...
        def get_stats(self) -> dict[str, int]:
            ...
            return {"tokens_issued": self.tokens_issued, "tokens_validated": self.tokens_validated}
...
async def load_test_config():
    ...
    return {
        ...
        "load_test_scenarios": {
            ...
            "sustained_load": {"rps": 50, "duration_seconds": 60, "ramp_up_seconds": 10},
            "stress_test": {"max_rps": 500, "duration_seconds": 30, "acceptable_degradation": 0.2},
        },
        "resource_limits": {
            "max_memory_mb": 1000,
            "max_cpu_percent": 80,
            ...
            while self.monitoring:
                try:
                    ...
                    metrics = {
                        ...
                        "disk_io_read": system.disk_io_counters().read_bytes
                        if system.disk_io_counters()
                        else 0,
                        "disk_io_write": system.disk_io_counters().write_bytes
                        if system.disk_io_counters()
                        else 0,
                        "network_sent": system.net_io_counters().bytes_sent
                        if system.net_io_counters()
                        else 0,
                        "network_recv": system.net_io_counters().bytes_recv
                        if system.net_io_counters()
                        else 0,
                    }
...
async def concurrent_test_executor():
    ...
    class ConcurrentTestExecutor:
        ...
        async def run_concurrent_tasks(
            self, task_func: Callable, task_args: list[Any], max_concurrent: int = 10
        ) -> list[Any]:
        ...
        async def run_load_pattern(
            self, task_func: Callable, rps: int, duration_seconds: int, ramp_up_seconds: int = 0
        ) -> list[Any]:
