...
from src.core.types import (
    MarketData, Signal, SignalDirection, Order, OrderSide, OrderType, OrderStatus, TimeInForce,
...
class MockExchangeFactory:
    ...
    def create_binance_mock(
        ...
        async def place_order(order_data: Dict[str, Any]) -> str:
            ...
            order = Order(
                ...
                order_type=OrderType(order_data.get("type", "MARKET").lower()),  # Convert to lowercase
            ...
            if order.order_type == OrderType.MARKET:
                ...
                if order.side == OrderSide.BUY:
                    ...
                    if exchange._balances.get(quote_asset, Decimal("0")) >= cost:
                        ...
                        exchange._balances[base_asset] = exchange._balances.get(base_asset, Decimal("0")) + order.quantity
                else:  # SELL
                    if exchange._balances.get(base_asset, Decimal("0")) >= order.quantity:
                        ...
                        exchange._balances[quote_asset] = exchange._balances.get(quote_asset, Decimal("0")) + proceeds
    ...
    def create_coinbase_mock(
        ...
        exchange = MockExchangeFactory.create_binance_mock(initial_balance, market_prices, order_latency)
    ...
    def create_momentum_strategy(
        ...
        async def generate_signal() -> Signal:
            ...
            return Signal(
                ...
                price=Decimal("50000.0") + Decimal(str((random.random() - 0.5) * 1000)),  # Price noise
                timestamp=datetime.now(timezone.utc),
    ...
    def create_mean_reversion_strategy(
        ...
        async def generate_signal() -> Signal:
            ...
            if contrarian and random.random() > 0.3:  # Often contrarian
                direction = SignalDirection.SELL if base_direction == SignalDirection.BUY else SignalDirection.BUY
            else:
            ...
            return Signal(
                ...
                timestamp=datetime.now(timezone.utc),
                metadata={"strategy": "mean_reversion", "indicator_values": {"bollinger_position": -0.8, "rsi": 30.2}}
            )
...
class PerformanceMonitor:
    ...
    def stop(self):
        ...
        if self.metrics["order_latency"]:
            self.metrics["avg_order_latency"] = sum(self.metrics["order_latency"]) / len(self.metrics["order_latency"])
        if self.metrics["signal_generation_time"]:
            self.metrics["avg_signal_time"] = sum(self.metrics["signal_generation_time"]) / len(self.metrics["signal_generation_time"])
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if monitor.metrics["total_duration"] > max_duration:
                    pytest.fail(f"Test exceeded maximum duration: {monitor.metrics['total_duration']:.2f}s > {max_duration}s")
