...
from tests.integration.test_dependency_injection_integration import DependencyInjectionIntegrationTest
from tests.integration.test_performance_scalability_integration import PerformanceScalabilityTest
...
class ComprehensiveModuleIntegrationRunner:
    ...
    def __init__(self):
        self.test_suites = {
            "cross_module_integration": CrossModuleIntegrationTest,
            "dependency_injection_integration": DependencyInjectionIntegrationTest,
            "performance_scalability": PerformanceScalabilityTest
        }
        ...
        for suite_name, suite_class in self.test_suites.items():
            ...
            try:
                ...
                if suite_summary["failed"] == 0:
                    ...
                    if fail_fast:
                        logger.error(f"🛑 Stopping execution due to fail_fast=True")
                        break
    ...
    def _generate_final_report(self, overall_results: Dict[str, Any]) -> None:
        ...
        if summary["failed_suites"] == 0 and summary["failed_tests"] == 0:
            ...
            logger.error(f"💥 {summary['failed_suites']} test suites and {summary['failed_tests']} individual tests FAILED")
        ...
        for suite_name, suite_data in overall_results["test_suites"].items():
            ...
            if suite_data["status"] == "FAILED":
                ...
                failed_tests = [
                    test_name for test_name, test_result in suite_data["results"].items()
...
async def main():
    ...
    parser.add_argument(
        "--suite", 
        choices=["all", "cross_module_integration", "dependency_injection_integration", "performance_scalability"],
        default="all",
