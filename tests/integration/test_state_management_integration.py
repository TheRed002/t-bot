...
from src.core.types import (
    ...
    BotStatus, Signal, SignalDirection
)
...
class TestStatepersistence(BaseIntegrationTest):
    ...
    async def test_incremental_state_updates(self):
        ...
        current_state = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "positions": {
                "BTC/USDT": {"quantity": "1.0", "average_price": "50000.0"}
            },
        ...
        updates = [
            ...
            # New position opened
            {
                "type": "new_position",
                "position_updates": {
                    "ETH/USDT": {"quantity": "10.0", "average_price": "3000.0"}
                },
                "balance_updates": {
        ...
        assert recovered_state["positions"]["BTC/USDT"]["quantity"] == current_state["positions"]["BTC/USDT"]["quantity"]
    ...
    async def test_state_corruption_detection_and_repair(self):
        ...
        for i, corrupted_state in enumerate(corrupted_states):
            ...
            logger.info(f"  After repair - Structure: {structure_valid_after}, "
...
class TestCheckpointManagement(BaseIntegrationTest):
    ...
    async def test_checkpoint_recovery_scenarios(self):
        ...
        logger.info(f"Normal recovery: Loaded checkpoint {latest_checkpoint['id']} "
    ...
    async def test_checkpoint_integrity_validation(self):
        ...
        original_state = {
            ...
            "positions": {
                "BTC/USDT": {"quantity": "2.0", "average_price": "50000.0"}
            },
        ...
        for corrupted_checkpoint in corrupted_checkpoints:
            ...
            logger.info(f"Checkpoint {corrupted_checkpoint['id']}: "
...
class TestStateSynchronization(BaseIntegrationTest):
    ...
    async def test_conflict_resolution_in_state_sync(self):
        ...
        def resolve_by_consensus(updates):
            ...
            closest_update = min(updates, key=lambda u: abs(Decimal(u["update"]["portfolio_value"]) - average_value))
        ...
        logger.info(f"Timestamp resolution: Selected update from {timestamp_resolution['component']} "
        ...
        logger.info(f"Authority resolution: Selected update from {authority_resolution['component']} "
        ...
        value_range = max(Decimal(u["update"]["portfolio_value"]) for u in conflicting_updates) - \
