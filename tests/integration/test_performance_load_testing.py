...
from src.core.types.market import MarketData, Ticker
from src.core.types.trading import Order, Position, Trade
from tests.integration.base_integration import BaseIntegrationTest, PerformanceMonitor
...
class LoadTestMetrics:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'p95_response_time': sorted(self.request_times)[int(len(self.request_times) * 0.95)] if len(self.request_times) > 0 else 0,
            'p99_response_time': sorted(self.request_times)[int(len(self.request_times) * 0.99)] if len(self.request_times) > 0 else 0,
            'min_response_time': min(self.request_times),
            'max_response_time': max(self.request_times),
            'errors_per_second': self.error_count / max(sum(self.request_times), 1),
            'timeouts': self.timeout_count,
            'throughput_rps': len(self.request_times) / max(sum(self.request_times), 1) if self.request_times else 0
        }
...
class TestHighFrequencyTradingLoad(BaseIntegrationTest):
    ...
    async def test_burst_order_processing(self):
        ...
        assert stats['success_rate'] >= 0.95, f"Success rate {stats['success_rate']} below threshold"
        assert stats['avg_response_time'] < 0.05, f"Average response time {stats['avg_response_time']}s too high"
        assert stats['p95_response_time'] < 0.1, f"P95 response time {stats['p95_response_time']}s too high"
        assert successful_orders >= 95, f"Only {successful_orders}/100 orders processed successfully"
        assert total_duration < 10.0, f"Total processing time {total_duration}s too long"
        
        logger.info(f"Burst test completed: {successful_orders}/100 orders in {total_duration:.2f}s")
    ...
    async def test_concurrent_market_data_processing(self):
        ...
        await asyncio.gather(*[
            process_market_data_stream(symbol) for symbol in symbols
        ])
        total_duration = time.time() - start_time
        ...
        # Performance assertions
        assert stats['success_rate'] >= 0.98, f"Market data success rate {stats['success_rate']} too low"
        assert stats['avg_response_time'] < 0.01, f"Average processing time {stats['avg_response_time']}s too high"
        assert total_messages / total_duration >= 500, f"Throughput {total_messages/total_duration} msg/s too low"
...
class TestWebSocketConnectionLoad(BaseIntegrationTest):
    ...
    async def test_multiple_websocket_connections(self):
        ...
        start_time = time.time()
        await asyncio.gather(*[
            simulate_websocket_connection(i) for i in range(num_connections)
        ])
        total_duration = time.time() - start_time
        ...
        # Performance assertions
        assert stats['success_rate'] >= 0.99, f"WebSocket success rate {stats['success_rate']} too low"
        assert stats['avg_response_time'] < 0.005, f"Average message processing {stats['avg_response_time']}s too slow"
        assert total_messages / total_duration >= 80, f"Message throughput {total_messages/total_duration} msg/s too low"
        
        logger.info(f"Handled {num_connections} connections with {total_messages} messages in {total_duration:.2f}s")
    ...
    async def test_websocket_connection_resilience(self):
        ...
        reconnection_rate = successful_reconnections / connection_drops if connection_drops > 0 else 0
        assert reconnection_rate >= 0.8, f"Reconnection rate {reconnection_rate} too low"
        
        logger.info(f"Connection resilience test: {successful_reconnections}/{connection_drops} successful reconnections")
...
class TestDatabasePerformanceLoad(BaseIntegrationTest):
    ...
    async def test_concurrent_database_operations(self):
        ...
        start_time = time.time()
        await asyncio.gather(*[
            database_operation_task(i) for i in range(num_concurrent_ops)
        ])
        total_duration = time.time() - start_time
        ...
        # Performance assertions
        assert stats['success_rate'] >= 0.95, f"Database success rate {stats['success_rate']} too low"
        assert stats['avg_response_time'] < 0.1, f"Average database response {stats['avg_response_time']}s too slow"
        assert stats['p95_response_time'] < 0.2, f"P95 database response {stats['p95_response_time']}s too slow"
        assert total_operations / total_duration >= 200, f"Database throughput {total_operations/total_duration} ops/s too low"
    ...
    async def test_database_connection_pool_performance(self):
        ...
        start_time = time.time()
        await asyncio.gather(*[
            simulate_database_query(i) for i in range(num_queries)
        ])
        total_duration = time.time() - start_time
        ...
        assert max_active_connections <= pool_size, f"Connection pool overflow: {max_active_connections} > {pool_size}"
        
        stats = self.db_metrics.get_statistics()
        assert stats['success_rate'] >= 0.98, f"Database query success rate {stats['success_rate']} too low"
        
        logger.info(f"Connection pool test: {num_queries} queries, avg wait: {avg_wait_time:.3f}s, max active: {max_active_connections}")
...
class TestMemoryAndResourceLoad(BaseIntegrationTest):
    ...
    async def test_memory_usage_under_load(self):
        ...
        for cycle in range(10):
            ...
            try:
                ...
                logger.info(f"Memory cycle {cycle}: {current_memory:.1f}MB (delta: {current_memory - initial_memory:.1f}MB)")
        ...
        assert memory_growth < 500, f"Memory growth {memory_growth:.1f}MB exceeds threshold"  # 500MB max growth
        assert max_memory < initial_memory + 1000, f"Peak memory {max_memory:.1f}MB too high"  # 1GB max peak
        
        logger.info(f"Memory test completed: Growth {memory_growth:.1f}MB, Peak {max_memory:.1f}MB, Avg {avg_memory:.1f}MB")
    ...
    async def test_resource_cleanup_efficiency(self):
        ...
        logger.info(f"Resource cleanup test: {cleanup_efficiency:.2%} efficiency in {cleanup_duration:.2f}s")
...
class TestSystemIntegrationLoad(BaseIntegrationTest):
    ...
    async def test_end_to_end_system_load(self):
        ...
        assert stats['avg_response_time'] < 0.1, f"System response time {stats['avg_response_time']}s too high"
        assert total_orders_processed >= (load_test_config['concurrent_users'] * 
                                        load_test_config['orders_per_user_per_second'] * 
                                        load_test_config['test_duration_seconds'] * 0.8), "Order processing rate too low"
        assert total_market_updates >= (load_test_config['market_data_updates_per_second'] * 
                                      load_test_config['test_duration_seconds'] * 0.8), "Market data processing rate too low"
    ...
    async def test_system_degradation_under_extreme_load(self):
        ...
        for load_level in load_levels:
            ...
            logger.info(f"Load level {load_level}: {actual_rps:.1f} actual rps, {stats['success_rate']:.2%} success")
        ...
        for i, metrics in enumerate(degradation_metrics):
            ...
            assert metrics['success_rate'] >= 0.5, f"System failed at {load_level} rps: {metrics['success_rate']:.2%} success rate"
            
            # Check that response times don't become unreasonable
            assert metrics['avg_response_time'] <= 1.0, f"Response time too high at {load_level} rps: {metrics['avg_response_time']:.3f}s"
            
            # Check that throughput degradation is gradual, not cliff-like
            if i > 0:
                prev_metrics = degradation_metrics[i-1]
                throughput_drop = (prev_metrics['actual_rps'] - metrics['actual_rps']) / prev_metrics['actual_rps']
                assert throughput_drop <= 0.5, f"Throughput cliff detected between {load_levels[i-1]} and {load_level} rps"
                
        logger.info("System degradation test completed - system shows graceful degradation under extreme load")