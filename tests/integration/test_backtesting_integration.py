...
class TestBacktestingIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_backtest_single_symbol(self, backtest_config, strategy, mock_db_manager):
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_backtest_multi_symbol(self, backtest_config, strategy, mock_db_manager):
    ...
    @pytest.mark.asyncio
    async def test_backtest_with_risk_manager_integration(self, backtest_config, strategy, mock_db_manager):
    ...
    @pytest.mark.asyncio
    async def test_backtest_commission_slippage_accuracy(self, backtest_config, strategy, mock_db_manager):
        ...
        if result.total_trades > 0:
            ...
            assert float(result.total_return) * float(backtest_config.initial_capital) / 100 <= gross_return
    ...
    async def test_backtest_position_management(self, backtest_config, strategy, mock_db_manager):
        ...
        for trade in result.trades:
            ...
            open_positions = [pos for pos in open_positions if pos["exit_time"] > trade["entry_time"]]
