...
import { connected, disconnect, error, heartbeat, messageReceived } from '@/store/slices/websocketSlice';
...
class WebSocketService {
  ...
  constructor(config: Partial<WebSocketConfig> = {}) {
    this.config = {
      url: process.env.REACT_APP_WS_URL || 'http://localhost:8000',
      autoConnect: false,
      reconnection: true,
  ...
  private setupEventHandlers(): void {
    ...
    this.socket.on('unauthorized', (err) => {
      ...
      store.dispatch(error('Authentication failed'));
      this.disconnect();
    });
