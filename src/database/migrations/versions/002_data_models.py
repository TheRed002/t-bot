...
def upgrade() -> None:
    ...
    op.create_table('market_data_records',
    ...
    op.create_index('idx_market_data_symbol_timestamp', 'market_data_records',
                    ['symbol', 'timestamp'], unique=False)
    op.create_index('idx_market_data_exchange_timestamp', 'market_data_records',
                    ['exchange', 'timestamp'], unique=False)
    op.create_index('idx_market_data_quality', 'market_data_records',
                    ['quality_score'], unique=False)
    op.create_index('idx_market_data_validation', 'market_data_records',
                    ['validation_status'], unique=False)
    op.create_unique_constraint('uq_market_data_unique', 'market_data_records', [
    ...
    op.create_index('idx_feature_symbol_type', 'feature_records',
                    ['symbol', 'feature_type'], unique=False)
    op.create_index('idx_feature_timestamp', 'feature_records',
                    ['calculation_timestamp'], unique=False)
    op.create_index('idx_feature_name', 'feature_records', ['feature_name'], unique=False)
    op.create_unique_constraint('uq_feature_unique', 'feature_records', [
    ...
    op.create_table('data_quality_records',
                    ...
                    sa.Column('symbol', sa.String(length=20), nullable=False),
                    sa.Column('data_source', sa.String(length=100), nullable=False),
                    sa.Column('quality_check_timestamp', sa.DateTime(
    ...
    op.create_index('idx_quality_symbol_timestamp', 'data_quality_records',
                    ['symbol', 'quality_check_timestamp'], unique=False)
    op.create_index('idx_quality_source_timestamp', 'data_quality_records', [
                    'data_source', 'quality_check_timestamp'], unique=False)
    op.create_index('idx_quality_overall_score', 'data_quality_records',
                    ['overall_score'], unique=False)
    op.create_index('idx_quality_check_type', 'data_quality_records', ['check_type'], unique=False)

    # Create data_pipeline_records table
    op.create_table('data_pipeline_records',
                    sa.Column('id', sa.String(), nullable=False),
                    ...
                    sa.Column('error_messages', postgresql.JSONB(
                        astext_type=sa.Text()), nullable=True),
    ...
    op.create_index('idx_pipeline_name_timestamp', 'data_pipeline_records',
                    ['pipeline_name', 'execution_timestamp'], unique=False)
    ...
    op.create_index('idx_pipeline_execution_id', 'data_pipeline_records',
                    ['execution_id'], unique=False)
