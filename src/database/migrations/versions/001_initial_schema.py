...
def upgrade() -> None:
    ...
    op.create_check_constraint('valid_bot_status', 'bot_instances', "status IN ('stopped', 'running', 'paused', 'error')")
    ...
    op.create_check_constraint('valid_order_type', 'trades', "order_type IN ('market', 'limit', 'stop_loss', 'take_profit')")
    op.create_check_constraint('valid_trade_status', 'trades', "status IN ('pending', 'filled', 'cancelled', 'rejected')")
