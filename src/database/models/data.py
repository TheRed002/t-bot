...
class FeatureRecord(Base, TimestampMixin):
    ...
    market_data_id = Column(
        UUID(as_uuid=True), ForeignKey("market_data_records.id", ondelete="CASCADE")
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_feature_market_data_symbol", "market_data_id", "symbol"
        ),  # Market data relationship optimization
        Index(
            "idx_feature_symbol_timestamp", "symbol", "calculation_timestamp"
        ),  # Time-series queries
        Index(
            "idx_feature_type_confidence", "feature_type", "confidence_score"
        ...
        Index("idx_feature_lookback_period", "lookback_period"),  # Lookback period queries
        CheckConstraint(
            "confidence_score >= 0 AND confidence_score <= 1", name="check_feature_confidence_range"
        ),
        CheckConstraint(
            "lookback_period IS NULL OR lookback_period > 0", name="check_lookback_period_positive"
        ),
        CheckConstraint(
            "source_data_end IS NULL OR source_data_start IS NULL OR "
            "source_data_end >= source_data_start",
        ...
        CheckConstraint(
            "symbol IS NOT NULL AND symbol != ''", name="check_feature_symbol_not_empty"
        ),
        UniqueConstraint(
            "feature_name",
            "symbol",
...
class DataQualityRecord(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_data_quality_source_symbol", "data_source", "symbol"
        ),  # Performance optimization
        Index("idx_data_quality_period", "data_period_start", "data_period_end"),  # Period queries
        Index(
            "idx_data_quality_market_data_score", "market_data_id", "overall_score"
        ),  # Quality filtering
        Index(
            "idx_data_quality_symbol_timestamp", "symbol", "quality_check_timestamp"
        ),  # Symbol quality history
        Index(
            "idx_data_quality_source_timestamp", "data_source", "quality_check_timestamp"
        ),  # Source monitoring
        Index(
            "idx_data_quality_score_timestamp", "overall_score", "quality_check_timestamp"
        ),  # Quality trends
        CheckConstraint(
        ...
        CheckConstraint(
            "accuracy_score >= 0 AND accuracy_score <= 1", name="check_accuracy_score_range"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "timeliness_score >= 0 AND timeliness_score <= 1", name="check_timeliness_score_range"
        ),
        CheckConstraint(
            "overall_score >= 0 AND overall_score <= 1", name="check_overall_score_range"
        ),
        ...
        CheckConstraint(
            "data_period_end IS NULL OR data_period_start IS NULL OR "
            "data_period_end >= data_period_start",
        ...
        CheckConstraint(
            "symbol IS NOT NULL AND symbol != ''", name="check_quality_symbol_not_empty"
        ),
        CheckConstraint(
            "data_source IS NOT NULL AND data_source != ''", name="check_quality_source_not_empty"
        ),
        UniqueConstraint(
            "symbol",
            "data_source",
...
class DataPipelineRecord(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_pipeline_name_version", "pipeline_name", "pipeline_version"
        ),  # Version management
        Index("idx_pipeline_execution_status", "execution_id", "status"),  # Execution tracking
        Index(
            "idx_pipeline_name_timestamp", "pipeline_name", "execution_timestamp"
        ),  # Pipeline history
        ...
        CheckConstraint(
            "pipeline_name IS NOT NULL AND pipeline_name != ''",
            name="check_pipeline_name_not_empty",
        ),
        CheckConstraint(
            "execution_id IS NOT NULL AND execution_id != ''", name="check_execution_id_not_empty"
        ),
        UniqueConstraint(
            "execution_id", "pipeline_name", "execution_timestamp", name="uq_pipeline_execution"
        ),
    )
