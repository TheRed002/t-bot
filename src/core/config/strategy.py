...
class StrategyConfig(BaseConfig):
    ...
    active_strategies: list[str] = Field(
        default_factory=list, description="List of active strategy IDs"
    )
    default_strategy: str = Field(default="market_making", description="Default strategy type")
    backtest_enabled: bool = Field(default=False, description="Enable backtesting mode")
    paper_trading_enabled: bool = Field(default=False, description="Enable paper trading mode")

    # Common strategy parameters
    default_timeframe: str = Field(
        default="1h", description="Default timeframe for strategies", alias="STRATEGY_TIMEFRAME"
    )
    default_lookback_period: int = Field(
        default=100, ge=10, le=1000, description="Default lookback period for indicators"
    )

    # Mean Reversion parameters
    mean_reversion_window: int = Field(
        default=20, ge=5, le=200, description="Window size for mean calculation"
    )
    mean_reversion_std_dev: float = Field(
        default=2.0, ge=0.5, le=5.0, description="Number of standard deviations for entry/exit"
    )
    mean_reversion_entry_threshold: float = Field(
        default=0.95, ge=0.0, le=1.0, description="Entry threshold for mean reversion"
    )
    mean_reversion_exit_threshold: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Exit threshold for mean reversion"
    )

    # Momentum parameters
    momentum_period: int = Field(
        default=14, ge=5, le=100, description="Period for momentum calculation"
    )
    momentum_threshold: float = Field(
        default=0.02, ge=0.0, le=0.1, description="Momentum threshold for signals"
    )
    momentum_acceleration_factor: float = Field(
        default=0.02, ge=0.01, le=0.2, description="Acceleration factor for momentum"
    )

    # Arbitrage parameters
    arbitrage_min_spread: float = Field(
        default=0.002, ge=0.0001, le=0.1, description="Minimum spread for arbitrage opportunities"
    )
    arbitrage_execution_time: int = Field(
        default=100, ge=10, le=1000, description="Max execution time in milliseconds"
    )

    # Market Making parameters
    market_making_spread: float = Field(
        default=0.001, ge=0.0001, le=0.1, description="Spread for market making orders"
    )
    market_making_inventory_target: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Target inventory level (0.5 = balanced)"
    )
    market_making_order_levels: int = Field(
        default=3, ge=1, le=10, description="Number of order levels on each side"
    )

    # Trend Following parameters
    trend_ema_fast: int = Field(
        default=12, ge=5, le=50, description="Fast EMA period for trend following"
    )
    trend_ema_slow: int = Field(
        default=26, ge=20, le=200, description="Slow EMA period for trend following"
    )
    trend_macd_signal: int = Field(default=9, ge=5, le=20, description="MACD signal line period")

    # Signal generation
    signal_min_strength: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Minimum signal strength to act on"
    )
    signal_combination_method: str = Field(
        default="weighted_average", description="Method for combining multiple signals"
    )
    ...
    backtest_end_date: str | None = Field(
    ...
    backtest_commission: float = Field(
        default=0.001, ge=0.0, le=0.01, description="Commission rate for backtesting"
    )
    backtest_slippage: float = Field(
        default=0.0005, ge=0.0, le=0.01, description="Slippage rate for backtesting"
    )

    # Performance tracking
    track_metrics: bool = Field(default=True, description="Enable performance metrics tracking")
    metrics_update_interval: int = Field(
        default=300, ge=60, le=3600, description="Metrics update interval in seconds"
    )
    ...
    def get_strategy_params(self, strategy_type: str) -> dict[str, Any]:
        ...
        if strategy_type == "mean_reversion":
            ...
            params.update(
                {
                    "min_profit_threshold": self.arbitrage_min_spread,
                    "max_exposure": 10000,
            ...
            params.update(
                {
                    "bid_spread": self.market_making_spread,
                    "ask_spread": self.market_making_spread,
