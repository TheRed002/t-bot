...
class RiskConfig(BaseConfig):
    ...
    max_total_positions: int = Field(
        ...
        ge=1,
        le=100,
        description="Maximum total positions across all symbols"
    )
    ...
    max_monthly_loss: Decimal = Field(
        ...
        description="Maximum monthly loss allowed"
    )
    ...
    enable_circuit_breakers: bool = Field(
        ...
        alias="RISK_CIRCUIT_BREAKERS"
    )
    loss_limit_circuit_breaker: float = Field(
        default=0.05,
        ge=0.01,
        le=0.2,
        description="Loss limit to trigger circuit breaker"
    )
    volatility_circuit_breaker: float = Field(
        default=0.1,
        ge=0.01,
        le=0.5,
        description="Volatility threshold for circuit breaker"
    )
    ...
    correlation_window: int = Field(
        ...
        ge=10,
        le=100,
        description="Correlation calculation window in days"
    )
    ...
    kelly_win_probability: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Estimated win probability for Kelly"
    )
    kelly_win_loss_ratio: Optional[float] = Field(
        default=None,
        ge=0.1,
        le=10.0,
        description="Win/loss ratio for Kelly"
    )
    ...
    risk_parity_target_volatility: float = Field(
        ...
        ge=0.01,
        le=0.5,
        description="Target volatility for risk parity"
    )
    risk_parity_rebalance_threshold: float = Field(
        default=0.1,
        ge=0.01,
        le=0.5,
        description="Rebalance threshold for risk parity"
    )
    ...
    min_portfolio_diversification: int = Field(
        ...
        ge=1,
        le=20,
        description="Minimum number of assets in portfolio"
    )
    
    # Capital protection
    capital_preservation_mode: bool = Field(
        default=False,
        description="Enable capital preservation mode"
    )
    ...
    emergency_stop_threshold: Decimal = Field(
        ...
        description="Emergency stop trading threshold"
    )
    ...
    def validate_sizing_method(cls, v: str) -> str:
        ...
        valid_methods = ["fixed", "kelly_criterion", "risk_parity", "volatility_adjusted", "equal_weight"]
    ...
    def is_risk_exceeded(self, current_loss: Decimal) -> bool:
        ...
        if current_loss >= self.max_daily_loss:
            return True
        if self.capital_preservation_mode and current_loss >= self.min_capital_threshold * Decimal("0.1"):
            return True
        return False