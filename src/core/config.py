...
class SecurityConfig(BaseConfig):
    ...
    secret_key: str = Field(
        default="test-secret-key-32-chars-long-for-testing",
    ...
    encryption_key: str = Field(
        default="test-encryption-key-32-chars-long-for-testing",
...
class ErrorHandlingConfig(BaseConfig):
    ...
    @field_validator('circuit_breaker_failure_threshold',
                     'max_retry_attempts', 'order_rejection_max_retries')
    @classmethod
...
class ExchangeConfig(BaseConfig):
    ...
    default_timeout: int = Field(default=30,
                                 description="Default exchange API timeout")
    max_retries: int = Field(
        default=3,
        description="Maximum retry attempts for exchange API calls")
...
class RiskConfig(BaseConfig):
    ...
    @field_validator('default_position_size_pct',
    ...
    @field_validator('max_total_positions',
                     'kelly_lookback_days',
                     'volatility_window',
...
class CapitalManagementConfig(BaseConfig):
    ...
    base_currency: str = Field(
        default="USDT",
        pattern=r"^[A-Z]{3,10}$",
        description="Base currency for all calculations")
    total_capital: float = Field(
        default=10000.0,
        gt=0,
        description="Total available capital")
    ...
    hedge_ratio: float = Field(default=0.8,
                               description="Hedge coverage ratio (80%)")
    ...
    profit_lock_pct: float = Field(default=0.5,
                                   description="Profit lock percentage (50%)")
...
class StrategyManagementConfig(BaseConfig):
    ...
    min_win_rate: float = Field(default=0.4,
    ...
    @field_validator('max_concurrent_strategies',
    ...
    @field_validator('default_min_confidence',
...
class Config(BaseConfig):
    ...
    @classmethod
    def from_yaml_with_env_override(
            cls, yaml_path: Union[str, Path]) -> 'Config':
