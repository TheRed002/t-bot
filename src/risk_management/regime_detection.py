...
class MarketRegimeDetector:
    ...
    async def detect_volatility_regime(self, symbol: str, price_data: List[float]) -> MarketRegime:
        ...
        try:
            if len(price_data) < self.volatility_window:
                logger.warning("Insufficient data for volatility regime detection", 
                             symbol=symbol, data_points=len(price_data))
            ...
            logger.debug("Volatility regime detected", 
                        symbol=symbol, volatility=volatility, regime=regime.value)
            ...
            raise RiskManagementError(f"Volatility regime detection failed: {str(e)}")
    ...
    async def detect_trend_regime(self, symbol: str, price_data: List[float]) -> MarketRegime:
        ...
        try:
            if len(price_data) < self.trend_window:
                logger.warning("Insufficient data for trend regime detection", 
                             symbol=symbol, data_points=len(price_data))
            ...
            r_squared = 1 - np.sum((price_data - y_pred) ** 2) / np.sum((price_data - np.mean(price_data)) ** 2)
            ...
            logger.debug("Trend regime detected", 
                        symbol=symbol, slope=slope, r_squared=r_squared, regime=regime.value)
            ...
            logger.error("Error detecting trend regime", 
                        symbol=symbol, error=str(e))
            raise RiskManagementError(f"Trend regime detection failed: {str(e)}")
    
    @time_execution
    async def detect_correlation_regime(self, symbols: List[str], 
                                      price_data_dict: Dict[str, List[float]]) -> MarketRegime:
        """
        ...
        try:
            ...
            logger.debug("Correlation regime detected", 
                        avg_correlation=avg_correlation, regime=regime.value)
            
            return regime
            
        except Exception as e:
            logger.error("Error detecting correlation regime", error=str(e))
            raise RiskManagementError(f"Correlation regime detection failed: {str(e)}")
    ...
    async def detect_comprehensive_regime(self, market_data: List[MarketData]) -> MarketRegime:
        ...
        try:
            ...
            raise RiskManagementError(f"Comprehensive regime detection failed: {str(e)}")
    
    def _combine_regimes(self, volatility_regimes: List[MarketRegime], 
                        trend_regimes: List[MarketRegime], 
                        correlation_regime: MarketRegime) -> MarketRegime:
        """
        ...
        dominant_vol = max(vol_counts.items(), key=lambda x: x[1])[0] if vol_counts else MarketRegime.MEDIUM_VOLATILITY
        
        # Determine dominant trend regime
        dominant_trend = max(trend_counts.items(), key=lambda x: x[1])[0] if trend_counts else MarketRegime.RANGING
    ...
    def get_regime_statistics(self) -> Dict[str, Any]:
        ...
        regime_duration = (datetime.now() - self.regime_history[-1].timestamp).total_seconds() / 3600
