...
class BaseCircuitBreaker(ABC):
    ...
    async def evaluate(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error("Circuit breaker evaluation failed",
                            error=str(e), breaker_type=self.__class__.__name__)
    ...
    async def _trigger_circuit_breaker(self, data: Dict[str, Any]) -> None:
        ...
        if self.state == CircuitBreakerState.CLOSED:
            ...
            # Use the breaker_type class attribute defined in each circuit breaker class
            breaker_type = getattr(self.__class__, 'breaker_type', CircuitBreakerType.MANUAL_TRIGGER)
