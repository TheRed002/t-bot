...
class PositionSizer:
        ...
        try:
            ...
            if method is None:
                method = PositionSizeMethod(
                    self.risk_config.default_position_size_method)
            ...
            if method == PositionSizeMethod.FIXED_PCT:
                ...
                raise ValidationError(
                    f"Unsupported position sizing method: {method}")
            ...
            min_position_size = portfolio_value * \
                Decimal("0.001")  # 0.1% minimum
    ...
    async def _fixed_percentage_sizing(self, signal: Signal,
        ...
        base_size = portfolio_value * \
            Decimal(str(self.risk_config.default_position_size_pct))
    ...
    async def _kelly_criterion_sizing(self, signal: Signal,
        ...
        try:
            ...
            returns_array = np.array(
                returns[-self.risk_config.kelly_lookback_days:])
            mean_return = np.mean(returns_array)
            variance = np.var(returns_array)

            if variance == 0:
                self.logger.warning(
                    "Zero variance in returns, using fixed percentage")
                return await self._fixed_percentage_sizing(signal, portfolio_value)
    ...
    async def _volatility_adjusted_sizing(self, signal: Signal,
        ...
        try:
            ...
            prices_array = np.array(
                prices[-self.risk_config.volatility_window:])
            returns = np.diff(prices_array) / prices_array[:-1]
            volatility = np.std(returns)

            # Calculate volatility adjustment factor
            target_volatility = self.risk_config.volatility_target
            volatility_adjustment = target_volatility / \
                max(volatility, 0.001)  # Avoid division by zero
            ...
            position_size = base_size * \
                Decimal(str(volatility_adjustment)) * Decimal(str(signal.confidence))
    ...
    async def get_position_size_summary(
            self, signal: Signal, portfolio_value: Decimal) -> Dict[str, Any]:
        """
        ...
        for method in PositionSizeMethod:
            try:
                ...
                summary[method.value] = {"position_size": float(size), "portfolio_percentage": float(
                    size / portfolio_value) if portfolio_value > 0 else 0}
            except Exception as e:
