...
from src.core.exceptions import AnalyticsError, ValidationError
...
class EnvironmentAwareAnalyticsManager(EnvironmentAwareServiceMixin):
    ...
    async def generate_environment_aware_report(
        ...
        try:
            ...
            logger.info(f"Generated {report_type} report for {exchange} (environment: {context.environment.value})")
            return report
    ...
    async def _generate_performance_report(
        ...
        if reporting_level in (ReportingLevel.DETAILED, ReportingLevel.COMPREHENSIVE):
            report['detailed_metrics'] = await self._calculate_detailed_performance_metrics(exchange)
            
            if context.is_production:
                # Production-specific performance metrics
                report['production_metrics'] = await self._calculate_production_performance_metrics(exchange)
            else:
                # Sandbox-specific performance metrics
                report['sandbox_metrics'] = await self._calculate_sandbox_performance_metrics(exchange)
        
        if reporting_level == ReportingLevel.COMPREHENSIVE:
            report['advanced_analytics'] = await self._calculate_advanced_performance_analytics(exchange)
    ...
    async def _generate_risk_report(
        ...
        if context.is_production:
            report['production_risk_metrics'] = await self._calculate_production_risk_metrics(exchange)
        else:
            report['sandbox_risk_metrics'] = await self._calculate_sandbox_risk_metrics(exchange)
        
        if reporting_level in (ReportingLevel.DETAILED, ReportingLevel.COMPREHENSIVE):
            report['detailed_risk_analysis'] = await self._calculate_detailed_risk_analysis(exchange)
    ...
    async def _generate_execution_report(
        ...
        if analytics_config.get('enable_market_impact_analysis'):
            report['market_impact_analysis'] = await self._calculate_market_impact_analysis(exchange)
    ...
    async def _generate_portfolio_report(
        ...
        if analytics_config.get('enable_performance_attribution'):
            report['performance_attribution'] = await self._calculate_performance_attribution(exchange)
    ...
    async def _update_analytics_metrics(
        ...
        if success:
            ...
            metrics['average_computation_time_ms'] = (
                (prev_avg * (successful_runs - 1) + computation_time) / successful_runs
            )
            ...
            if was_cached:
                ...
                metrics['cache_hit_rate'] = Decimal(cache_hits) / Decimal(metrics['total_analytics_runs'])
        else:
        ...
        metrics['error_rate'] = Decimal(failed_runs) / Decimal(total_runs) * 100 if total_runs > 0 else Decimal('0')
    ...
    def get_environment_analytics_metrics(self, exchange: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'environment': context.environment.value,
            'is_production': context.is_production,
            'analytics_mode': analytics_config.get('analytics_mode', AnalyticsMode.PRODUCTION).value,
            'reporting_level': analytics_config.get('reporting_level', ReportingLevel.STANDARD).value,
            ...
            'success_rate': (
                metrics.get('successful_runs', 0) / max(metrics.get('total_analytics_runs', 1), 1) * 100
            ),
            ...
            'enable_experimental_metrics': analytics_config.get('enable_experimental_metrics', False),
