...
class AdaptiveMomentumStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.momentum_lookback = self.config.parameters.get(
            'momentum_lookback', 10)
        self.volume_threshold = self.config.parameters.get(
    ...
    def set_regime_detector(
        ...
        logger.info(
            "Regime detector set for adaptive momentum strategy",
            strategy=self.name)
    ...
    def set_adaptive_risk_manager(
        ...
        logger.info(
            "Adaptive risk manager set for adaptive momentum strategy",
            strategy=self.name)
    ...
    async def _calculate_momentum_score(self, symbol: str) -> float:
        ...
        try:
            ...
            price_momentum = (
                prices[-1] - prices[-self.momentum_lookback]) / prices[-self.momentum_lookback]
    ...
    async def _calculate_volume_score(self, symbol: str) -> float:
        ...
        try:
            if symbol not in self.volume_history or len(
                    self.volume_history[symbol]) < 20:
                return 0.0
    ...
    async def _calculate_rsi_score(self, symbol: str) -> float:
        ...
        try:
            if symbol not in self.price_history or len(
                    self.price_history[symbol]) < self.rsi_period + 1:
                return 0.0
