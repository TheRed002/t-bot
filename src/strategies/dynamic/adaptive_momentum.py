...
from src.utils.helpers import calculate_volatility, calculate_zscore
from src.utils.validators import validate_price, validate_quantity, validate_signal
...
class AdaptiveMomentumStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.info(
            "Adaptive momentum strategy initialized",
            strategy=self.name,
    ...
    async def _generate_signals_impl(self, data: MarketData) -> list[Signal]:
        ...
        try:
            ...
            # Apply regime-aware confidence adjustments using existing adaptive
            # risk manager
            signals = await self._apply_regime_confidence_adjustments(signals, current_regime)

            logger.info(
                "Generated adaptive momentum signals",
                strategy=self.name,
                symbol=data.symbol,
            ...
            logger.error(
                "Adaptive momentum signal generation failed",
                strategy=self.name,
                symbol=symbol,
                error=str(e),
            )
    ...
    async def _update_price_history(self, data: MarketData) -> None:
        ...
        if validate_quantity(data.volume, data.symbol):
            ...
            logger.warning(
                "Invalid volume data, using default",
                strategy=self.name,
                symbol=symbol,
                volume=data.volume,
            )
    ...
    async def _calculate_momentum_score(self, symbol: str) -> float:
        ...
        try:
            if (
                ...
                or len(self.price_history[symbol]) < self.slow_ma_period
            ):
                return 0.0
            ...
            price_momentum = (prices[-1] - prices[-self.momentum_lookback]) / prices[
            ...
            logger.error(
                "Momentum score calculation failed", strategy=self.name, symbol=symbol, error=str(e)
            )
            return 0.0
    ...
    async def _calculate_volume_score(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.error(
                "Volume score calculation failed", strategy=self.name, symbol=symbol, error=str(e)
            )
            return 0.0
    ...
    async def _calculate_rsi_score(self, symbol: str) -> float:
        ...
        try:
            if (
                symbol not in self.price_history
                or len(self.price_history[symbol]) < self.rsi_period + 1
            ):
                return 0.0
            ...
            logger.error(
                "RSI score calculation failed", strategy=self.name, symbol=symbol, error=str(e)
            )
            return 0.0
    ...
    async def _apply_regime_confidence_adjustments(
        ...
        try:
            ...
            for signal in signals:
                ...
                # Higher confidence in favorable regimes, lower in unfavorable
                # ones
            ...
            logger.info(
                "Applied regime confidence adjustments",
                strategy=self.name,
            ...
            logger.error("Regime confidence adjustment failed", strategy=self.name, error=str(e))
    ...
    async def validate_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            if signal.symbol not in self.momentum_scores:
                logger.warning(
                    "No momentum data for signal validation",
                    strategy=self.name,
            ...
            logger.info(
                "Adaptive momentum signal validated",
                strategy=self.name,
                symbol=signal.symbol,
        ...
        try:
            ...
            if position.symbol in self.momentum_scores:
                ...
                if (position.side.value == "buy" and current_momentum < -0.2) or (
                    ...
                    logger.info(
                        "Momentum reversal exit triggered",
                        strategy=self.name,
