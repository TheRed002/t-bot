...
from src.utils.validators import validate_price, validate_signal
...
class VolatilityBreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.info(
            "Volatility breakout strategy initialized",
            strategy=self.name,
    ...
    def set_adaptive_risk_manager(self, adaptive_risk_manager: AdaptiveRiskManager) -> None:
        ...
        logger.info(
            "Adaptive risk manager set for volatility breakout strategy", strategy=self.name
        )
    ...
    async def _generate_signals_impl(self, data: MarketData) -> list[Signal]:
        ...
        try:
            ...
            breakout_levels = await self._calculate_breakout_levels(
                data.symbol, atr_value, current_regime
            )
            ...
            logger.info(
                "Generated volatility breakout signals",
                strategy=self.name,
                symbol=data.symbol,
            ...
            logger.error(
                "Volatility breakout signal generation failed",
                strategy=self.name,
                symbol=symbol,
                error=str(e),
            )
    ...
    async def _update_price_history(self, data: MarketData) -> None:
        ...
        if validate_price(data.price, symbol):
            ...
            logger.warning(
                "Invalid price data, skipping update",
                strategy=self.name,
                symbol=symbol,
                price=data.price,
            )
        ...
        if (
            data.high_price
            and data.low_price
            and validate_price(data.high_price, data.symbol)
            and validate_price(data.low_price, data.symbol)
        ):
    ...
    async def _get_current_regime(self, symbol: str) -> MarketRegime:
        ...
        try:
            ...
            logger.warning(
                "Regime detection failed, using default",
                strategy=self.name,
                symbol=symbol,
                error=str(e),
            )
    ...
    async def _calculate_atr(self, symbol: str) -> float:
        ...
        try:
            if (
                symbol not in self.high_low_history
                or len(self.high_low_history[symbol]) < self.atr_period + 1
            ):
                return 0.0
            ...
            logger.error("ATR calculation failed", strategy=self.name, symbol=symbol, error=str(e))
            return 0.0
    ...
    async def _calculate_consolidation_score(self, symbol: str) -> float:
        ...
        try:
            if (
                ...
                or len(self.price_history[symbol]) < self.consolidation_period
            ):
                return 0.0
            ...
            logger.error(
                "Consolidation score calculation failed",
                strategy=self.name,
                symbol=symbol,
                error=str(e),
            )
    ...
    async def _calculate_breakout_levels(
        self, symbol: str, atr_value: float, current_regime: MarketRegime
    ) -> dict[str, float]:
        ...
        try:
            ...
            logger.error(
                "Breakout levels calculation failed",
                strategy=self.name,
                symbol=symbol,
                error=str(e),
            )
    ...
    async def _generate_breakout_signals(
        ...
        try:
            ...
            logger.error(
                "Breakout signal generation failed",
                strategy=self.name,
                symbol=data.symbol,
                error=str(e),
            )
    ...
    async def _apply_regime_filtering(
        self, signals: list[Signal], current_regime: MarketRegime
    ) -> list[Signal]:
        ...
        try:
            ...
            logger.error("Regime filtering failed", strategy=self.name, error=str(e))
    ...
    def _is_signal_valid_for_regime(self, signal: Signal, regime: MarketRegime) -> bool:
        ...
        try:
            ...
            logger.error(
                "Regime validation failed", strategy=self.name, symbol=signal.symbol, error=str(e)
            )
            return False
    ...
    async def _apply_time_decay(self, signals: list[Signal], symbol: str) -> list[Signal]:
        ...
        try:
            ...
            logger.error("Time decay application failed", strategy=self.name, error=str(e))
            return signals  # Return original signals on error
    ...
    async def validate_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            logger.info(
                "Volatility breakout signal validated",
                strategy=self.name,
                symbol=symbol,
            ...
            logger.error(
                "Signal validation failed", strategy=self.name, symbol=signal.symbol, error=str(e)
            )
            return False
    ...
    async def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            if self.adaptive_risk_manager:
                ...
                logger.info(
                    "Used adaptive risk manager for position sizing",
                    strategy=self.name,
                    symbol=signal.symbol,
        ...
        try:
            ...
            if position.symbol in self.consolidation_scores:
                ...
                if current_consolidation < entry_consolidation * 0.5:
                    logger.info(
                        "Consolidation breakdown exit triggered",
                        strategy=self.name,
