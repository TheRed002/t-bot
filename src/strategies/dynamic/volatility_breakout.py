...
# MANDATORY: Import from P-010 - Use existing regime detection and
# adaptive risk
...
class VolatilityBreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.breakout_multiplier = self.config.parameters.get(
            'breakout_multiplier', 2.0)
        self.consolidation_period = self.config.parameters.get(
            'consolidation_period', 20)
        self.volume_confirmation = self.config.parameters.get(
            'volume_confirmation', True)
        self.min_consolidation_ratio = self.config.parameters.get(
            'min_consolidation_ratio', 0.8)
        self.max_consolidation_ratio = self.config.parameters.get(
            'max_consolidation_ratio', 1.2)
        self.time_decay_factor = self.config.parameters.get(
    ...
    def set_regime_detector(
            self,
            regime_detector: MarketRegimeDetector) -> None:
        """Set the regime detector for adaptive parameter adjustment."""
        self.regime_detector = regime_detector
        logger.info(
            "Regime detector set for volatility breakout strategy",
            strategy=self.name)

    def set_adaptive_risk_manager(
            self, adaptive_risk_manager: AdaptiveRiskManager) -> None:
        """Set the adaptive risk manager for dynamic risk adjustment."""
        self.adaptive_risk_manager = adaptive_risk_manager
        logger.info(
            "Adaptive risk manager set for volatility breakout strategy",
            strategy=self.name)
    ...
    async def _generate_signals_impl(self, data: MarketData) -> List[Signal]:
        ...
        try:
            ...
            # Apply regime-specific filtering using existing adaptive risk
            # manager
    ...
    async def _update_price_history(self, data: MarketData) -> None:
        ...
        if data.high_price and data.low_price and validate_price(
                data.high_price, data.symbol) and validate_price(
                data.low_price, data.symbol):
    ...
    async def _calculate_atr(self, symbol: str) -> float:
        ...
        try:
            if symbol not in self.high_low_history or len(
                    self.high_low_history[symbol]) < self.atr_period + 1:
                return 0.0
    ...
    async def _calculate_consolidation_score(self, symbol: str) -> float:
        ...
        try:
            if symbol not in self.price_history or len(
                    self.price_history[symbol]) < self.consolidation_period:
                return 0.0
            ...
            if (self.min_consolidation_ratio <=
                    consolidation_ratio <= self.max_consolidation_ratio):
                ...
                # Adjust for volatility (lower volatility = higher
                # consolidation score)
                if volatility > 0:
    ...
    async def _calculate_breakout_levels(self,
                                         symbol: str,
                                         atr_value: float,
                                         current_regime: MarketRegime) -> Dict[str,
                                                                               float]:
        ...
        try:
            ...
            # Apply regime-specific adjustments using existing adaptive risk
            # manager
            regime_adjustment = self._get_regime_breakout_adjustment(
                current_regime)
    ...
    async def _generate_breakout_signals(self,
                                         data: MarketData,
                                         atr_value: float,
                                         consolidation_score: float,
                                         breakout_levels: Dict[str,
                                                               float],
        ...
        try:
            ...
            for signal in signals:
                ...
                if self._is_signal_valid_for_regime(signal, current_regime):
                    ...
                    regime_confidence_multiplier = self._get_regime_confidence_multiplier(
                        current_regime)
                    signal.confidence = min(
                        signal.confidence * regime_confidence_multiplier, 0.95)
    ...
    def _is_signal_valid_for_regime(
            self,
            signal: Signal,
        ...
        try:
            ...
            consolidation_score = signal.metadata.get(
                'consolidation_score', 0.0)
    ...
    async def _apply_time_decay(
        ...
        try:
            if symbol not in self.last_breakout_time:
                logger.info(
                    "No last breakout time found, returning original signals",
                    strategy=self.name,
                    symbol=symbol)
                return signals
            ...
            time_since_breakout = (
                datetime.now() - last_breakout).total_seconds() / 3600  # Hours

            # Apply time decay factor with minimum decay
            decay_factor = max(self.time_decay_factor **
                               time_since_breakout, 0.8)
    ...
    async def validate_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            if symbol in self.atr_values:
                ...
                if abs(current_atr - signal_atr) / \
                        max(current_atr, 0.001) > 0.2:
            ...
            if symbol in self.consolidation_scores:
                ...
                signal_consolidation = signal.metadata.get(
                    'consolidation_score', 0.0)
    ...
    async def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            if self.adaptive_risk_manager:
                ...
                # This integrates with the comprehensive regime detection and
                # adaptive sizing
    ...
    def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            if position.symbol in self.consolidation_scores:
                ...
                entry_consolidation = position.metadata.get(
                    'entry_consolidation', 0.0)
            ...
            if position.symbol in self.atr_values and len(
                    self.atr_values[position.symbol]) >= 1:
