...
class CorrelationAnalyzer:
    ...
    def add_strategy_return(self, strategy_name: str, return_value: float) -> None:
        ...
        if len(self.strategy_returns[strategy_name]) > self.window_size:
            self.strategy_returns[strategy_name] = self.strategy_returns[strategy_name][-self.window_size:]
...
class VotingMechanism:
    ...
    @staticmethod
    def weighted_vote(signals: list[tuple[str, Signal]], weights: dict[str, float]) -> Signal | None:
    ...
    @staticmethod
    def confidence_weighted_vote(signals: list[tuple[str, Signal]], 
                                performance_scores: dict[str, float]) -> Signal | None:
...
class EnsembleStrategy(BaseStrategy):
    ...
    def remove_strategy(self, strategy_name: str) -> None:
        ...
        try:
            ...
            logger.error("Error removing strategy from ensemble", strategy=strategy_name, error=str(e))
        ...
        try:
            ...
            if ensemble_signal:
                ...
                signal_record = {
                    ...
                    "component_signals": [(name, signal.model_dump()) for name, signal in component_signals],
    ...
    async def _vote_on_signals(self, component_signals: list[tuple[str, Signal]], 
                              regime: Any) -> Signal | None:
        ...
        try:
            ...
            if self.voting_method == "majority":
                ...
                return self.voting_mechanism.confidence_weighted_vote(filtered_signals, performance_scores)
            else:
                logger.warning("Unknown voting method, using confidence_weighted", method=self.voting_method)
                performance_scores = {
                    name: tracker.get_performance_score() 
                    for name, tracker in self.performance_trackers.items()
                }
    ...
    async def _filter_signals_by_regime(self, signals: list[tuple[str, Signal]], 
                                       regime: Any) -> list[tuple[str, Signal]]:
    ...
    async def _check_and_rebalance(self) -> None:
        ...
        try:
            ...
            if (self.last_rebalance is None or 
                current_time - self.last_rebalance > timedelta(hours=self.rebalance_frequency)):
    ...
    def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            final_size = base_size * confidence_multiplier * participation_multiplier * diversity_multiplier
    ...
    def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            for strategy_name, strategy in self.component_strategies.items():
                try:
                    signals = asyncio.create_task(strategy.generate_signals(data))
                    ...
                    if hasattr(strategy, 'last_signal_direction'):
                        if (position.side.value == "buy" and 
                            getattr(strategy, 'last_signal_direction') == SignalDirection.SELL):
                            opposing_signals += 1
                        elif (position.side.value == "sell" and 
                              getattr(strategy, 'last_signal_direction') == SignalDirection.BUY):
                            opposing_signals += 1
                except:
                    continue
    ...
    async def _on_start(self) -> None:
        ...
        for strategy_name, strategy in self.component_strategies.items():
            try:
                ...
                logger.error("Failed to start component strategy", strategy=strategy_name, error=str(e))
    ...
    async def _on_stop(self) -> None:
        ...
        for strategy_name, strategy in self.component_strategies.items():
            try:
                ...
                logger.error("Failed to stop component strategy", strategy=strategy_name, error=str(e))

    def update_strategy_performance(self, strategy_name: str, return_pct: float, 
        ...
        try:
            ...
            logger.error("Error updating strategy performance", strategy=strategy_name, error=str(e))
