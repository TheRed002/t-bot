...
class FailureDetector:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.max_volatility_threshold = config.get("max_volatility_threshold", 0.5)  # 50% annualized
    ...
    def add_timeout(self, duration: float, timestamp: datetime) -> None:
        ...
        self.recent_timeouts = [
            timeout for timeout in self.recent_timeouts 
            if timeout["timestamp"] > cutoff_time
        ]
...
class SafeModeStrategy:
    ...
    async def generate_signal(self, data: MarketData, 
                            price_history: list[float]) -> Signal | None:
...
class FallbackStrategy(BaseStrategy):
    ...
    async def _switch_mode(self, new_mode: FallbackMode, failure_info: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.warning(
                ...
                from_mode=old_mode.value,
                to_mode=new_mode.value,
                failure_type=failure_info.get("failure_type", {}).value if failure_info.get("failure_type") else "unknown",
                failure_count=self.failure_count
            )
        ...
        try:
            ...
            if self.current_mode == FallbackMode.RECOVERY:
                if current_time - self.last_mode_change > self.recovery_test_duration:
                    ...
                    if recovery_success:
                        await self._switch_mode(FallbackMode.PRIMARY, {"reason": "recovery_successful"})
                        self.failure_count = 0  # Reset failure count
                        self.recovery_attempts = 0
                    else:
                        # Recovery failed, go back to fallback mode
                        fallback_mode = FallbackMode.DEGRADED if self.failure_count < 2 else FallbackMode.SAFE_MODE
    ...
    async def _evaluate_recovery_performance(self) -> bool:
        ...
        try:
            ...
            recent_trades = [
                trade for trade in self.failure_detector.recent_trades
            ...
            if (win_rate >= self.recovery_performance_threshold and 
                total_return > 0 and
                len(recent_trades) >= 3):
                return True
    ...
    async def _generate_signal_for_current_mode(self, data: MarketData) -> Signal | None:
        ...
        try:
            ...
            logger.error("Error generating signal for current mode", mode=self.current_mode.value, error=str(e))
            # Record error and potentially switch to safe mode
            self.failure_detector.add_error("signal_generation", datetime.now())
            if self.current_mode != FallbackMode.SAFE_MODE:
                await self._switch_mode(FallbackMode.SAFE_MODE, {
                    "failure_type": FailureType.TECHNICAL_ERROR,
    ...
    def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            failure_adjustment = max(Decimal("0.5"), Decimal("1.0") - Decimal(str(self.failure_count * 0.1)))
    ...
    def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            if pnl_pct <= -stop_loss:
                logger.info("Stop loss triggered", symbol=position.symbol, mode=self.current_mode.value)
                return True
                
            if pnl_pct >= take_profit:
                logger.info("Take profit triggered", symbol=position.symbol, mode=self.current_mode.value)
                return True
