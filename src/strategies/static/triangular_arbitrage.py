...
class TriangularArbitrageStrategy(BaseStrategy):
    ...
    def __init__(self, config: dict):
        ...
        self.exchange = config.get("exchange", "binance")  # Single exchange for triangular
            ...
            if net_profit_percentage >= float(self.min_profit_threshold * 100):
                ...
                if await self._validate_triangular_timing(path):
                    ...
                    signal = Signal(
                        ...
                        confidence=min(0.9, net_profit_percentage / 2),  # Scale confidence with profit
    ...
    def _calculate_triangular_fees(self, rate1: Decimal, rate2: Decimal, rate3: Decimal) -> Decimal:
        ...
        try:
            ...
            slippage_cost1 = round_to_precision_decimal(rate1 * self.slippage_limit, PRECISION_LEVELS["price"])
            slippage_cost2 = round_to_precision_decimal(rate2 * self.slippage_limit, PRECISION_LEVELS["price"])
            slippage_cost3 = round_to_precision_decimal(rate3 * self.slippage_limit, PRECISION_LEVELS["price"])
    ...
    def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            risk_per_trade = self.config.parameters.get("risk_per_trade", 0.02) * 0.7  # Lower risk for triangular
            ...
            triangular_multiplier = min(Decimal("1.5"), profit_potential * Decimal("8"))  # More conservative
            ...
            # Apply minimum position size from constants (smaller for triangular)
            ...
            raise ArbitrageError(f"Triangular position size calculation failed: {str(e)}")
    ...
    async def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            if "arbitrage_type" not in position.metadata or position.metadata.get("arbitrage_type") != "triangular":
