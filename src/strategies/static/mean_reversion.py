...
class MeanReversionStrategy(BaseStrategy):
    ...
    async def validate_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            if not isinstance(z_score, (int, float)):
                logger.warning("Invalid z_score type", strategy=self.name, z_score_type=type(z_score))
                return False
    ...
    def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            # Adjust based on Z-score magnitude (stronger deviation = larger position)
            metadata = signal.metadata
            z_score = abs(metadata.get("z_score", 0))
            z_score_factor = min(z_score / self.entry_threshold, 2.0)  # Cap at 2x
            
            # Calculate final position size
            position_size = base_size * Decimal(str(confidence_factor)) * Decimal(str(z_score_factor))
    ...
    def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        """
        try:
            # Update price history with current data for accurate Z-score calculation
            self._update_price_history(data)
            ...
            if len(self.high_history) >= self.atr_period and len(self.low_history) >= self.atr_period and len(self.price_history) >= self.atr_period:
                try:
                    # Use the helper function which handles ta-lib with fallback
                    # Use more data points for ATR calculation (need at least period + 1)
                    ...
                    # TODO: Remove in production - Debug logging
                    logger.debug(f"ATR calculation result: {atr}, high_history_len: {len(self.high_history)}, low_history_len: {len(self.low_history)}, price_history_len: {len(self.price_history)}")
