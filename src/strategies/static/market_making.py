...
class MarketMakingStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.order_size_distribution = self.config.parameters.get("order_size_distribution", "exponential")
        ...
        # Risk parameters
        self.max_position_value = Decimal(str(self.config.parameters.get("max_position_value", 10000)))
        self.stop_loss_inventory = Decimal(str(self.config.parameters.get("stop_loss_inventory", 2.0)))
        self.daily_loss_limit = Decimal(str(self.config.parameters.get("daily_loss_limit", 100)))
        self.min_profit_per_trade = Decimal(str(self.config.parameters.get("min_profit_per_trade", 0.00001)))
    ...
    async def _generate_signals_impl(self, data: MarketData) -> List[Signal]:
        ...
        try:
            ...
            for level in range(1, self.order_levels + 1):
                ...
                level_spread = self._calculate_level_spread(level, current_spread, current_volatility)
    ...
    def _calculate_level_spread(self, level: int, base_spread: Decimal, volatility: float) -> Decimal:
        ...
        if self.inventory_skew_enabled:
            inventory_adjustment = self.inventory_state.inventory_skew * 0.001
        ...
        try:
            ...
            if not signal or signal.confidence < self.config.min_confidence:
                logger.warning("Signal confidence too low", 
                             strategy=self.name, confidence=signal.confidence)
                return False
            ...
            if self.daily_pnl < -self.daily_loss_limit:
                logger.warning("Daily loss limit reached", 
                             strategy=self.name, daily_pnl=float(self.daily_pnl))
                return False
    ...
    async def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            if abs(position.quantity) > self.stop_loss_inventory:
                logger.info("Position exceeds stop loss inventory", 
                           strategy=self.name, quantity=float(position.quantity))
                return True
            ...
            if position.unrealized_pnl > self.min_profit_per_trade:
                ...
                if current_spread < self.base_spread * Decimal("0.5"):
                    logger.info("Closing position due to narrow spread", 
                               strategy=self.name, spread=float(current_spread))
                    return True
    ...
    async def _should_rebalance_inventory(self, position: Position) -> bool:
        ...
        target_diff = abs(self.inventory_state.current_inventory - self.inventory_state.target_inventory)
        threshold = self.inventory_state.max_inventory * Decimal(str(self.rebalance_threshold))
        
        logger.debug("Checking rebalancing conditions", 
                    strategy=self.name,
