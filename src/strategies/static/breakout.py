...
class BreakoutStrategy(BaseStrategy):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.false_breakout_threshold = self.config.parameters.get("false_breakout_threshold", 0.01)  # 1%
    ...
    def _check_resistance_breakout(self, data: MarketData) -> Optional[Dict[str, Any]]:
    ...
    def _check_support_breakout(self, data: MarketData) -> Optional[Dict[str, Any]]:
    ...
    def _check_false_breakout(self, data: MarketData) -> Optional[Dict[str, Any]]:
    ...
    async def _generate_bullish_breakout_signal(self, data: MarketData, breakout_info: Dict[str, Any]) -> Optional[Signal]:
        ...
        try:
            ...
            logger.error("Bullish breakout signal generation failed", strategy=self.name, error=str(e))
            return None
    
    async def _generate_bearish_breakout_signal(self, data: MarketData, breakout_info: Dict[str, Any]) -> Optional[Signal]:
        ...
        try:
            ...
            # Calculate target price
            range_size = max(self.resistance_levels) - level if self.resistance_levels else level * 0.1
            target_price = breakout_price - (range_size * self.target_multiplier)
            ...
            logger.error("Bearish breakout signal generation failed", strategy=self.name, error=str(e))
            return None
    
    async def _generate_false_breakout_exit_signal(self, data: MarketData, false_breakout_info: Dict[str, Any]) -> Optional[Signal]:
        ...
        try:
            ...
            logger.error("False breakout exit signal generation failed", strategy=self.name, error=str(e))
            return None
    ...
    async def validate_signal(self, signal: Signal) -> bool:
        ...
        try:
            ...
            # Check if signal is too old (more than 5 minutes for breakout signals)
            ...
            if metadata.get("signal_type") == "breakout_entry":
                required_fields = ["breakout_level", "breakout_price", "target_price", "breakout_direction"]
                ...
                if breakout_price is None or breakout_price <= 0:
                    logger.warning("Invalid breakout price", strategy=self.name, breakout_price=breakout_price)
                    return False
    ...
    def get_position_size(self, signal: Signal) -> Decimal:
        ...
        try:
            ...
            position_size = base_size * Decimal(str(confidence_factor)) * Decimal(str(breakout_strength))
    ...
    def should_exit(self, position: Position, data: MarketData) -> bool:
        ...
        try:
            ...
            if len(self.high_history) >= self.atr_period + 1:  # Need at least period + 1 data points
                atr = calculate_atr(
                    self.high_history[-(self.atr_period + 1):],  # Pass more data for talib
