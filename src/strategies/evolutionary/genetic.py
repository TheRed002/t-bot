...
class GeneticConfig(BaseModel):
    ...
    parallel_evaluation: bool = Field(
        default=True, description="Evaluate population in parallel"
    )
    ...
    diversity_threshold: float = Field(
        default=0.1, description="Minimum population diversity"
    )
...
class GeneticAlgorithm:
            ...
            if tasks:
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        logger.error(f"Evaluation failed for individual", error=str(result))
    ...
    async def _evaluate_individual(self, individual: Individual) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to evaluate individual", error=str(e))
    ...
    def _create_offspring(self, parents: List[Individual]) -> Population:
        ...
        for i in range(0, len(parents) - 1, 2):
            ...
            # Mutation
            child1_genes = self.mutation_operator.mutate(
                child1_genes, self.parameter_ranges
            )
            child2_genes = self.mutation_operator.mutate(
                child2_genes, self.parameter_ranges
            )
        ...
        for i in range(len(individuals)):
            for j in range(i + 1, len(individuals)):
                distance = self._gene_distance(
                    individuals[i].genes, individuals[j].genes
                )
    ...
    def get_evolution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_population_stats": {
                "avg_fitness": self.evolution_history[-1]["avg_fitness"]
                if self.evolution_history else 0,
                "std_fitness": self.evolution_history[-1]["std_fitness"]
                if self.evolution_history else 0,
            },
        }