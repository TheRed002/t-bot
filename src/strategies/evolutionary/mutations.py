        ...
        for param, value in genes.items():
            if random.random() < current_rate:
                if param in parameter_ranges:
                    mutated[param] = self._mutate_parameter(
                        value, parameter_ranges[param]
                    )
        
        return mutated

    def _mutate_parameter(
        self, value: Any, param_range: Tuple[Any, Any]
    ) -> Any:
            ...
            if random.random() < self.mutation_rate:
                ...
                if param in parameter_ranges:
                    if strategy == "gaussian":
                        mutated[param] = self._gaussian_mutation(
                            value, parameter_ranges[param]
                        )
                    elif strategy == "uniform":
                        mutated[param] = self._uniform_mutation(
                            value, parameter_ranges[param]
                        )
                    elif strategy == "polynomial":
                        mutated[param] = self._polynomial_mutation(
                            value, parameter_ranges[param]
                        )
                    else:
                        mutated[param] = self._mutate_parameter(
