...
from src.utils.decorators import time_execution, memory_usage, retry
...
class MultiObjectiveConfig(BaseModel):
    ...
    diversity_preservation: bool = Field(
        default=True, description="Enable diversity preservation"
    )
    parallel_evaluation: bool = Field(
        default=True, description="Enable parallel fitness evaluation"
    )
    convergence_threshold: float = Field(
        default=1e-6, ge=0.0, description="Convergence threshold"
    )
...
class DominanceComparator:
    ...
    def __init__(self, objectives: List[OptimizationObjective]):
        ...
        logger.debug(
            "DominanceComparator initialized",
...
class CrowdingDistanceCalculator:
    ...
    def __init__(self, objectives: List[OptimizationObjective]):
        ...
        logger.debug(
            "CrowdingDistanceCalculator initialized",
            objective_count=len(objectives)
        )
    ...
    def calculate_crowding_distance(
        ...
        for objective in self.objectives:
            ...
            sorted_indices = sorted(
                ...
                key=lambda i: solutions[front_indices[i]][obj_name]
            )
...
class ParetoFrontierManager:
    ...
    def __init__(self, config: MultiObjectiveConfig):
        ...
        logger.info(
            "ParetoFrontierManager initialized",
    ...
    def _calculate_spread(self, solutions: List[ParetoSolution]) -> float:
        ...
        distances = [sol.crowding_distance for sol in solutions 
    ...
    def _calculate_convergence(
        self, 
        current: List[ParetoSolution], 
        previous: List[ParetoSolution]
    ) -> float:
        """
    ...
    def get_frontier_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_crowding_distance": np.mean([
                sol.crowding_distance for sol in self.current_frontier
                if not math.isinf(sol.crowding_distance)
            ]) if self.current_frontier else 0.0,
...
class NSGAIIOptimizer:
    ...
    async def optimize(self) -> List[ParetoSolution]:
        ...
        try:
            ...
            raise OptimizationError(f"NSGA-II optimization failed: {str(e)}")
    ...
    async def _evaluate_individual(self, individual: Individual) -> Optional[ParetoSolution]:
        """
        ...
        try:
            ...
            constraint_violations = self.frontier_manager.constraint_handler.evaluate_constraints(objectives)
...
class MultiObjectiveOptimizer:
    ...
    def __init__(self, config: MultiObjectiveConfig):
        ...
        """
        self.config = config
        self.optimizer: Optional[NSGAIIOptimizer] = None
    ...
    async def optimize_strategy(
        ...
        try:
            ...
            raise OptimizationError(f"Multi-objective optimization failed: {str(e)}")
