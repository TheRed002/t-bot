...
class StrategyConfigurationManager:
    ...
    def load_strategy_config(self, strategy_name: str) -> StrategyConfig:
        ...
        try:
            ...
            logger.error("Failed to load strategy config", 
                        strategy_name=strategy_name, 
                        error=str(e))
            raise ConfigurationError(f"Failed to load configuration for {strategy_name}: {str(e)}")
    ...
    def save_strategy_config(self, strategy_name: str, config: StrategyConfig) -> None:
        ...
        try:
            ...
            logger.error("Failed to save strategy config", 
                        strategy_name=strategy_name, 
                        error=str(e))
            raise ConfigurationError(f"Failed to save configuration for {strategy_name}: {str(e)}")
        ...
        try:
            ...
            if parameter in config_dict:
                ...
                if self.validate_config(config_dict):
                    ...
                    logger.error("Invalid configuration after parameter update", 
                                strategy_name=strategy_name, 
                                parameter=parameter)
                    return False
            else:
    ...
    def create_strategy_config(self, strategy_name: str, strategy_type: StrategyType, 
                             symbols: List[str], **kwargs) -> StrategyConfig:
        ...
        try:
            ...
            config_data.update({
                "name": strategy_name,
                "strategy_type": strategy_type.value if hasattr(strategy_type, 'value') else strategy_type,
                "symbols": symbols,
                **kwargs
            })
            ...
            logger.info("Strategy config created",
                       strategy_name=strategy_name,
                       strategy_type=strategy_type.value if hasattr(strategy_type, 'value') else strategy_type)
            ...
            logger.error("Failed to create strategy config", 
                        strategy_name=strategy_name, 
                        error=str(e))
            raise ConfigurationError(f"Failed to create configuration for {strategy_name}: {str(e)}")
