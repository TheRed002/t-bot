...
class BaseAlgorithm(ABC):
    ...
    async def _update_execution_result(
            ...
            if child_order.filled_quantity > 0:
                ...
                if child_order.price and child_order.filled_quantity > 0:
                    if old_total > 0 and old_avg_price > 0:
                        ...
                        total_value = (old_total * old_avg_price) + (child_order.filled_quantity * child_order.price)
                        execution_result.average_fill_price = total_value / execution_result.total_filled_quantity
                    else:
        ...
        if status in [ExecutionStatus.COMPLETED, ExecutionStatus.FAILED, ExecutionStatus.CANCELLED]:
            ...
            execution_result.execution_duration = (end_time - execution_result.start_time).total_seconds()
    ...
    async def _calculate_slippage_metrics(
        ...
        """
        try:
            if not execution_result.average_fill_price or execution_result.total_filled_quantity <= 0:
                return
            ...
            if expected_price:
                ...
                if execution_result.original_order.side.value == "buy":
                    ...
                    execution_result.price_slippage = execution_result.average_fill_price - expected_price
                else:
                    # For sell orders, slippage is receiving less than expected
                    execution_result.price_slippage = expected_price - execution_result.average_fill_price
    ...
    async def cleanup_completed_executions(self, max_history: int = 100) -> None:
        ...
        completed_executions = [
            (execution_id, result) for execution_id, result in self.current_executions.items()
