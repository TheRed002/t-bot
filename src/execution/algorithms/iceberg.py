    ...
    async def _execute_iceberg_strategy(
        ...
        try:
            ...
            while remaining_quantity > 0 and refresh_count < self.max_refresh_attempts:
                ...
                if (self.price_improvement_enabled and 
                    instruction.order.order_type == OrderType.LIMIT and
                    (datetime.now(timezone.utc) - last_price_update).total_seconds() > self.price_staleness_seconds):
                ...
                try:
                    ...
                    self.logger.error(
                        f"Failed to place iceberg slice {refresh_count + 1}: {e}"
                    )
