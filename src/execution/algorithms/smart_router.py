...
class SmartOrderRouter(BaseAlgorithm):
    ...
    async def _validate_algorithm_parameters(self, instruction: ExecutionInstruction) -> None:
        ...
        if instruction.avoid_exchanges:
            remaining_exchanges = [
                ...
                if ex not in instruction.avoid_exchanges
            ]
    ...
    async def _create_routing_plan(
        ...
        try:
            ...
            if (order_value >= self.min_order_value_for_splitting and 
                len(exchange_scores) > 1 and
                not instruction.is_urgent):
                # Use multi-exchange routing for large orders
                routes = await self._create_split_routing(
                    instruction, exchange_scores
                )
    ...
    async def _get_candidate_exchanges(self, instruction: ExecutionInstruction) -> list[str]:
        ...
        if instruction.preferred_exchanges:
            candidates = [ex for ex in instruction.preferred_exchanges 
    ...
    async def _score_exchanges(
        ...
        for exchange_name in exchanges:
            try:
                ...
                if (cache_key in self.last_score_update and
                    (current_time - self.last_score_update[cache_key]).total_seconds() < self.score_cache_ttl_seconds):
    ...
    async def _create_split_routing(
        self, 
        instruction: ExecutionInstruction,
        exchange_scores: dict[str, float]
    ) -> list[dict[str, Any]]:
        """
    ...
    async def _finalize_execution(self, execution_result: ExecutionResult) -> None:
        ...
        try:
            ...
            if execution_result.total_filled_quantity > 0:
                ...
                fill_rate = execution_result.total_filled_quantity / execution_result.original_order.quantity
                execution_result.metadata["smart_routing_fill_rate"] = float(fill_rate)
                execution_result.metadata["multi_exchange_execution"] = len(execution_result.child_orders) > 1
                execution_result.metadata["exchanges_used"] = len(set(
