...
class SlippageModel:
    ...
    async def predict_slippage(
        ...
        try:
            ...
            spread_cost_bps = await self._calculate_spread_cost(
                order, market_data
            )
            ...
            total_slippage_bps = (
                adjusted_market_impact + 
                adjusted_timing_cost + 
                adjusted_spread_cost
            )
    ...
    async def _calculate_spread_cost(
        self,
        order: OrderRequest,
        market_data: MarketData
    ) -> Decimal:
    ...
    async def update_historical_data(
        ...
        try:
            ...
            if len(self.historical_slippage[symbol]) > max_history_size:
                self.historical_slippage[symbol] = self.historical_slippage[symbol][-max_history_size:]
                self.market_conditions_history[symbol] = self.market_conditions_history[symbol][-max_history_size:]
    ...
    async def _update_model_parameters(self, symbol: str) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Failed to update model parameters for {symbol}: singular matrix")
    ...
    async def get_slippage_confidence_interval(
        self,
        predicted_slippage: SlippageMetrics,
        ...
        try:
            ...
            summary = {
                ...
                "total_predictions": sum(
                    len(self.historical_slippage[s]) for s in symbols
                ),
            ...
            for sym in symbols:
                ...
                summary["symbol_details"][sym] = {
                    ...
                    "last_update": self.model_parameters[sym]["updated_at"].isoformat() if has_model else None
                }
