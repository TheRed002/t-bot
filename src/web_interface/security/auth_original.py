...
# Database client for user operations
db_session = None
...
def init_auth(config: Config) -> None:
    ...
    """
    global jwt_handler, db_session
    jwt_handler = JWTHandler(config)

    # Initialize database connection for user management
    try:
        database_url = getattr(config, "database_url", None) or config.database.postgresql_url
        if database_url:
            engine = create_engine(database_url)
            SessionLocal = sessionmaker(bind=engine)
            db_session = SessionLocal()
            logger.info("Database connection initialized for authentication")
        else:
            logger.warning("No database URL configured, authentication may not work properly")
    except Exception as e:
        logger.error(f"Failed to initialize database for authentication: {e}")
    ...
    try:
        ...
        query = text(
            ...
            SELECT user_id, username, email, password_hash, full_name,
                   is_active, scopes, created_at, last_login
            FROM users
        ...
        try:
            scopes = json.loads(result.scopes) if result.scopes else ["read"]
...
def authenticate_user(username: str, password: str) -> UserInDB | None:
    ...
    try:
        ...
        result = db_session.execute(user_query, {"username": username}).fetchone()
        ...
        if result.locked_until and datetime.now(timezone.utc) < result.locked_until:
        ...
        # Verify password
        if not jwt_handler.verify_password(password, result.password_hash):
            ...
            if failed_attempts >= 5:
                ...
                logger.warning(
                    f"Account locked due to {failed_attempts} failed attempts", username=username
                )
            ...
            db_session.execute(
                update_query,
                {"attempts": failed_attempts, "lock_until": lock_until, "username": username},
            )
            db_session.commit()
        ...
        db_session.execute(reset_query, {"now": datetime.now(timezone.utc), "username": username})
        db_session.commit()
