...
from src.monitoring.metrics import get_metrics_collector, MetricsCollector
from src.monitoring.alerting import (
    get_alert_manager, AlertManager, Alert, AlertRule, AlertSeverity,
    NotificationChannel, AlertStatus
)
from src.monitoring.performance import get_performance_profiler, PerformanceProfiler
from src.monitoring.telemetry import get_trading_tracer, TradingTracer
...
class AlertRuleRequest(BaseModel):
    ...
    escalation_delay: Optional[str] = None
    enabled: bool = True
    ...
    try:
        ...
        config = {
            "metrics_collection": {
                ...
                "collection_interval": getattr(collector, "_collection_interval", None) if collector else None
            },
            "alerting": {
                "enabled": alert_manager is not None,
                "rules_count": len(getattr(alert_manager, "_rules", {})) if alert_manager else 0,
                "escalation_policies_count": len(getattr(alert_manager, "_escalation_policies", {})) if alert_manager else 0
            },
            "performance_profiling": {
                "enabled": profiler is not None,
                "memory_tracking": getattr(profiler, "enable_memory_tracking", False) if profiler else False,
                "cpu_profiling": getattr(profiler, "enable_cpu_profiling", False) if profiler else False
            }
        }
