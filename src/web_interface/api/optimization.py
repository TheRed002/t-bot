...
class OptimizationRequest(BaseModel):
    ...
    ml_models_to_test: list[str] = Field(
        default_factory=list, description="ML models to include in optimization"
    )
...
async def create_optimization_job(
    ...
    try:
        ...
        for param_range in request.parameter_ranges:
            range_size = (
                int((param_range.max_value - param_range.min_value) / param_range.step_size) + 1
            )
        ...
        logger.info(
            f"Created optimization job {job_id} with {total_combinations} combinations for user {user.username}"
        )
        ...
        logger.error(f"Error creating optimization job: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to create optimization job: {e!s}",
        )
...
async def start_optimization_job(
    ...
    try:
        if job_id not in active_optimization_jobs:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Optimization job not found"
            )
        ...
        logger.error(f"Error starting optimization job {job_id}: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to start optimization job: {e!s}",
        )
...
async def get_optimization_job_status(
    ...
    try:
        ...
        estimated_remaining = (
            (total - completed) / combinations_per_minute if combinations_per_minute > 0 else 0
        )
        ...
        logger.error(f"Error getting optimization job status {job_id}: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_optimization_job_results(
    ...
    try:
        ...
        optimization_summary = {
            ...
            "best_metric_value": (
                valid_results[0].get(optimization_metric, 0) if valid_results else 0
            ),
            "improvement_over_baseline": 0.25,  # Mock 25% improvement
            "stability_analysis": {
                "consistent_performers": len(
                    [r for r in valid_results if r.get("stability_score", 0) > 0.7]
                ),
                "overfitted_results": len([r for r in results if not r.get("is_valid", True)]),
            },
            "risk_analysis": {
                "low_risk_results": len(
                    [r for r in valid_results if r.get("max_drawdown", 1) < 0.15]
                ),
                "high_sharpe_results": len(
                    [r for r in valid_results if r.get("sharpe_ratio", 0) > 1.5]
                ),
            },
        }
        ...
        logger.error(f"Error getting optimization job results {job_id}: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def stop_optimization_job(
    ...
    try:
        ...
        logger.error(f"Error stopping optimization job {job_id}: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to stop optimization job: {e!s}",
        )
    ...
    try:
        ...
        if job["status"] in ["running", "starting"]:
            try:
                ...
                logger.warning(f"Error stopping job during deletion: {e!s}")
        ...
        logger.error(f"Error deleting optimization job {job_id}: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to delete optimization job: {e!s}",
        )
...
async def list_optimization_jobs(
    ...
    try:
        ...
        logger.error(f"Error listing optimization jobs: {e!s}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to list optimization jobs: {e!s}",
        )
...
def _generate_parameter_combinations(request_data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for combination_values in itertools.product(*param_value_lists):
        combination = dict(zip(param_names, combination_values, strict=False))
...
async def _run_parameter_combination(
    job_id: str, parameters: dict[str, Any], request_data: dict[str, Any]
) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error running parameter combination: {e!s}")
        return None
