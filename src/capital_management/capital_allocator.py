...
from src.core.types import (
    CapitalAllocation, CapitalMetrics, AllocationStrategy,
    Position, MarketData
)
...
class CapitalAllocator:
    ...
    def __init__(self, config: Config, risk_manager: Optional[BaseRiskManager] = None):
        """
        ...
        self.emergency_reserve = self.total_capital * Decimal(str(self.capital_config.emergency_reserve_pct))
    ...
    @time_execution
    async def allocate_capital(self, strategy_id: str, exchange: str, 
                             requested_amount: Decimal) -> CapitalAllocation:
        """
        ...
        try:
            ...
            max_allocation = self.available_capital * Decimal(str(self.capital_config.max_allocation_pct))
    ...
    @time_execution
    async def update_utilization(self, strategy_id: str, exchange: str, 
                               utilized_amount: Decimal) -> None:
        """
    ...
    async def get_capital_metrics(self) -> CapitalMetrics:
        ...
        try:
            ...
            utilization_rate = float(total_utilized / total_allocated) if total_allocated > 0 else 0.0
            
            # Calculate allocation efficiency (based on performance vs allocation)
    ...
    async def _calculate_performance_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for strategy_id in self.strategy_allocations.keys():
            ...
            # In production, this would fetch real performance data from database
            if strategy_key not in self.strategy_performance:
                self.strategy_performance[strategy_key] = {
                    'sharpe_ratio': 0.5 + (hash(strategy_key) % 100) / 1000,  # Mock Sharpe ratio
                    'return_rate': 0.02 + (hash(strategy_key) % 50) / 1000,    # Mock return rate
                    'volatility': 0.15 + (hash(strategy_key) % 30) / 1000,     # Mock volatility
                    'max_drawdown': 0.05 + (hash(strategy_key) % 20) / 1000,   # Mock drawdown
                    'win_rate': 0.55 + (hash(strategy_key) % 30) / 1000        # Mock win rate
                }
    ...
    async def _equal_weight_allocation(self) -> Dict[str, CapitalAllocation]:
        ...
        strategies = list(set(alloc.strategy_id.split('_')[0] for alloc in self.strategy_allocations.values()))
        ...
        for strategy_id in strategies:
            for exchange in ['binance', 'okx', 'coinbase']:
                ...
                if key in self.strategy_allocations:
                    allocation = self.strategy_allocations[key]
                    allocation.allocated_amount = allocation_per_strategy / 3  # Split across exchanges
                    allocation.allocation_percentage = float(allocation.allocated_amount / self.total_capital)
                    new_allocations[key] = allocation
        
        return new_allocations
    
    async def _performance_weighted_allocation(self, performance_metrics: Dict[str, Dict[str, float]]) -> Dict[str, CapitalAllocation]:
        """Performance-weighted allocation strategy."""
        # Calculate weights based on Sharpe ratios
        total_sharpe = sum(metrics.get('sharpe_ratio', 0) for metrics in performance_metrics.values())
        
        new_allocations = {}
        for strategy_id, metrics in performance_metrics.items():
            weight = metrics.get('sharpe_ratio', 0) / total_sharpe if total_sharpe > 0 else 1.0 / len(performance_metrics)
    ...
    async def _volatility_weighted_allocation(self, performance_metrics: Dict[str, Dict[str, float]]) -> Dict[str, CapitalAllocation]:
        ...
        for strategy_id, inverse_vol in inverse_volatilities.items():
            weight = inverse_vol / total_inverse_vol if total_inverse_vol > 0 else 1.0 / len(inverse_volatilities)
            allocation_amount = self.available_capital * Decimal(str(weight))
    ...
    async def _risk_parity_allocation(self, performance_metrics: Dict[str, Dict[str, float]]) -> Dict[str, CapitalAllocation]:
    ...
    async def _dynamic_allocation(self, performance_metrics: Dict[str, Dict[str, float]]) -> Dict[str, CapitalAllocation]:
    ...
    async def _apply_rebalancing_limits(self, new_allocations: Dict[str, CapitalAllocation]) -> Dict[str, CapitalAllocation]:
        """Apply rebalancing limits to prevent excessive changes."""
        max_daily_change = self.total_capital * Decimal(str(self.capital_config.max_daily_reallocation_pct))
        
        for key, allocation in new_allocations.items():
            if key in self.strategy_allocations:
                current_allocation = self.strategy_allocations[key]
                change_amount = abs(allocation.allocated_amount - current_allocation.allocated_amount)
                
                if change_amount > max_daily_change:
                    # Limit the change
                    if allocation.allocated_amount > current_allocation.allocated_amount:
                        allocation.allocated_amount = current_allocation.allocated_amount + max_daily_change
                    else:
                        allocation.allocated_amount = current_allocation.allocated_amount - max_daily_change
    ...
    async def _calculate_allocation_efficiency(self) -> float:
        ...
        # Base efficiency: utilization rate (how much allocated capital is being used)
        ...
        final_efficiency = utilization_efficiency * performance_multiplier * market_regime_multiplier
    ...
    async def _calculate_market_regime_multiplier(self) -> float:
        ...
        # Placeholder implementation - should be replaced with real market regime detection
    ...
    async def get_allocation_summary(self) -> Dict[str, Any]:
        ...
        total_allocated = sum(alloc.allocated_amount for alloc in self.strategy_allocations.values())
