...
class CapitalAllocator:
    ...
    def __init__(
        ...
        self.emergency_reserve = self.total_capital * \
            Decimal(str(self.capital_config.emergency_reserve_pct))
        ...
        self.performance_window = timedelta(
            days=30)  # 30-day performance window
    ...
    async def allocate_capital(self, strategy_id: str, exchange: str,
        ...
        try:
            ...
            max_allocation = self.available_capital * \
                Decimal(str(self.capital_config.max_allocation_pct))
            ...
            allocation_percentage = float(
                requested_amount / self.total_capital)
    ...
    async def rebalance_allocations(self) -> Dict[str, CapitalAllocation]:
        ...
        try:
            ...
            strategy = AllocationStrategy(
                self.capital_config.allocation_strategy)
        ...
        try:
            ...
            if key in self.strategy_allocations:
                ...
                logger.debug(
                    ...
                    strategy_id=strategy_id,
                    exchange=exchange,
                    utilized=format_currency(
                        float(utilized_amount)),
                    available=format_currency(
                        float(
    ...
    async def get_capital_metrics(self) -> CapitalMetrics:
        ...
        try:
            ...
            utilization_rate = float(
                total_utilized /
                total_allocated) if total_allocated > 0 else 0.0
    ...
    async def _equal_weight_allocation(self) -> Dict[str, CapitalAllocation]:
        ...
        strategies = list(set(alloc.strategy_id.split(
            '_')[0] for alloc in self.strategy_allocations.values()))
    ...
    async def _performance_weighted_allocation(
            self, performance_metrics: Dict[str, Dict[str, float]]) -> Dict[str, CapitalAllocation]:
        """Performance-weighted allocation strategy."""
        # Calculate weights based on Sharpe ratios
        total_sharpe = sum(metrics.get('sharpe_ratio', 0)
        ...
        for strategy_id, metrics in performance_metrics.items():
            weight = metrics.get(
                'sharpe_ratio',
                0) / total_sharpe if total_sharpe > 0 else 1.0 / len(performance_metrics)
    ...
    async def _volatility_weighted_allocation(
        ...
        for strategy_id, inverse_vol in inverse_volatilities.items():
            weight = inverse_vol / total_inverse_vol if total_inverse_vol > 0 else 1.0 / \
                len(inverse_volatilities)
            allocation_amount = self.available_capital * Decimal(str(weight))
    ...
    async def _risk_parity_allocation(
        ...
        for strategy_id, risk in risk_contributions.items():
            ...
            weight = target_risk / risk if risk > 0 else 1.0 / \
    ...
    async def _calculate_allocation_efficiency(self) -> float:
        ...
        final_efficiency = utilization_efficiency * \
            performance_multiplier * market_regime_multiplier
    ...
    async def _calculate_performance_multiplier(self) -> float:
        ...
        avg_performance = total_performance_score / \
            strategy_count if strategy_count > 0 else 1.0
    ...
    def _get_minimum_allocation(self, strategy_id: str) -> float:
        ...
        strategy_type = strategy_id.split(
            '_')[0] if '_' in strategy_id else strategy_id
        return self.capital_config.per_strategy_minimum.get(
            strategy_type, 1000.0)
    ...
    async def get_allocation_summary(self) -> Dict[str, Any]:
        ...
        for key, allocation in self.strategy_allocations.items():
            strategy_id, exchange = key.split(
                '_', 1) if '_' in key else (
                key, "unknown")
