...
from src.core.types import (
    ExchangeAllocation, CapitalAllocation, FundFlow
)
...
class ExchangeDistributor:
    ...
    async def distribute_capital(
            self, total_amount: Decimal) -> Dict[str, ExchangeAllocation]:
        """
    ...
    async def rebalance_exchanges(self) -> Dict[str, ExchangeAllocation]:
        ...
        try:
            ...
            logger.info(
                "Exchange rebalancing completed",
                allocations_count=len(final_allocations)
            )
    ...
    async def get_exchange_allocation(
            self, exchange: str) -> Optional[ExchangeAllocation]:
        """
    ...
    @time_execution
    async def update_exchange_utilization(
            self, exchange: str, utilized_amount: Decimal) -> None:
        """
        ...
        try:
            if exchange in self.exchange_allocations:
                ...
                logger.debug(
                    "Exchange utilization updated", exchange=exchange, utilized=format_currency(
                        float(utilized_amount)), available=format_currency(
                        float(
                            allocation.available_amount)), utilization_rate=f"{
                        allocation.utilization_rate: .2 %}")

        except Exception as e:
            logger.error(
                "Exchange utilization update failed",
                exchange=exchange,
                error=str(e)
            )
    ...
    async def calculate_optimal_distribution(
            self, total_capital: Decimal) -> Dict[str, Decimal]:
        """
        ...
        try:
            ...
            for exchange, score in composite_scores.items():
                ...
                distribution[exchange] = total_capital * \
            ...
            logger.error(
                "Optimal distribution calculation failed",
                error=str(e))
            raise
    ...
    async def _update_exchange_metrics(self) -> None:
        ...
        try:
            for exchange_name, exchange in self.exchanges.items():
                ...
                self.liquidity_scores[exchange_name] = await self._calculate_liquidity_score(exchange)

                # Update fee efficiency
                self.fee_efficiencies[exchange_name] = await self._calculate_fee_efficiency(exchange)

                # Update reliability score
                self.reliability_scores[exchange_name] = await self._calculate_reliability_score(exchange)
    ...
    async def _calculate_liquidity_score(
            self, exchange: BaseExchange) -> float:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to calculate liquidity score for {exchange}",
                error=str(e))
    ...
    async def _calculate_fee_efficiency(self, exchange: BaseExchange) -> float:
        ...
        try:
            ...
            logger.error(
                f"Failed to calculate fee efficiency for {exchange}",
                error=str(e))
            return 0.5  # Default score
    ...
    async def _calculate_reliability_score(
        ...
        try:
            ...
            logger.error(
                f"Failed to calculate reliability score for {exchange}",
                error=str(e))
            return 0.5  # Default score
    ...
    async def _update_slippage_data(self, exchange_name: str) -> None:
        ...
        try:
            ...
            if len(self.historical_slippage[exchange_name]) > 100:
                self.historical_slippage[exchange_name] = self.historical_slippage[exchange_name][-100:]

        except Exception as e:
            logger.error(
                f"Failed to update slippage data for {exchange_name}",
                error=str(e))

    async def _dynamic_distribution(
            self, total_amount: Decimal) -> Dict[str, ExchangeAllocation]:
    ...
    async def _weighted_distribution(self,
        ...
        for exchange, weight in weights.items():
            if exchange in self.exchanges:
                ...
                allocation = ExchangeAllocation(
                    ...
                    available_amount=amount,
                    utilization_rate=0.0,
                    liquidity_score=self.liquidity_scores.get(
                        exchange,
                        0.5),
                    fee_efficiency=self.fee_efficiencies.get(
                        exchange,
                        0.5),
    ...
    async def _apply_minimum_balances(self,
                                      allocations: Dict[str,
                                                        ExchangeAllocation]) -> Dict[str,
                                                                                     ExchangeAllocation]:
        """Apply minimum balance requirements."""
    ...
    async def _apply_rebalancing_limits(self,
                                        new_allocations: Dict[str,
                                                              ExchangeAllocation]) -> Dict[str,
                                                                                           ExchangeAllocation]:
        """Apply rebalancing limits to prevent excessive changes."""
        max_daily_change = self.total_capital * \
    ...
    async def get_exchange_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for exchange_name in self.exchanges.keys():
            metrics[exchange_name] = {
                'liquidity_score': self.liquidity_scores.get(
                    exchange_name, 0.5), 'fee_efficiency': self.fee_efficiencies.get(
                    exchange_name, 0.5), 'reliability_score': self.reliability_scores.get(
                    exchange_name, 0.5), 'avg_slippage': statistics.mean(
                    self.historical_slippage.get(
                        exchange_name, [0.001]))}
