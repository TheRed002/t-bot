...
class CurrencyManager:
    ...
    @time_execution
    async def update_currency_exposures(self, balances: Dict[str, Dict[str, Decimal]]) -> Dict[str, CurrencyExposure]:
        """
        ...
        try:
            ...
            for currency, amount in total_exposures.items():
                ...
                exposure_percentage = float(base_equivalent / total_base_value) if total_base_value > 0 else 0.0
        ...
        try:
            ...
            for currency, exposure in self.currency_exposures.items():
                if exposure.hedging_required:
                    ...
                    required_hedge = exposure.base_currency_equivalent * Decimal(str(self.hedge_ratio))
    ...
    @time_execution
    async def execute_currency_conversion(self, from_currency: str, to_currency: str, 
                                        amount: Decimal, exchange: str) -> FundFlow:
        """
            ...
            if from_currency == to_currency:
                ...
                if not rate:
                    ...
                    if reverse_rate:
                        ...
                        raise ValidationError(f"No exchange rate available for {from_currency}/{to_currency}")
    ...
    @time_execution
    async def optimize_currency_allocation(self, target_allocations: Dict[str, Decimal]) -> Dict[str, Decimal]:
        """
        ...
        try:
            ...
            current_exposures = {curr: exp.total_exposure for curr, exp in self.currency_exposures.items()}
            ...
            logger.info(
                ...
                total_conversion_cost=format_currency(float(sum(abs(change) for change in optimized_changes.values())))
            )
        ...
        try:
            ...
            for currency, exposure in self.currency_exposures.items():
                ...
                if len(rate_history) > 1:
                    rates = [rate for _, rate in rate_history[-30:]]  # Last 30 data points
    ...
    async def _optimize_conversions(self, required_changes: Dict[str, Decimal]) -> Dict[str, Decimal]:
        """
    ...
    async def get_currency_exposure(self, currency: str) -> Optional[CurrencyExposure]:
    ...
    async def get_hedging_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'hedge_positions': {curr: float(amount) for curr, amount in self.hedge_positions.items()}
        } 