...
from src.core.types import (
    CurrencyExposure, FundFlow, CapitalAllocation
)
...
class CurrencyManager:
    ...
    async def update_currency_exposures(
            self, balances: Dict[str, Dict[str, Decimal]]) -> Dict[str, CurrencyExposure]:
        """
        ...
        try:
            ...
            for currency, amount in total_exposures.items():
                if currency == self.base_currency:
                    ...
                    rate = self.exchange_rates.get(
                        f"{currency}/{self.base_currency}", Decimal("1"))
            ...
            for currency, amount in total_exposures.items():
                ...
                exposure_percentage = float(
                    base_equivalent / total_base_value) if total_base_value > 0 else 0.0

                # Determine if hedging is required
                hedging_required = (
                    self.capital_config.hedging_enabled and
                    currency != self.base_currency and
                    exposure_percentage > self.hedging_threshold
                )

                # Calculate hedge amount
                hedge_amount = Decimal("0")
                if hedging_required:
                    hedge_amount = base_equivalent * \
                        Decimal(str(self.hedge_ratio))
            ...
            logger.info(
                ...
                total_base_value=format_currency(
                    float(total_base_value)),
                hedging_required=sum(
                    1 for e in exposures.values() if e.hedging_required))
        ...
        try:
            ...
            for currency, exposure in self.currency_exposures.items():
                if exposure.hedging_required:
                    ...
                    required_hedge = exposure.base_currency_equivalent * \
                        Decimal(str(self.hedge_ratio))
                    current_hedge = self.hedge_positions.get(
                        currency, Decimal("0"))

                    hedge_delta = required_hedge - current_hedge
                    if abs(hedge_delta) > Decimal(
                            "0.01"):  # Minimum hedge amount
            ...
            logger.error(
                "Failed to calculate hedging requirements",
                error=str(e))
            raise
    ...
    async def execute_currency_conversion(
        ...
        try:
            ...
            if from_currency == to_currency:
                ...
                if not rate:
                    # Try reverse rate
                    reverse_rate = self.exchange_rates.get(
    ...
    async def optimize_currency_allocation(
            self, target_allocations: Dict[str, Decimal]) -> Dict[str, Decimal]:
        """
        ...
        try:
            ...
            current_exposures = {
                curr: exp.total_exposure for curr,
                exp in self.currency_exposures.items()}
            ...
            logger.info(
                ...
                total_conversion_cost=format_currency(
                    float(
                        sum(
                            abs(change) for change in optimized_changes.values()))))

            return optimized_changes

        except Exception as e:
            logger.error(
                "Failed to optimize currency allocation",
                error=str(e))
            raise
    ...
    async def get_currency_risk_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            for currency, exposure in self.currency_exposures.items():
                ...
                rate_history = self.rate_history.get(
            ...
            logger.error(
                "Failed to calculate currency risk metrics",
                error=str(e))
            raise
    ...
    async def _optimize_conversions(
            self, required_changes: Dict[str, Decimal]) -> Dict[str, Decimal]:
        """
    ...
    async def get_currency_exposure(
            self, currency: str) -> Optional[CurrencyExposure]:
    ...
    async def update_hedge_position(
            self,
            currency: str,
            hedge_amount: Decimal) -> None:
    ...
    async def get_hedging_summary(self) -> Dict[str, Any]:
        ...
        currencies_hedged = len([curr for curr, exp in self.currency_exposures.items()
                                 if exp.hedging_required])
        ...
        return {
            ...
            'hedge_positions': {
                curr: float(amount) for curr,
                amount in self.hedge_positions.items()}}
