...
class DataProcessor:
    ...
    async def process_market_data(
        ...
        try:
            ...
            logger.debug(f"Market data processed successfully for {data.symbol if data else 'unknown'}")
            return result
                ...
                if symbol_key in self.data_windows:
                    ...
                    if window:
                        ...
                        if last_price and data.price:
                            price_change = calculate_percentage_change(float(last_price), float(data.price))
    ...
    async def get_aggregated_data(self, symbol: str, exchange: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to get aggregated data for {symbol}: {str(e)}")
            return {}
