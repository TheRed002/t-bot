...
from src.core.types import MarketData, Signal, ValidationLevel, ValidationResult
    ...
    async def validate_signal(self, signal: Signal) -> tuple[bool, list[ValidationIssue]]:
        ...
        try:
            ...
            is_valid = (
                len(
                    [
                        i
                        for i in issues
                        if i.level in [ValidationLevel.CRITICAL, ValidationLevel.HIGH]
                    ]
                )
                == 0
            )
            ...
            if issues:
                ...
                logger.debug(
                    "Signal validation passed", symbol=signal.symbol, strategy=signal.strategy_name
                )
    ...
    async def validate_cross_source_consistency(
        ...
        try:
            ...
            if primary_data.price and secondary_data.price:
                price_diff = abs(primary_data.price -
                                 secondary_data.price) / primary_data.price
            ...
            if primary_data.volume and secondary_data.volume:
                volume_diff = abs(primary_data.volume -
                                  secondary_data.volume) / primary_data.volume
