...
class DataValidator:
    ...
    def __init__(self, config: Config):
        ...
        self.price_change_threshold = getattr(
            config, 'price_change_threshold', 0.5)  # 50% max change
        self.volume_change_threshold = getattr(
            config, 'volume_change_threshold', 10.0)  # 1000% max change
        self.outlier_std_threshold = getattr(
            config, 'outlier_std_threshold', 3.0)  # 3 standard deviations
        ...
        self.consistency_threshold = getattr(
            config, 'consistency_threshold', 0.01)  # 1% max difference

        logger.info("DataValidator initialized", config=config)

    @time_execution
    async def validate_market_data(
            self, data: MarketData) -> Tuple[bool, List[ValidationIssue]]:
        """
        ...
        try:
            ...
            high_issues = [
                i for i in issues if i.level == ValidationLevel.HIGH]
    ...
    async def validate_signal(self,
        ...
        try:
            ...
            is_valid = len([i for i in issues if i.level in [
                           ValidationLevel.CRITICAL, ValidationLevel.HIGH]]) == 0
    ...
    async def validate_cross_source_consistency(
        ...
        try:
            ...
            if primary_data.price and secondary_data.price:
                price_diff = abs(
                    primary_data.price - secondary_data.price) / primary_data.price
            ...
            if primary_data.volume and secondary_data.volume:
                volume_diff = abs(
                    primary_data.volume - secondary_data.volume) / primary_data.volume
                ...
                if volume_diff > self.consistency_threshold * 2:  # More lenient for volume
                    issues.append(
                        ValidationIssue(
                            ...
                            message=f"Volume inconsistency between sources: {
                                volume_diff: .4f}",
                            level=ValidationLevel.MEDIUM,
            ...
            is_consistent = len([i for i in issues if i.level in [
                                ValidationLevel.CRITICAL, ValidationLevel.HIGH]]) == 0
    ...
    def _is_valid_symbol_format(self, symbol: str) -> bool:
        ...
        quote_currencies = [
            ...
            'ADA',
            'DOT']
        has_valid_quote = any(symbol_upper.endswith(quote)
