...
class DataCleaner:
    ...
    async def clean_market_data(self, data: MarketData) -> Tuple[MarketData, CleaningResult]:
        ...
        try:
            ...
            if applied_strategies:
                ...
                logger.debug("Market data passed cleaning without changes", symbol=data.symbol if data else "unknown")
            
            return data, cleaning_result
            
        except Exception as e:
            logger.error("Market data cleaning failed", symbol=data.symbol if data and data.symbol else "unknown", error=str(e))
            cleaning_result = CleaningResult(
                original_data=original_data,
            ...
            if std_price > 0:
                ...
                if z_score > self.outlier_threshold:
                    ...
                    if self.config.get('outlier_strategy') == CleaningStrategy.ADJUST:
                        adjusted_price = mean_price + (self.outlier_threshold * std_price * np.sign(current_price - mean_price))
        ...
        if data.volume and data.symbol in self.volume_history:
            ...
            if len(volume_history) >= 10:
                ...
                if std_volume > 0:
                    ...
                    if z_score > self.outlier_threshold:
                        if self.config.get('outlier_strategy') == CleaningStrategy.ADJUST:
                            adjusted_volume = mean_volume + (self.outlier_threshold * std_volume * np.sign(current_volume - mean_volume))
    ...
    async def _is_duplicate_signal(self, signal: Signal, existing_signals: List[Signal]) -> bool:
        ...
        for existing_signal in existing_signals:
            if (signal.symbol == existing_signal.symbol and
                signal.direction == existing_signal.direction and
                abs((signal.timestamp - existing_signal.timestamp).total_seconds()) < self.duplicate_threshold):
                return True
        return False
