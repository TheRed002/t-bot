...
class QualityMonitor:
    ...
    async def monitor_data_quality(self, data: MarketData) -> Tuple[float, List[DriftAlert]]:
        ...
        try:
            ...
            if len(self.quality_scores[data.symbol]) > self.distribution_window:
    ...
    @time_execution
    async def monitor_signal_quality(self, signals: List[Signal]) -> Tuple[float, List[DriftAlert]]:
        """
    ...
    @time_execution
    async def generate_quality_report(self, symbol: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            report['distribution_summary'] = {
                'price_distributions': {sym: len(dist) for sym, dist in self.price_distributions.items()},
                'volume_distributions': {sym: len(dist) for sym, dist in self.volume_distributions.items()}
            }
    ...
    async def _calculate_quality_score(self, data: MarketData) -> float:
        ...
        if data.timestamp:
            ...
            if age_seconds > 60:  # 1 minute threshold
                freshness_score = max(0.0, 1.0 - (age_seconds - 60) / 300)  # Decay over 5 minutes
        score_components.append(freshness_score * 0.2)  # 20% weight
        
        # Consistency score (0-1)
        consistency_score = 1.0
        if data.symbol in self.price_distributions and len(self.price_distributions[data.symbol]) >= 10:
    ...
    async def _detect_drift(self, data: MarketData) -> List[DriftAlert]:
        ...
        if (data.symbol in self.price_distributions and 
            len(self.price_distributions[data.symbol]) >= 10):  # Reduced minimum requirement for testing
            ...
            if window_size >= 5:  # Need at least 5 points for meaningful drift detection
                ...
                historical_prices = self.price_distributions[data.symbol][-2*window_size:-window_size]
        ...
        if (data.symbol in self.volume_distributions and 
            len(self.volume_distributions[data.symbol]) >= 10):  # Reduced minimum requirement for testing
            ...
            if window_size >= 5:  # Need at least 5 points for meaningful drift detection
                ...
                historical_volumes = self.volume_distributions[data.symbol][-2*window_size:-window_size]
    ...
    async def _detect_signal_drift(self, signals: List[Signal]) -> List[DriftAlert]:
    ...
    async def _calculate_distribution_drift(self, recent: List[float], historical: List[float]) -> float:
    ...
    async def _generate_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        if overall_score < self.quality_thresholds['poor']:
            recommendations.append("CRITICAL: Overall data quality is poor. Review data sources and validation rules.")
        elif overall_score < self.quality_thresholds['fair']:
            recommendations.append("WARNING: Data quality needs improvement. Consider adjusting validation thresholds.")
        
        # Check for specific issues
        drift_summary = report.get('drift_summary', {})
        if drift_summary.get('covariate_drift', 0) > 5:
            recommendations.append("High covariate drift detected. Consider retraining models or adjusting features.")
        ...
        for symbol, size in price_dists.items():
            if size < self.distribution_window // 2:
                recommendations.append(f"Insufficient price data for {symbol}. Collect more data for reliable monitoring.")
