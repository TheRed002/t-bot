...
class SocialMediaDataSource:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize SocialMediaDataSource: {
                    str(e)}")
            raise DataSourceError(
                f"Social media data source initialization failed: {
                    str(e)}")
    ...
    async def _test_connections(self) -> None:
        ...
        try:
            ...
            logger.error(f"Social media connection test failed: {str(e)}")
            raise
    ...
    async def get_social_sentiment(
        ...
        try:
            ...
            for platform in platforms:
                if platform == 'twitter' and self.twitter_config.get(
                        'enabled', False):
            ...
            logger.error(
                f"Failed to get social sentiment for {symbol}: {
                    str(e)}")
            raise DataSourceError(
                f"Social sentiment analysis failed: {
                    str(e)}")

    async def _get_twitter_posts(
            self,
            symbol: str,
            hours_back: int) -> List[SocialPost]:
        ...
        try:
            ...
            for i in range(10):  # Simulate 10 posts
                ...
                content = f"  # {symbol} looking {
                    'bullish' if i %
                    2 == 0 else 'bearish'} today!"
            ...
            logger.error(f"Failed to get Twitter posts for {symbol}: {str(e)}")
            return []

    async def _get_reddit_posts(
            self,
            symbol: str,
            hours_back: int) -> List[SocialPost]:
        ...
        try:
            ...
            logger.error(f"Failed to get Reddit posts for {symbol}: {str(e)}")
            return []

    def _analyze_social_sentiment(
            self, content: str) -> tuple[SocialSentiment, float]:
        ...
        bullish_words = [
            'bullish', 'moon', 'pump', 'rocket', '🚀', 'buy', 'hodl', 'bullrun',
            'gain', 'profit', 'up', 'rise', 'surge', 'breakout'
        ]
        bearish_words = [
            'bearish', 'dump', 'crash', 'sell', 'drop', 'fall', 'dip', 'rekt',
            'loss', 'down', 'decline', 'correction', 'bear market'
        ]
        ...
        if sentiment_score > 0.1:
            sentiment = SocialSentiment.VERY_BULLISH if sentiment_score > 0.2 else SocialSentiment.BULLISH
        elif sentiment_score < -0.1:
            sentiment = SocialSentiment.VERY_BEARISH if sentiment_score < - \
                0.2 else SocialSentiment.BEARISH
        else:
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    logger.warning(
                        f"Failed to get metrics for {symbol}: {
                            str(e)}")
                    continue
            ...
            logger.error(f"Failed to get trending symbols: {str(e)}")
            raise DataSourceError(
                f"Trending symbols analysis failed: {
                    str(e)}")
    ...
    async def monitor_symbol_mentions(
        self,
        symbols: List[str],
        callback: Callable[[str, List[SocialPost]], None]
    ) -> None:
        """
    ...
    async def get_platform_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_sizes": {
                "posts": sum(
                    len(posts) for posts in self.posts_cache.values()),
                "metrics": len(
                    self.metrics_cache)}}
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error during SocialMediaDataSource cleanup: {
                    str(e)}")
