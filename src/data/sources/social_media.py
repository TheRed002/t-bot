...
class SocialMediaDataSource:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize SocialMediaDataSource: {str(e)}")
            raise DataSourceError(f"Social media data source initialization failed: {str(e)}")
    ...
    async def get_social_sentiment(
        ...
        try:
            ...
            raise DataSourceError(f"Social sentiment analysis failed: {str(e)}")
    ...
    def _analyze_social_sentiment(self, content: str) -> tuple[SocialSentiment, float]:
        ...
        if sentiment_score > 0.1:
            ...
            sentiment = SocialSentiment.VERY_BEARISH if sentiment_score < -0.2 else SocialSentiment.BEARISH
        else:
    ...
    def _calculate_social_metrics(
        ...
        if recent_posts:
            ...
            trending_score = (len(recent_posts) / len(posts)) * (recent_engagement / engagement_volume)
        else:
            trending_score = 0.0
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    logger.warning(f"Failed to get metrics for {symbol}: {str(e)}")
                    continue
            ...
            raise DataSourceError(f"Trending symbols analysis failed: {str(e)}")
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error during SocialMediaDataSource cleanup: {str(e)}")
