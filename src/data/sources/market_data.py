...
class MarketDataSource:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise DataSourceError(f"Market data source initialization failed: {str(e)}")
    ...
    async def subscribe_to_ticker(
        ...
        try:
            ...
            logger.error(f"Failed to subscribe to ticker {exchange_name}:{symbol}: {str(e)}")
    ...
    async def get_historical_data(
        ...
        try:
            ...
            logger.error(f"Failed to get historical data {exchange_name}:{symbol}: {str(e)}")
            raise DataSourceError(f"Historical data retrieval failed: {str(e)}")
                ...
                try:
                    ...
                    for subscription in ticker_subs:
                        try:
                            ...
                            logger.warning(f"Ticker update failed for {subscription.symbol}: {str(e)}")
                    
                    await asyncio.sleep(1)  # 1 second update interval
                    
                except Exception as e:
                    logger.error(f"Error in ticker stream {exchange_name}: {str(e)}")
                    await asyncio.sleep(5)  # Longer delay on error
                    
        except Exception as e:
            logger.error(f"Fatal error in ticker stream {exchange_name}: {str(e)}")
        finally:
