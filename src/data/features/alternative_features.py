...
class AlternativeFeatureCalculator:
    ...
    def __init__(self, config: Config):
        ...
        if hasattr(alt_config, 'get'):
            self.default_lookbacks = alt_config.get('default_lookbacks', {
                'news_sentiment': 24,      # 24 hours
                'social_sentiment': 12,    # 12 hours
                'economic': 168,           # 1 week
                'microstructure': 6        # 6 hours
            })
            self.sentiment_weights = alt_config.get('sentiment_weights', {
            ...
            self.default_lookbacks = {
                'news_sentiment': 24, 'social_sentiment': 12,
                'economic': 168, 'microstructure': 6
            }
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize alternative data sources: {str(e)}")
            raise DataError(f"Alternative data source initialization failed: {str(e)}")
        ...
        try:
            ...
            if not self.news_source:
                return AlternativeResult(
                    ...
                    value=None,
                    metadata={'lookback_hours': lookback_hours, 'reason': 'news_source_not_available'},
            ...
            return AlternativeResult(
                ...
                metadata={'lookback_hours': lookback_hours, 'sources_count': len(set(a.get('source', '') for a in news_articles))},
                calculation_time=(datetime.now() - start_time).total_seconds()
            )
            
        except Exception as e:
            self.calculation_stats['failed_calculations'] += 1
            logger.error(f"News sentiment calculation failed for {symbol}: {str(e)}")
            raise DataError(f"News sentiment calculation failed: {str(e)}")
        ...
        try:
            ...
            if not self.social_source:
                return AlternativeResult(
                    ...
                    timestamp=datetime.now(timezone.utc),
                    value=None,
                    metadata={'lookback_hours': lookback_hours, 'reason': 'social_source_not_available'},
                    calculation_time=(datetime.now() - start_time).total_seconds()
                )
            ...
            if total_engagement > 0:
                engagement_weights = [post.get('engagement_score', 1) / total_engagement for post in 
                                    [p for posts in social_sentiment.values() for p in posts]]
                volume_weighted_sentiment = np.average(sentiment_scores, weights=engagement_weights[:len(sentiment_scores)])
            else:
            ...
            return AlternativeResult(
                ...
                timestamp=datetime.now(timezone.utc),
                value=social_values,
                metadata={'lookback_hours': lookback_hours, 'platforms': list(social_sentiment.keys())},
                calculation_time=(datetime.now() - start_time).total_seconds()
            )
            
        except Exception as e:
            self.calculation_stats['failed_calculations'] += 1
            logger.error(f"Social sentiment calculation failed for {symbol}: {str(e)}")
            raise DataError(f"Social sentiment calculation failed: {str(e)}")
        ...
        try:
            ...
            if not self.alt_data_source:
                return AlternativeResult(
                    ...
                    timestamp=datetime.now(timezone.utc),
                    value=None,
                    metadata={'lookback_hours': lookback_hours, 'reason': 'alt_data_source_not_available'},
                    calculation_time=(datetime.now() - start_time).total_seconds()
                )
            ...
            # This is a simplified example - in practice, you'd use economic models
            economic_score = 0.0
            weight_sum = 0.0
            
            weights = {'inflation': -0.3, 'unemployment': -0.3, 'gdp_growth': 0.4, 'interest_rates': -0.2}
            ...
            economic_values = {
                ...
                'data_freshness_hours': min([(datetime.now(timezone.utc) - 
                                           datetime.fromisoformat(data_points[-1]['timestamp'].replace('Z', '+00:00'))).total_seconds() / 3600
                                          for data_points in economic_data.values() if data_points])
            }
            ...
            self.calculation_stats['failed_calculations'] += 1
            logger.error(f"Economic indicators calculation failed for {symbol}: {str(e)}")
            raise DataError(f"Economic indicators calculation failed: {str(e)}")
    ...
    def _determine_sentiment_strength(self, avg_sentiment: float, sentiment_std: float) -> SentimentStrength:
    ...
    async def calculate_market_microstructure(
        ...
        try:
            ...
            self.calculation_stats['failed_calculations'] += 1
            logger.error(f"Market microstructure calculation failed for {symbol}: {str(e)}")
            raise DataError(f"Market microstructure calculation failed: {str(e)}")
            ...
            for feature in features:
                try:
                    if feature.upper() == 'NEWS_SENTIMENT':
                        ...
                        results['ECONOMIC_INDICATORS'] = await self.calculate_economic_indicators(symbol)
                    elif feature.upper() == 'MARKET_MICROSTRUCTURE':
                        results['MARKET_MICROSTRUCTURE'] = await self.calculate_market_microstructure(symbol)
                    else:
                        logger.warning(f"Unknown alternative feature: {feature}")
                        
                except Exception as e:
                    logger.error(f"Failed to calculate {feature} for {symbol}: {str(e)}")
                    results[feature] = None
            
            logger.info(f"Calculated {len([r for r in results.values() if r is not None])} alternative features for {symbol}")
            return results
            
        except Exception as e:
            logger.error(f"Batch alternative feature calculation failed for {symbol}: {str(e)}")
