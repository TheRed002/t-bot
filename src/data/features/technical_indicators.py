...
class TechnicalIndicatorCalculator:
    ...
    def __init__(self, config: Config):
        ...
        if hasattr(indicators_config, 'get'):
            ...
            self.cache_ttl = indicators_config.get('cache_ttl', 300)  # 5 minutes
        else:
    ...
    async def calculate_bollinger_bands(
        ...
        try:
            ...
            self.calculation_stats['failed_calculations'] += 1
            logger.error(f"Bollinger Bands calculation failed for {symbol}: {str(e)}")
    ...
    async def calculate_batch_indicators(
        ...
        try:
            ...
            logger.info(f"Calculated {len([r for r in results.values() if r is not None])} indicators for {symbol}")
            return results
            
        except Exception as e:
            logger.error(f"Batch indicator calculation failed for {symbol}: {str(e)}")
        ...
        try:
            ...
            cache_hit_rate = (
                (self.calculation_stats['cache_hits'] / 
                 (self.calculation_stats['cache_hits'] + self.calculation_stats['cache_misses']) * 100)
