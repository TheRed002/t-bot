...
class BotInstance:
    ...
    def __init__(
        ...
        # Internal state
        self._state = BotStatus.READY  # Newly created bots are READY, not STOPPED
        ...
        self._logger.info(f"Created bot instance: {bot_config.bot_id}")
    ...
    async def start(self) -> None:
        ...
        """
        try:
            if self._state == BotStatus.RUNNING:
                self._logger.warning(f"Bot {self.bot_config.bot_id} is already running")
                return

            self._state = BotStatus.RUNNING
            self._last_heartbeat = datetime.now(timezone.utc)

            self._logger.info(f"Started bot: {self.bot_config.bot_id}")

        except Exception as e:
            self._logger.error(f"Failed to start bot {self.bot_config.bot_id}: {e}")
            self._state = BotStatus.ERROR
            raise
    ...
    async def stop(self) -> None:
        ...
        """
        try:
            if self._state == BotStatus.STOPPED:
                self._logger.warning(f"Bot {self.bot_config.bot_id} is already stopped")
                return

            self._state = BotStatus.STOPPED

            self._logger.info(f"Stopped bot: {self.bot_config.bot_id}")

        except Exception as e:
            self._logger.error(f"Failed to stop bot {self.bot_config.bot_id}: {e}")
            self._state = BotStatus.ERROR
            raise
    ...
    async def pause(self) -> None:
        ...
        try:
            if self._state != BotStatus.RUNNING:
                self._logger.warning(f"Bot {self.bot_config.bot_id} is not running, cannot pause")
                return

            self._state = BotStatus.PAUSED

            self._logger.info(f"Paused bot: {self.bot_config.bot_id}")

        except Exception as e:
            self._logger.error(f"Failed to pause bot {self.bot_config.bot_id}: {e}")
            self._state = BotStatus.ERROR
            raise
    ...
    async def resume(self) -> None:
        ...
        """
        try:
            if self._state != BotStatus.PAUSED:
                self._logger.warning(f"Bot {self.bot_config.bot_id} is not paused, cannot resume")
                return

            self._state = BotStatus.RUNNING
            self._last_heartbeat = datetime.now(timezone.utc)

            self._logger.info(f"Resumed bot: {self.bot_config.bot_id}")

        except Exception as e:
            self._logger.error(f"Failed to resume bot {self.bot_config.bot_id}: {e}")
            self._state = BotStatus.ERROR
            raise
    ...
    async def execute_trade(self, order_request: OrderRequest, execution_params: dict) -> Any:
        ...
        try:
            ...
            if self._state != BotStatus.RUNNING:
                raise ValidationError(f"Bot {self.bot_config.bot_id} is not running")
            ...
            self._logger.info(f"Executed trade for bot {self.bot_config.bot_id}")
            return result

        except Exception as e:
            self._logger.error(f"Failed to execute trade for bot {self.bot_config.bot_id}: {e}")
    ...
    async def update_position(self, symbol: str, position_data: dict) -> None:
        ...
        try:
            ...
            self._logger.debug(f"Updated position for bot {self.bot_config.bot_id}, symbol {symbol}")

        except Exception as e:
            self._logger.error(f"Failed to update position for bot {self.bot_config.bot_id}: {e}")
            raise
    ...
    async def close_position(self, symbol: str, reason: str) -> bool:
        ...
        try:
            if symbol in self._positions:
                ...
                self._logger.info(f"Closed position for bot {self.bot_config.bot_id}, symbol {symbol}, reason: {reason}")
                return True
            else:
                self._logger.warning(f"No position found for bot {self.bot_config.bot_id}, symbol {symbol}")
                return False

        except Exception as e:
            self._logger.error(f"Failed to close position for bot {self.bot_config.bot_id}: {e}")
            return False
    ...
    async def restart(self, reason: str) -> None:
        ...
        """
        try:
            self._logger.info(f"Restarting bot {self.bot_config.bot_id}, reason: {reason}")

            # Stop then start
            await self.stop()
            await self.start()

            self._logger.info(f"Successfully restarted bot {self.bot_config.bot_id}")

        except Exception as e:
            self._logger.error(f"Failed to restart bot {self.bot_config.bot_id}: {e}")
            self._state = BotStatus.ERROR
            raise
    ...
    async def queue_websocket_message(self, message: dict) -> bool:
        ...
        try:
            ...
            self._logger.debug(f"Queued websocket message for bot {self.bot_config.bot_id}")
            return True

        except Exception as e:
            self._logger.error(f"Failed to queue websocket message for bot {self.bot_config.bot_id}: {e}")
            return False
