...
class PerformanceMetrics(BaseModel):
    ...
    def get_quality_score(self) -> Decimal:
        ...
        quality_components = [
            min(self.sharpe_ratio / Decimal("2"), Decimal("1")) if self.sharpe_ratio > 0 else Decimal("0"),
            min(self.sortino_ratio / Decimal("3"), Decimal("1")) if self.sortino_ratio > 0 else Decimal("0"),
            self.win_rate,
            min(self.profit_factor / Decimal("2"), Decimal("1")) if self.profit_factor > 0 else Decimal("0"),
...
class SensitivityAnalysis(BaseModel):
    ...
    p_value: Optional[Decimal] = Field(
        default=None, 
        description="Statistical significance of parameter effect"
    )
    confidence_interval: Optional[Tuple[Decimal, Decimal]] = Field(
        default=None, 
        description="Confidence interval for parameter effect"
    )
    
    # Practical metrics
    optimal_range: Tuple[Decimal, Decimal] = Field(
        description="Optimal parameter range"
    )
    ...
    interaction_partners: List[str] = Field(
        ...
        description="Parameters with significant interaction effects"
    )
    interaction_strength: Dict[str, Decimal] = Field(
        default_factory=dict,
        description="Strength of interaction with other parameters"
    )
...
class StabilityAnalysis(BaseModel):
    ...
    # Temporal stability
    period_consistency: Decimal = Field(
        description="Consistency across different time periods"
    )
    regime_consistency: Decimal = Field(
    ...
    performance_stability: Decimal = Field(
        description="Stability of performance metrics"
    )
    
    # Market condition analysis
    bull_market_performance: Optional[Decimal] = Field(
        default=None,
        description="Performance during bull markets"
    )
    bear_market_performance: Optional[Decimal] = Field(
        default=None,
        description="Performance during bear markets"
    )
    sideways_market_performance: Optional[Decimal] = Field(
        default=None,
        description="Performance during sideways markets"
    )
    
    # Volatility regime analysis
    low_vol_performance: Optional[Decimal] = Field(
        default=None,
        description="Performance during low volatility periods"
    )
    high_vol_performance: Optional[Decimal] = Field(
        default=None,
    ...
    crisis_resilience: Decimal = Field(
        description="Resilience during market crises"
    )
    
    def get_overall_stability_score(self) -> Decimal:
        """Calculate overall stability score."""
        components = [
            self.period_consistency,
        ...
        if valid_market_components:
            ...
            if len(valid_market_components) > 1:
                market_std = Decimal(str(statistics.stdev([float(c) for c in valid_market_components])))
                market_mean = sum(valid_market_components) / len(valid_market_components)
                if market_mean != 0:
                    market_consistency = Decimal("1") / (Decimal("1") + abs(market_std / market_mean))
                else:
...
class ParameterImportanceAnalyzer:
    ...
    def _analyze_single_parameter(
        ...
        try:
            ...
            combined_data = list(zip(param_values, performance_values))
            ...
            logger.warning(f"Failed to analyze parameter {param_name}: {str(e)}")
            return None
    ...
    def _find_interaction_partners(
        ...
        for other_param, other_values in all_parameter_data.items():
            ...
            try:
                ...
                interaction_values = [a * b for a, b in zip(target_param_values, other_values)]
                ...
                logger.debug(f"Failed to calculate interaction between {param_name} and {other_param}: {str(e)}")
...
class ResultsAnalyzer:
    ...
    def _analyze_performance_distribution(
        ...
        return {
            ...
            'q25': statistics.quantiles(performance_values, n=4)[0] if len(performance_values) >= 4 else min(performance_values),
            'q75': statistics.quantiles(performance_values, n=4)[2] if len(performance_values) >= 4 else max(performance_values),
    ...
    def _calculate_parameter_correlations(
        ...
        for param1 in parameter_names:
            ...
            for param2 in parameter_names:
                if (param1 in parameter_data and param2 in parameter_data and
                    len(parameter_data[param1]) == len(parameter_data[param2]) and
                    len(parameter_data[param1]) > 1):
    ...
    def _detect_performance_plateaus(self, sorted_performance: List[float]) -> Dict[str, Any]:
        ...
        tolerance = statistics.stdev(sorted_performance) * 0.1 if len(sorted_performance) > 1 else 0.01
    ...
    def _analyze_convergence(self, optimization_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'improvement_curve': running_max[-20:] if len(running_max) >= 20 else running_max  # Last 20 points
        }
    ...
    def _create_analysis_summary(
        ...
        if 'landscape_analysis' in analysis_results:
            ...
            summary['landscape_characteristics'] = {
                ...
                'has_multiple_modes': landscape.get('multimodality', {}).get('has_multiple_modes', False),
                'improvement_potential': landscape.get('improvement_potential', {}).get('improvement_potential', 'unknown')
            }
