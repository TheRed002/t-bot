...
class ParameterDefinition(BaseModel, ABC):
    ...
    conditions: Dict[str, Any] = Field(
        ...
        description="Conditions for parameter activation"
    )
...
class ContinuousParameter(ParameterDefinition):
    ...
    precision: Optional[int] = Field(
        default=None, 
        ge=0, 
        description="Decimal precision (None for arbitrary precision)"
    )
    log_scale: bool = Field(
        default=False, 
        description="Whether parameter should be sampled on log scale"
    )
    default_value: Optional[Decimal] = Field(
        default=None, 
...
class CategoricalParameter(ParameterDefinition):
    ...
    weights: Optional[List[float]] = Field(
        default=None, 
        description="Sampling weights for choices (must match choices length)"
    )
...
class BooleanParameter(ParameterDefinition):
    ...
    true_probability: float = Field(
        ...
        ge=0.0, 
        le=1.0, 
        description="Probability of sampling True"
    )
...
class ConditionalParameter(ParameterDefinition):
    ...
    base_parameter: ParameterDefinition = Field(
...
class ParameterSpace(BaseModel):
    ...
    constraints: List[str] = Field(
        ...
        description="List of parameter constraints (expressions)"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, 
        description="Parameter space metadata"
    )
...
class ParameterSpaceBuilder:
    ...
    def add_continuous(
        ...
        default_value: Optional[Union[float, Decimal]] = None,
        description: str = "",
        **kwargs
