...
class OptimizationObjective(BaseModel):
    ...
    target_value: Decimal | None = Field(
        default=None, description="Target value for objective (if applicable)"
    )
    constraint_min: Decimal | None = Field(default=None, description="Minimum constraint value")
    constraint_max: Decimal | None = Field(default=None, description="Maximum constraint value")
    description: str = Field(default="", description="Objective description")
    is_primary: bool = Field(default=False, description="Whether this is the primary objective")
    ...
    @classmethod
    def validate_constraints(cls, v: Decimal | None) -> Decimal | None:
...
class OptimizationConstraint(BaseModel):
    ...
    is_hard: bool = Field(
        default=True, description="Whether this is a hard constraint (vs soft constraint)"
    )
...
class OptimizationProgress(BaseModel):
    ...
    # Performance metrics
    best_objective_value: Decimal | None = Field(
        default=None, description="Best objective value found so far"
    )
    current_objective_value: Decimal | None = Field(
        default=None, description="Current objective value"
    )
    evaluations_completed: int = Field(
        default=0, ge=0, description="Number of evaluations completed"
    )

    # Timing information
    start_time: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Optimization start time"
    )
    last_update_time: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc), description="Last progress update time"
    )
    ...
    current_parameters: dict[str, Any] = Field(
        default_factory=dict, description="Current best parameters"
    )
    ...
    def update_progress(
        ...
        if self.total_iterations > 0:
            self.completion_percentage = Decimal(str((iteration / self.total_iterations) * 100))
    ...
    def estimate_completion_time(self) -> None:
        ...
        self.estimated_completion_time = datetime.now(timezone.utc) + timedelta(
...
class OptimizationConfig(BaseModel):
    ...
    max_evaluations: int = Field(
        default=10000, ge=1, description="Maximum number of function evaluations"
    )
    timeout_seconds: int | None = Field(
        default=None, ge=1, description="Maximum optimization time in seconds"
    )
    ...
    parallel_evaluations: bool = Field(
        default=True, description="Enable parallel evaluation of candidates"
    )
    max_workers: int | None = Field(
        default=None, ge=1, description="Maximum number of parallel workers"
    )

    # Validation and robustness
    validation_enabled: bool = Field(
        default=True, description="Enable validation during optimization"
    )
    ...
    log_level: str = Field(default="INFO", description="Logging level for optimization")
    progress_callback_interval: int = Field(
        default=10, ge=1, description="Interval for progress callbacks"
    )
    save_intermediate_results: bool = Field(
        default=True, description="Save intermediate optimization results"
    )

    # Resource management
    memory_limit_mb: int | None = Field(default=None, ge=1, description="Memory limit in megabytes")
    disk_space_limit_mb: int | None = Field(
        default=None, ge=1, description="Disk space limit in megabytes"
    )

    # Trading-specific settings
    trading_mode: TradingMode = Field(
        default=TradingMode.BACKTEST, description="Trading mode for optimization"
    )
    preserve_decimal_precision: bool = Field(
        default=True, description="Preserve decimal precision in calculations"
    )
...
class OptimizationResult(BaseModel):
    ...
    # Results
    optimal_parameters: dict[str, Any] = Field(description="Optimal parameters found")
    optimal_objective_value: Decimal = Field(description="Optimal objective value achieved")
    objective_values: dict[str, Decimal] = Field(
        description="All objective values at optimal point"
    )

    # Validation results
    validation_score: Decimal | None = Field(
        default=None, description="Validation score (out-of-sample performance)"
    )
    overfitting_score: Decimal | None = Field(
        default=None, description="Overfitting detection score"
    )
    robustness_score: Decimal | None = Field(default=None, description="Robustness analysis score")

    # Optimization statistics
    iterations_completed: int = Field(ge=0, description="Number of iterations completed")
    evaluations_completed: int = Field(ge=0, description="Number of function evaluations completed")
    ...
    parameter_stability: dict[str, Decimal] = Field(
        default_factory=dict, description="Parameter stability scores"
    )
    sensitivity_analysis: dict[str, Decimal] = Field(
        default_factory=dict, description="Parameter sensitivity analysis"
    )

    # Additional metadata
    warnings: list[str] = Field(
        default_factory=list, description="Warnings encountered during optimization"
    )
    config_used: dict[str, Any] = Field(description="Configuration used for optimization")

    # Statistical significance
    statistical_significance: Decimal | None = Field(
        default=None, description="Statistical significance of results"
    )
    confidence_interval: tuple[Decimal, Decimal] | None = Field(
        default=None, description="Confidence interval for optimal value"
    )

    def is_statistically_significant(self, significance_level: Decimal = Decimal("0.05")) -> bool:
        """
    ...
    def update_progress(
        ...
        if iteration % self.config.progress_callback_interval == 0:
            logger.info(
                ...
                best_value=(
                    float(self.progress.best_objective_value)
                    if self.progress.best_objective_value
                    else None
                ),
            )
