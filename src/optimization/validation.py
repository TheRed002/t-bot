...
class ValidationMetrics(BaseModel):
    ...
    p_value: Decimal | None = Field(default=None, description="Statistical significance p-value")
    confidence_interval: tuple[Decimal, Decimal] | None = Field(
        default=None, description="Confidence interval for out-of-sample performance"
    )

    # Robustness metrics
    stability_score: Decimal = Field(description="Parameter stability across validation folds")
    robustness_score: Decimal = Field(description="Performance robustness to parameter changes")
    ...
    walk_forward_scores: list[Decimal] = Field(
        default_factory=list, description="Performance scores from walk-forward analysis"
    )
    regime_consistency: Decimal | None = Field(
        default=None, description="Consistency across different market regimes"
    )
    ...
    is_robust: bool = Field(description="Whether results pass robustness tests")
    has_overfitting: bool = Field(description="Whether overfitting is detected")
...
class ValidationConfig(BaseModel):
    ...
    cv_method: str = Field(default="time_series", description="Cross-validation method")
    cv_folds: int = Field(default=5, ge=2, description="Number of cross-validation folds")
    cv_gap: int = Field(default=0, ge=0, description="Gap between train and test in CV (days)")

    # Walk-forward analysis
    enable_walk_forward: bool = Field(default=True, description="Enable walk-forward analysis")
    walk_forward_periods: int = Field(
        default=12, ge=2, description="Number of walk-forward periods"
    )
    walk_forward_step_days: int = Field(
        default=30, ge=1, description="Step size for walk-forward analysis"
    )
    walk_forward_window_type: str = Field(
        default="expanding", description="Window type: 'expanding' or 'rolling'"
    )
    walk_forward_min_train_days: int = Field(
        default=180, ge=30, description="Minimum training period for walk-forward"
    )
    ...
    multiple_testing_correction: str = Field(
        default="bonferroni", description="Multiple testing correction method"
    )
    bootstrap_samples: int = Field(
        default=10000, ge=1000, description="Number of bootstrap samples"
    )
    ...
    monte_carlo_runs: int = Field(
        default=1000, ge=100, description="Number of Monte Carlo simulation runs"
    )
...
class TimeSeriesValidator:
    ...
    def __init__(self, config: ValidationConfig):
        ...
        logger.info(
            "TimeSeriesValidator initialized", cv_method=config.cv_method, cv_folds=config.cv_folds
        )

    def create_time_series_splits(
        self, data_length: int, start_date: datetime, end_date: datetime
        ...
        for period in range(self.config.walk_forward_periods):
            ...
            try:
                ...
                score = await self._evaluate_period(
                    objective_function, parameters, train_start, train_end, test_start, test_end
                )
...
class OverfittingDetector:
    ...
    def detect_overfitting(
        ...
        has_overfitting = (
            degradation > self.config.overfitting_threshold
            or overfitting_ratio > (Decimal("1") + self.config.overfitting_threshold)
            or cv > Decimal("0.5")  # High variability in validation scores
        )
...
class StatisticalTester:
    ...
    def _bootstrap_confidence_interval(self, data: list[Decimal]) -> tuple[Decimal, Decimal]:
    ...
    def _two_sample_test(self, data1: list[Decimal], data2: list[Decimal]) -> Decimal:
...
class RobustnessAnalyzer:
    ...
    def _calculate_parameter_sensitivities(
        self, parameter_sensitivities: dict[str, list[tuple[Any, Decimal]]]
    ) -> dict[str, Decimal]:
    ...
    async def _evaluate_in_out_sample(
        ...
        try:
            ...
            in_sample_params.update(
                {"start_date": start_date, "end_date": split_date, "validation_mode": "in_sample"}
            )
        ...
        try:
            ...
            oos_params.update(
                {"start_date": split_date, "end_date": end_date, "validation_mode": "out_of_sample"}
            )
