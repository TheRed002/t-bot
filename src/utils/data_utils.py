...
class DataUtils:
    """All data conversion operations."""
    
    @staticmethod
    def dict_to_dataframe(data: Union[Dict, List[Dict]]) -> pd.DataFrame:
        """
        ...
        try:
            ...
            raise ValidationError(f"Failed to convert to DataFrame: {str(e)}")
    
    @staticmethod
    def normalize_array(arr: Union[List, np.ndarray]) -> np.ndarray:
        """
    ...
    @staticmethod
    def normalize_price(price: Union[float, Decimal], symbol: str) -> Decimal:
        """
