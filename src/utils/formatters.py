...
def format_performance_log(function_name: str, execution_time_ms: float, 
...
def format_indicator_data(indicator_name: str, values: List[float], 
...
def format_chart_data(symbol: str, ohlcv_data: List[Dict[str, Any]], 
...
def format_trade_report(trades: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    total_volume = sum(float(trade.get("quantity", 0)) * float(trade.get("price", 0)) 
                      for trade in trades)
