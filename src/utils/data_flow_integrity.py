...
from src.monitoring.financial_precision import (
    ...
    validate_financial_range as precision_validate_financial_range,
...
class PrecisionTracker:
    ...
    def track_conversion(
        ...
        if event["relative_error"] > 0.0001:  # > 0.01%
            logger.warning(
...
def get_precision_tracker() -> PrecisionTracker:
    """Get the global precision tracker instance."""
    return _precision_tracker
...
class DataFlowValidator:
    ...
    def _get_rule_for_field(self, field_name: str) -> Optional[Dict[str, Any]]:
...
class IntegrityPreservingConverter:
    ...
    def __init__(self, track_precision: bool = True):
        self.track_precision = track_precision
        self.tracker = get_precision_tracker() if track_precision else None
    ...
    def safe_convert_for_metrics(
        ...
        try:
            ...
            result = safe_decimal_to_float(
                value, metric_name, precision_digits, warn_on_loss=True
            )
            ...
            if fallback_strategy == "error":
                ...
                warnings.warn(f"Conversion failed for {metric_name}: {e}", FinancialPrecisionWarning)
                return 0.0
            else:  # silent
                return 0.0
    ...
    def batch_convert_with_integrity(
        self,
        data: Dict[str, Union[Decimal, float, int]],
...
# Global instances
_data_flow_validator = DataFlowValidator()
_integrity_converter = IntegrityPreservingConverter()


def get_data_flow_validator() -> DataFlowValidator:
    """Get the global data flow validator."""
    return _data_flow_validator


def get_integrity_converter() -> IntegrityPreservingConverter:
    """Get the global integrity-preserving converter."""
    return _integrity_converter
...
def fix_precision_cascade(
    ...
    for field_name, value in data.items():
        ...
        is_financial = any(term in field_name.lower() for term in 
                          ["price", "amount", "value", "cost", "fee", "balance", "pnl", "volume"])
