...
async def validate_required_fields_with_details(
    data: dict[str, Any], required_fields: list[str]
) -> dict[str, Any]:
    """
    Validate required fields and return detailed results with consistent pattern.
    ...
    """
    # Apply consistent data transformation and boundary validation first
    from src.utils.messaging_patterns import BoundaryValidator, MessagingCoordinator

    coordinator = MessagingCoordinator("StateValidationUtils")

    # Transform data using consistent patterns
    transformed_data = coordinator._apply_data_transformation(data)

    # Apply boundary validation for module consistency
    boundary_data = {
        "component": "StateValidationUtils",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "processing_mode": "stream",
        "message_pattern": "pub_sub",
        "data_format": "validation_request_v1",
    ...
    # Use centralized validation from state_utils
    from .state_utils import validate_required_fields

    missing_fields = await validate_required_fields(transformed_data, required_fields)
    ...
    return {
        ...
        "processing_mode": "stream",
        "message_pattern": "pub_sub",
        "data_format": "validation_response_v1",
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }


def validate_string_field_with_details(data: dict[str, Any], field_name: str) -> dict[str, Any]:
    """
...
async def validate_decimal_field_with_details(
    data: dict[str, Any], field_name: str, max_places: int = 8
) -> dict[str, Any]:
    """
    Validate that a field is a valid decimal with detailed results and consistent data transformation.
    ...
    Returns:
        Dictionary with validation results
    """
    # Apply consistent data transformation for financial precision
    ...
    try:
        ...
        # Use transformed decimal if available, otherwise convert
        ...
        if isinstance(exponent, str):
            return {
                "passed": False,
                "message": f"{field_name} contains invalid special value: {exponent}",
                "current_value": value,
                "decimal_places": 0,
                "processing_mode": "stream",
                "data_format": "validation_response_v1",
...
def validate_positive_value_with_details(data: dict[str, Any], field_name: str) -> dict[str, Any]:
    """
...
def validate_list_field_with_details(data: dict[str, Any], field_name: str) -> dict[str, Any]:
    """
...
def validate_dict_field_with_details(data: dict[str, Any], field_name: str) -> dict[str, Any]:
    """
...
def validate_bot_id_format(bot_id: str) -> dict[str, Any]:
    ...
    """
    if not bot_id:
        return {"passed": False, "message": "Bot ID cannot be empty", "current_value": bot_id}
    ...
    return {
        "passed": is_valid,
        "message": "Valid bot ID format"
        if is_valid
        else "Bot ID must contain only letters, numbers, dashes, and underscores",
...
def validate_symbol_format(symbol: str) -> dict[str, Any]:
    ...
    """
    if not symbol:
        return {"passed": False, "message": "Symbol cannot be empty", "current_value": symbol}
    ...
    return {
        ...
        if is_valid
        else "Symbol must be in valid format (e.g., BTC/USD, BTCUSD)",
...
def validate_capital_allocation(
    data: dict[str, Any], max_allocation: Decimal | None = None
) -> dict[str, Any]:
    """
...
def validate_cash_balance(
    data: dict[str, Any], min_cash_ratio: Decimal | None = None
) -> dict[str, Any]:
    """
...
def validate_var_limits(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if var_value is None:
        return {"passed": True, "message": "No VaR specified", "current_value": var_value}
...
def validate_trade_execution(data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "current_value": execution_data,
            "expected_types": {
                "filled_quantity": "non-negative number",
                "average_price": "positive number",
            },
        }
