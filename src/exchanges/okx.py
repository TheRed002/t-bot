...
class OKXExchange(BaseExchange):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to connect to OKX exchange: {str(e)}")
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error disconnecting from OKX exchange: {str(e)}")
            raise ExchangeConnectionError(
                f"Error disconnecting from OKX: {str(e)}")
    ...
    async def get_account_balance(self) -> Dict[str, Decimal]:
        ...
        try:
            ...
            raise ExchangeError(
                f"Failed to get account balance from OKX: {
                    str(e)}")
    ...
    async def get_market_data(
        ...
        try:
            ...
            logger.debug(
                f"Retrieved market data for {symbol}: {
                    market_data.price}")
            return market_data

        except Exception as e:
            logger.error(
                f"Failed to get market data for {symbol} from OKX: {
                    str(e)}")
            raise ExchangeError(
                f"Failed to get market data from OKX: {
                    str(e)}")
    ...
    async def subscribe_to_stream(
        ...
        try:
            ...
            logger.error(
                f"Failed to subscribe to stream for {symbol} on OKX: {
                    str(e)}")
            raise ExchangeError(
                f"Failed to subscribe to stream on OKX: {
                    str(e)}")
    ...
    async def get_order_book(self, symbol: str, depth: int = 10) -> OrderBook:
        ...
        try:
            ...
            result = self.public_client.get_orderbook(
                instId=symbol,
                sz=depth
            )
            ...
            logger.debug(
                f"Retrieved order book for {symbol}: {
                    len(bids)} bids, {
                    len(asks)} asks")
            return order_book
            ...
            logger.error(
                f"Failed to get order book for {symbol} from OKX: {
                    str(e)}")
    ...
    async def get_trade_history(
        ...
        try:
            ...
            result = self.public_client.get_trades(
                instId=symbol,
                limit=limit
            )
            ...
            logger.debug(
                f"Retrieved {
                    len(trades)} trades for {symbol} from OKX")
            return trades

        except Exception as e:
            logger.error(
                f"Failed to get trade history for {symbol} from OKX: {
                    str(e)}")
            raise ExchangeError(
                f"Failed to get trade history from OKX: {
                    str(e)}")
    ...
    async def get_exchange_info(self) -> ExchangeInfo:
        ...
        try:
            ...
            logger.info(
                f"Retrieved exchange info from OKX: {
                    len(supported_symbols)} symbols")
            return exchange_info

        except Exception as e:
            logger.error(f"Failed to get exchange info from OKX: {str(e)}")
            raise ExchangeError(
                f"Failed to get exchange info from OKX: {
                    str(e)}")
    ...
    async def get_ticker(self, symbol: str) -> Ticker:
        ...
        try:
            ...
            logger.debug(
                f"Retrieved ticker for {symbol} from OKX: {
                    ticker.last_price}")
            return ticker

        except Exception as e:
            logger.error(
                f"Failed to get ticker for {symbol} from OKX: {
                    str(e)}")
            raise ExchangeError(f"Failed to get ticker from OKX: {str(e)}")
    ...
    async def _initialize_websocket(
            self,
            stream_name: str,
            symbol: str) -> None:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to initialize WebSocket for {stream_name} on OKX: {
                    str(e)}")
            raise ExchangeConnectionError(
                f"Failed to initialize WebSocket on OKX: {str(e)}")
    ...
    async def _handle_stream(self, stream_name: str, stream) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error handling stream {stream_name} on OKX: {
                    str(e)}")
    ...
    async def _close_stream(self, stream_name: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error closing stream {stream_name} on OKX: {
                    str(e)}")
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Health check failed for OKX: {str(e)}")
            return False
