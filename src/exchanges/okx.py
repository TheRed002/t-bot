...
class OKXExchange(BaseExchange):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.status = "error"
            raise ExchangeConnectionError(f"Failed to connect to OKX: {str(e)}")
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            raise ExchangeConnectionError(f"Error disconnecting from OKX: {str(e)}")
    ...
    async def get_market_data(self, symbol: str, timeframe: str = "1m") -> MarketData:
        ...
        try:
            ...
            if result.get('code') != '0':
                raise ExchangeError(f"Failed to get market data: {result.get('msg', 'Unknown error')}")
            ...
            logger.error(f"Failed to get market data for {symbol} from OKX: {str(e)}")
            raise ExchangeError(f"Failed to get market data from OKX: {str(e)}")
    ...
    async def get_order_book(self, symbol: str, depth: int = 10) -> OrderBook:
        ...
        try:
            ...
            if result.get('code') != '0':
                raise ExchangeError(f"Failed to get order book: {result.get('msg', 'Unknown error')}")
    ...
    async def get_trade_history(self, symbol: str, limit: int = 100) -> List[Trade]:
        ...
        try:
            ...
            if result.get('code') != '0':
                raise ExchangeError(f"Failed to get trade history: {result.get('msg', 'Unknown error')}")
            ...
            for trade_data in data:
                trade = Trade(
                    ...
                    fee=Decimal('0')  # OKX doesn't provide fee in trade history
                )
                trades.append(trade)
    ...
    async def get_exchange_info(self) -> ExchangeInfo:
        ...
        try:
            ...
            if result.get('code') != '0':
                raise ExchangeError(f"Failed to get exchange info: {result.get('msg', 'Unknown error')}")
    ...
    async def get_ticker(self, symbol: str) -> Ticker:
        ...
        try:
            ...
            logger.error(f"Failed to get ticker for {symbol} from OKX: {str(e)}")
    ...
    async def _initialize_websocket(self, stream_name: str, symbol: str) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to initialize WebSocket for {stream_name} on OKX: {str(e)}")
            raise ExchangeConnectionError(f"Failed to initialize WebSocket on OKX: {str(e)}")
    ...
    async def _handle_stream(self, stream_name: str, stream) -> None:
        ...
        try:
            ...
            logger.error(f"Error handling stream {stream_name} on OKX: {str(e)}")
    ...
    async def _close_stream(self, stream_name: str) -> None:
        ...
        try:
            ...
            logger.error(f"Error closing stream {stream_name} on OKX: {str(e)}")
