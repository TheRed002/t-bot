...
class ExchangeFactory:
    ...
    def register_exchange(self, exchange_name: str, exchange_class: Type[BaseExchange]) -> None:
        ...
        if not issubclass(exchange_class, BaseExchange):
            raise ValidationError(f"Exchange class must inherit from BaseExchange")
    ...
    async def get_exchange(self, exchange_name: str, create_if_missing: bool = True) -> Optional[BaseExchange]:
        """
        ...
        if exchange_name in self._active_exchanges:
            ...
            if await exchange.health_check():
                ...
                logger.warning(f"Exchange {exchange_name} failed health check, removing from active pool")
        ...
        if create_if_missing:
            try:
                ...
                logger.error(f"Failed to get/create exchange {exchange_name}: {str(e)}")
                return None
    ...
    async def remove_exchange(self, exchange_name: str) -> bool:
        ...
        if exchange_name in self._active_exchanges:
            try:
                ...
                logger.error(f"Failed to remove exchange {exchange_name}: {str(e)}")
