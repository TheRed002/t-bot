...
class BinanceWebSocketHandler:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to connect Binance WebSocket streams: {str(e)}")
            self.connected = False
            return False
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error disconnecting Binance WebSocket streams: {str(e)}")
    ...
    async def subscribe_to_ticker_stream(self, symbol: str, callback: Callable) -> None:
        ...
        try:
            ...
            raise ExchangeError(f"Failed to subscribe to ticker stream: {str(e)}")
    
    async def subscribe_to_orderbook_stream(self, symbol: str, depth: str = "20", callback: Callable = None) -> None:
        """
        ...
        try:
            ...
            raise ExchangeError(f"Failed to subscribe to orderbook stream: {str(e)}")
    ...
    async def subscribe_to_trade_stream(self, symbol: str, callback: Callable) -> None:
        ...
        try:
            ...
            raise ExchangeError(f"Failed to subscribe to trade stream: {str(e)}")
    ...
    async def subscribe_to_user_data_stream(self, callback: Callable) -> None:
        ...
        try:
            ...
            raise ExchangeError(f"Failed to subscribe to user data stream: {str(e)}")
    ...
    async def unsubscribe_from_stream(self, stream_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error unsubscribing from stream {stream_name}: {str(e)}")
            return False
            ...
            async with stream as stream_handler:
                async for msg in stream_handler:
                    try:
                        ...
                        logger.error(f"Error processing ticker message: {str(e)}")
                        
        except asyncio.CancelledError:
            logger.info(f"Ticker stream {stream_name} cancelled")
        except Exception as e:
            logger.error(f"Error handling ticker stream {stream_name}: {str(e)}")
    ...
    async def _handle_orderbook_stream(self, stream_name: str, stream) -> None:
        ...
        try:
            ...
            logger.error(f"Error handling orderbook stream {stream_name}: {str(e)}")
            await self._handle_stream_error(stream_name)
    ...
    async def _handle_trade_stream(self, stream_name: str, stream) -> None:
        ...
        try:
            ...
            logger.error(f"Error handling trade stream {stream_name}: {str(e)}")
            await self._handle_stream_error(stream_name)
    ...
    async def _handle_user_data_stream(self, stream_name: str, stream) -> None:
        ...
        try:
            ...
            logger.error(f"Error handling user data stream {stream_name}: {str(e)}")
            await self._handle_stream_error(stream_name)
    ...
    def _convert_trade_message(self, msg: Dict) -> Trade:
        ...
        return Trade(
            id=str(msg['t']),
            symbol=msg['s'],
            side=OrderSide.BUY if msg['m'] else OrderSide.SELL,  # m=True means maker is seller
    ...
    async def _handle_stream_error(self, stream_name: str) -> None:
        ...
        try:
            ...
            if self.reconnect_attempts < self.max_reconnect_attempts:
                ...
                logger.info(f"Attempting to reconnect stream {stream_name} (attempt {self.reconnect_attempts})")
