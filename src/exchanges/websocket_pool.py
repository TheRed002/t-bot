...
class WebSocketConnectionPool:
    ...
    def __init__(
        ...
        self.connection_pools: Dict[ConnectionType,
                                    List[PooledConnection]] = defaultdict(list)
    ...
    async def get_connection(
            self,
            connection_type: ConnectionType) -> Optional[PooledConnection]:
        """
    ...
    def _create_websocket_connection(
            self, connection_type: ConnectionType) -> Any:
        """
    ...
    def _is_connection_healthy(self, connection: PooledConnection) -> bool:
        ...
        try:
            ...
            if not self._check_subscription_limit(connection):
                logger.debug(
                    "Connection exceeded subscription limit",
                    connection_id=connection.connection_id)
                return False
    ...
    def _check_subscription_limit(self, connection: PooledConnection) -> bool:
        ...
        """
        try:
            current_subscriptions = self.subscription_counters.get(
    ...
    async def _remove_connection(self, connection: PooledConnection) -> None:
        ...
        try:
            ...
            if connection in self.connection_pools[connection.connection_type]:
                self.connection_pools[connection.connection_type].remove(
                    connection)
    ...
    def get_connection_stats(self, connection_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            subscription_count = self.subscription_counters.get(
                connection_id, 0)
        ...
        try:
            ...
            for connection_type, connections in self.connection_pools.items():
                ...
                for connection in connections[:]:
                    # Check if connection is too old
                    if now - \
                            connection.created_at > timedelta(seconds=self.connection_timeout):
                        logger.info(
                    ...
                    # Check if connection has been inactive for too long
                    if now - \
                            connection.last_used > timedelta(seconds=self.connection_timeout):
                        logger.info(
    ...
    async def close_all_connections(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Closed all connections in pool",
                exchange=self.exchange)
        ...
        try:
            if max_connections is not None:
                ...
                logger.info(
                    ...
                    new_limit=max_connections)

            if max_messages_per_second is not None:
                if max_messages_per_second <= 0:
                    raise ValidationError(
                        "Max messages per second must be positive")
                self.max_messages_per_second = max_messages_per_second
                logger.info(
                    "Updated max messages per second",
                    new_limit=max_messages_per_second)
            ...
            if max_subscriptions is not None:
                ...
                self.max_subscriptions = max_subscriptions
                logger.info(
                    "Updated max subscriptions",
                    new_limit=max_subscriptions)
