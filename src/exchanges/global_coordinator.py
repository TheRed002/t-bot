...
class GlobalRateCoordinator:
    ...
    def __init__(self, config: Config):
        ...
        logger.debug("GlobalRateCoordinator initialized", 
    ...
    async def check_global_limits(self, request_type: str, count: int = 1) -> bool:
        ...
        try:
            ...
            logger.error(f"Global rate limit check failed: request_type {request_type}, count {count}, error {str(e)}")
    ...
    async def coordinate_request(self, exchange: str, endpoint: str, request_type: str) -> bool:
        ...
        try:
            ...
            if not await self.check_global_limits(request_type, 1):
                logger.warning(f"Global rate limit exceeded: exchange {exchange}, endpoint {endpoint}, request_type {request_type}")
                return False
            
            # Check exchange-specific limits
            if not await self._check_exchange_specific_limits(exchange, endpoint, request_type):
                logger.warning(f"Exchange-specific rate limit exceeded: exchange {exchange}, endpoint {endpoint}, request_type {request_type}")
                return False
            
            # Record exchange usage
            self._record_exchange_usage(exchange, endpoint, request_type)
            
            logger.debug(f"Request coordination successful: exchange {exchange}, endpoint {endpoint}, request_type {request_type}")
            return True
            
        except (ValidationError, ExchangeRateLimitError):
            raise
        except Exception as e:
            logger.error(f"Request coordination failed: exchange {exchange}, endpoint {endpoint}, request_type {request_type}, error {str(e)}")
            raise ExchangeRateLimitError(f"Request coordination failed: {str(e)}")
    ...
    async def _check_order_limits(self, count: int) -> bool:
        ...
        if len(self.order_history) + count > self.global_limits["orders_per_minute"]:
            logger.warning(f"Global order limit exceeded: {len(self.order_history)} current orders, {count} new orders, limit {self.global_limits['orders_per_minute']}")
            return False
    ...
    async def _check_connection_limits(self, count: int) -> bool:
        ...
        if len(self.connection_history) + count > self.global_limits["concurrent_connections"]:
            logger.warning(f"Global connection limit exceeded: {len(self.connection_history)} current connections, {count} new connections, limit {self.global_limits['concurrent_connections']}")
            return False
    ...
    async def _check_general_request_limits(self, count: int) -> bool:
        ...
        self.request_history["general"] = [
            t for t in self.request_history["general"] 
            if t > minute_ago
        ]
        
        # Check if adding these requests would exceed limit
        if len(self.request_history["general"]) + count > self.global_limits["total_requests_per_minute"]:
            logger.warning(f"Global request limit exceeded: {len(self.request_history['general'])} current requests, {count} new requests, limit {self.global_limits['total_requests_per_minute']}")
            return False
        
        return True
    
    async def _check_exchange_specific_limits(self, exchange: str, endpoint: str, request_type: str) -> bool:
        """Check exchange-specific limits."""
    ...
    def _record_exchange_usage(self, exchange: str, endpoint: str, request_type: str) -> None:
        ...
        if len(self.exchange_usage[exchange][endpoint]) > 1000:
            self.exchange_usage[exchange][endpoint] = \
                self.exchange_usage[exchange][endpoint][-1000:]
    ...
    async def wait_for_global_capacity(self, request_type: str, count: int = 1) -> float:
        ...
        try:
            ...
            if wait_time > 0:
                logger.info("Waiting for global capacity", 
                           request_type=request_type, count=count, wait_time=wait_time)
            ...
            logger.error("Global capacity wait failed", 
                        request_type=request_type, count=count, error=str(e))
            raise ExchangeRateLimitError(f"Global capacity wait failed: {str(e)}")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        connection_usage_pct = (len(recent_connections) / self.global_limits["concurrent_connections"]) * 100
        request_usage_pct = (len(recent_requests) / self.global_limits["total_requests_per_minute"]) * 100
        
        return {
            "status": "healthy" if max(order_usage_pct, connection_usage_pct, request_usage_pct) < 90 else "warning",
