...
class GlobalRateCoordinator:
    ...
    async def check_global_limits(
        ...
        try:
            ...
            logger.error(
                f"Global rate limit check failed: request_type {request_type}, count {count}, error {
                    str(e)}")
            raise ExchangeRateLimitError(
                f"Global rate limit check failed: {str(e)}")
        ...
        try:
            ...
            if not exchange or not endpoint or not request_type:
                raise ValidationError(
                    "Exchange, endpoint, and request_type are required")
            ...
            logger.error(
                f"Request coordination failed: exchange {exchange}, endpoint {endpoint}, request_type {request_type}, error {
                    str(e)}")
            raise ExchangeRateLimitError(
                f"Request coordination failed: {str(e)}")
    ...
    async def _check_order_limits(self, count: int) -> bool:
        ...
        if len(self.order_history) + \
                count > self.global_limits["orders_per_minute"]:
            logger.warning(
                f"Global order limit exceeded: {
                    len(
                        self.order_history)} current orders, {count} new orders, limit {
                    self.global_limits['orders_per_minute']}")
            return False
    ...
    async def _check_connection_limits(self, count: int) -> bool:
        ...
        self.connection_history = [
            t for t in self.connection_history if t > minute_ago]

        # Check if adding these connections would exceed limit
        if len(self.connection_history) + \
                count > self.global_limits["concurrent_connections"]:
            logger.warning(
                f"Global connection limit exceeded: {
                    len(
                        self.connection_history)} current connections, {count} new connections, limit {
    ...
    async def _check_general_request_limits(self, count: int) -> bool:
        ...
        if len(self.request_history["general"]) + \
                count > self.global_limits["total_requests_per_minute"]:
            logger.warning(
                f"Global request limit exceeded: {
                    len(
                        self.request_history['general'])} current requests, {count} new requests, limit {
    ...
    async def wait_for_global_capacity(
            self,
            request_type: str,
            count: int = 1) -> float:
        """
        ...
        try:
            ...
            raise ExchangeRateLimitError(
                f"Global capacity wait failed: {str(e)}")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        order_usage_pct = (
            len(recent_orders) / self.global_limits["orders_per_minute"]) * 100
        connection_usage_pct = (
            len(recent_connections) / self.global_limits["concurrent_connections"]) * 100
        request_usage_pct = (
            len(recent_requests) / self.global_limits["total_requests_per_minute"]) * 100

        return {
            "status": "healthy" if max(
                order_usage_pct,
                connection_usage_pct,
                request_usage_pct) < 90 else "warning",
            "order_usage_percent": round(
                order_usage_pct,
                2),
            "connection_usage_percent": round(
                connection_usage_pct,
                2),
            "request_usage_percent": round(
                request_usage_pct,
                2),
            "timestamp": now.isoformat()}
