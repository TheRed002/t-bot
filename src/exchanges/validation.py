...
class OrderValidationMixin:
    ...
    def validate_symbol_trading_rules(
        ...
        if not symbol_info:
            logger.warning(f"No trading rules found for {symbol}")
            return
        ...
        if max_qty and quantity > max_qty:
            raise ValidationError(
                f"Quantity {quantity} above maximum {max_qty} for {symbol}"
            )
        ...
        if price is not None:
            ...
            if max_price and price > max_price:
                raise ValidationError(
                    f"Price {price} above maximum {max_price} for {symbol}"
                )
            ...
            if tick_size:
                ...
                if remainder != 0:
                    raise ValidationError(
                        f"Price {price} not a multiple of tick size {tick_size}"
                    )
    ...
    def validate_balance_for_order(
        ...
        if available_balance is None:
            logger.warning("Balance validation skipped - no balance provided")
            return
        ...
        if side.upper() == 'BUY':
            if order_type.upper() == 'MARKET':
                ...
                logger.warning("Cannot validate market buy order without price estimate")
                return
