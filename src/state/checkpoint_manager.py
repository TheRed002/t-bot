...
class CheckpointManager:
    ...
    async def create_checkpoint(
        ...
        try:
            ...
            if len([c for c in self.checkpoints.values() if c.bot_id == bot_id]) > self.max_checkpoints_per_bot:
                ...
                if step_name == "validate_checkpoint":
                    ...
                    if not metadata or not metadata.validated:
                        ...
                        if not validation_result["valid"]:
                            raise StateError(f"Checkpoint validation failed: {validation_result['errors']}")
    ...
    async def get_checkpoint_stats(self) -> Dict[str, Any]:
        ...
        for metadata in self.checkpoints.values():
            ...
            if (not bot_stats[metadata.bot_id]["last_checkpoint"] or
                metadata.created_at > datetime.fromisoformat(bot_stats[metadata.bot_id]["last_checkpoint"])):
    ...
    async def _get_last_checkpoint(self, bot_id: str) -> Optional[CheckpointMetadata]:
    ...
    async def _find_best_checkpoint(
        ...
        bot_checkpoints = [
            c for c in self.checkpoints.values() 
            if c.bot_id == bot_id and c.validated
        ]
        ...
        valid_checkpoints = [
            c for c in bot_checkpoints 
            if c.created_at <= target_time
        ]
        ...
        if operation == "create":
            ...
            if original_size > 0:
                ...
                self.performance_stats["compression_ratio"] = (
                    (current_ratio * (count - 1) + compression_ratio) / count
                )
                ...
                for bot_id, scheduled_time in list(self.checkpoint_schedules.items()):
                    if current_time >= scheduled_time:
                        try:
                            ...
                            next_time = current_time + timedelta(minutes=self.default_interval_minutes)
                            self.checkpoint_schedules[bot_id] = next_time
                            
                        except Exception as e:
                            self.logger.error(f"Failed to create scheduled checkpoint for bot {bot_id}: {e}")
