...
from src.core.exceptions import StateError, ConflictError, SynchronizationError
...
class StateVersion:
    ...
    def __init__(self, version: int = 1, timestamp: Optional[datetime] = None):
...
class SyncEvent:
    ...
    # Version information
    old_version: Optional[StateVersion] = None
    new_version: Optional[StateVersion] = None
...
class StateSyncManager:
    ...
    def __init__(self, config: Config):
        ...
        self.conflict_resolution_timeout_seconds = sync_config.get("conflict_resolution_timeout_seconds", 30)
    ...
    async def register_conflict_resolver(
        self,
        entity_type: str,
        resolver: Callable[[StateConflict], Dict[str, Any]]
    ) -> None:
        """
    ...
    async def force_sync(self, entity_type: str, entity_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Force sync failed: {e}", entity_type=entity_type, entity_id=entity_id)
            return False
    ...
    async def resolve_conflict_manually(
        ...
        try:
            ...
            new_version = max(conflict.version_a, conflict.version_b, key=lambda v: v.version).increment()
            ...
            if success:
                ...
                self.logger.info(f"Conflict resolved manually", conflict_id=conflict_id)
                return True
        ...
        try:
            ...
            if self._states_differ(current_cached_state, storage_state):
                ...
                conflict = StateConflict(
                    ...
                    conflicting_fields=self._find_conflicting_fields(current_cached_state, new_state),
    ...
    async def _handle_conflict(self, conflict: StateConflict) -> None:
        ...
        try:
            ...
            if conflict.resolution_strategy == ConflictResolutionStrategy.LAST_WRITE_WINS:
                ...
                self.logger.warning(f"Manual resolution required for conflict {conflict.conflict_id}")
                return
            ...
            if success:
                ...
                self.logger.info(f"Conflict resolved automatically", conflict_id=conflict.conflict_id)
    ...
    async def _sync_to_storage_layers(
        ...
        try:
            ...
            postgres_success = await self._sync_to_postgres(entity_type, entity_id, state_data, version)
    ...
    async def _load_from_redis(self, entity_type: str, entity_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _update_sync_metrics(self, success: bool, sync_time_ms: float) -> None:
        ...
        if sync_time_ms > 0:
            ...
            self.sync_metrics.average_sync_time_ms = (
                (current_avg * (total_ops - 1) + sync_time_ms) / total_ops
            )
        ...
        while True:
            try:
                ...
                if hasattr(self, '_last_metrics_time'):
                    ...
                    if time_diff > 0:
                        ops_diff = self.sync_metrics.total_sync_operations - getattr(self, '_last_ops_count', 0)
