...
from src.core.exceptions import ConfigurationError, StateError, ValidationError
...
class StateManager:
    ...
    def __init__(self, config: Config):
        ...
        self.snapshot_interval_minutes = config.state_management.get(
    ...
    async def save_bot_state(
        self, bot_id: str, bot_state: BotState, create_snapshot: bool = False
    ) -> str:
        """
        ...
        async with self._get_bot_lock(bot_id):
            try:
                ...
                await self.redis_client.setex(
                    redis_key, self.redis_ttl_seconds, json.dumps(state_data, default=str)
                )
    ...
    @time_execution
    async def load_bot_state(self, bot_id: str, version_id: str | None = None) -> BotState | None:
        """
        ...
        try:
            ...
            async with self.db_manager.get_session() as session:
                ...
                result = await session.execute(select(BotInstance).where(BotInstance.id == bot_id))
                bot_instance = result.scalar_one_or_none()
                ...
                if bot_instance and bot_instance.config:
                    ...
                    if isinstance(config_data, dict) and "bot_id" in config_data:
                        try:
                            ...
                            self.logger.warning(
                                f"Invalid database state data for bot {bot_id}: {e}"
                            )
    ...
    async def restore_from_checkpoint(self, bot_id: str, checkpoint_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "Bot state restored from checkpoint", bot_id=bot_id, checkpoint_id=checkpoint_id
            )
            return True
    ...
    async def get_state_metrics(self, bot_id: str, hours: int = 24) -> dict[str, Any]:
        ...
        try:
            ...
            for snapshot in self._state_cache.values():
                if snapshot.bot_id == bot_id and snapshot.timestamp >= start_time:
                    metrics["checkpoints_count"] += 1
                    metrics["storage_size_bytes"] += snapshot.size_bytes

                    if not metrics["last_update"] or snapshot.timestamp > datetime.fromisoformat(
                        metrics["last_update"]
                    ):
            ...
            for transaction in self._transaction_log.values():
                if (
                    ...
                    and transaction.timestamp >= start_time
    ...
    async def cleanup_old_state(self, bot_id: str, keep_count: int = 50) -> int:
        ...
        try:
            ...
            bot_snapshots = [
                snapshot for snapshot in self._state_cache.values() if snapshot.bot_id == bot_id
            ]
    ...
    async def _create_state_snapshot(
        self, bot_id: str, bot_state: BotState, version_id: str | None = None
    ) -> str:
    ...
    async def _load_state_snapshot(self, bot_id: str, snapshot_id: str) -> StateSnapshot | None:
    ...
    async def _log_state_metrics(self, bot_id: str, bot_state: BotState, version_id: str) -> None:
        ...
        try:
            metrics_data = {
                ...
                "tags": {"bot_id": bot_id, "status": bot_state.status.value},
                "fields": {
                    "version_id": version_id,
    ...
    async def _cleanup_task(self) -> None:
        ...
        while True:
            try:
                ...
                old_transactions = [
                    tid for tid, txn in self._transaction_log.items() if txn.timestamp < cutoff_time
                ]
